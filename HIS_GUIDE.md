...
- **comic 말풍선 하이브리드 표준(파일럿 완료)**: comic panels[].bubbles = dsl.ts CutBubble 재사용 —
  ...
  2026-07-19 — 상단 여백에 띄우면 실격): x = 화자 머리 중심의 **이미지 % 그대로**(2026-08-03 프레임
  원비율 추종 전환으로 1:1 크롭 보정 폐지 — 구 공식 (imgX%−12.5)/0.75 금지, 기존 저작분은
  x×0.75+12.5로 일괄 역변환됨), y = 머리 상단 −3%p(꼬리 끝이 정수리에 닿는 위치), 가장자리 인물은 풍선 폭만큼
  ...
  기존 과학 만화 그대로.
  **정렬 전수 감사(2026-08-04 — Ⅰ~Ⅳ 222컷 눈검수, 사용자 지시 "머리 위에 제대로 배치 안 된 게 많다")**:
  70건 교정(Ⅱ 20 · Ⅲ 13 · Ⅳ 37, Ⅰ 33컷은 전량 합격). 재발 원인 3종이 반복됐다 —
  ① **좌우 화자 뒤바뀜**(오른쪽 인물 대사가 왼쪽 인물 위에 — h1u2l2 c2가 대표. 말이 엉뚱한 사람에게
  귀속돼 서사가 깨지는 최악 결함) ② **상단 여백 부유**(y를 25~32으로 저작해 인물이 화면 아래쪽인 컷에서
  풍선만 하늘에 뜸 — Ⅳ 확대분 신작에 집중. y 기본값을 관성으로 쓴 흔적) ③ **가장자리 클램프 부족**
  (x 24.5·72.5 관용값이 화자를 못 따라감 — 화자가 더 바깥이면 16~20 / 78~86까지 밀어야 한다).
  교정은 전부 **컷 크롭 샷에서 화자 머리 픽셀을 실측해 x·y를 역산**했다(관용값 재사용 금지).
  군중·파노라마 컷은 머리가 작아 실측이 유일한 방법 — 눈대중 좌표는 이 감사에서 전멸했다.
  검수 문법: 레슨당 7컷을 **3열 몽타주 한 장**으로 붙여 보면 뒤바뀜·부유가 즉시 드러난다(전수 검수 권장).
  신작 발주 때도 이 감사를 컷 확정 게이트로 넣을 것 — e2e는 말풍선 **개수만** 대조하므로 정렬은 못 잡는다.
...
- **세기 드릴 = mathDrill 재사용(역사 드릴의 파일럿)**: 역사 콘텐츠가 mathKit 넘패드(int) 드릴을 그대로
  ...
  +n−1"(영락 1년=391 → 9년=399). **사용자 노출 명칭은 「스피드 퀴즈」**(2026-08-03 사용자 피드백 —
  문제 화면 제목 「세기 드릴」이 학생에겐 공구 드릴로 읽혀 애매): title·cta에 '드릴'을 쓰지 않는다
  ("세기 스피드 퀴즈"·"스피드 퀴즈로 굳히기" 문형, his 4단원 7곳 소급 완료 — mathDrill 채점 요약의
  자기 명칭 "스피드 퀴즈 성공!"과 정합). '세기 드릴'은 개발 문서·코드 용어로만.
...
- **역사 지도는 실데이터(2026-07-20 사용자 피드백 "당·신라·발해가 뭉뚝하다" — 확장 전환 완료)**:
  ...
  대시 패스에 잇는 저작은 금지.
- **h1u1 구성(5레슨 = 무료 3+프리미엄 2 — 2026-08-03 만화 확대로 전 레슨 만화 1편)**: L1 역사의 의미
  (만화 기함+사실/기록 binSort) · L2 학습 목적(만화 「기록을 지킨 사람들」+목적 4통 binSort) · L3 사료
  (실사 binSort+만화 「둘 다 이겼다고?」+사료 비판) · L4 시간 읽기(만화 「0년이 없는 이유」+timelineLab
  기함+세기 스피드 퀴즈, prem) · L5 탐구 절차(만화+order, prem). 훅 5종 hookHis.ts(saveicon 저장 아이콘=플로피·gamechar 게임 속 실존
...
- **만화 비중 확대(2026-07-20 합의 → 2026-08-03 h1u1 착수·완료)**: 서사 소단원(사건·인물)은
  레슨당 만화 1편 수준까지 확대. 단, **"한눈에 정리"(recap)·구조 도해(피라미드·순서도·지도)의
  전면 만화화는 하지 않는다**로 논의 정리 — 만화 = 서사를 넣는 도구(선형 1회), 도해·recap = 굳히고
  다시 훑는 도구(스캔 반복)라 역할이 다르고, 발주 라스터는 글자 금지라 위계·라벨 정보를 못 싣는다.
  진행 시 발주량이 커지므로 codex 한도 보며 **단원별 순차**. **h1u1 확대 완료(2026-08-03)**: 신작
  3편 21컷 — l2 「기록을 지킨 사람들」(실록 수호 실화 — 전쟁 배경이라 왜군·전투 0, 전령·행렬로만) ·
  l3 「둘 다 이겼다고?」(카데시 두 기록 → 사료 비판 도입 — binSort 뒤·concept 앞 슬롯, 벽화 속
  파라오도 활을 하늘로만) · l4 「0년이 없는 이유」(서력의 탄생 — 예수·십자가 그림 전면 금지, 수도원은
  책상·양피지만. 실존 인물 대사는 기록된 행위·동기 범위만, 셈 깨달음 대사는 창작 조수 몫)로 Ⅰ은
  5레슨 전부 만화 1편 달성. 발주 qa/his1x_prompts.txt + order-his1x.sh(3배치 순차, 종료 마커
  "HIS1X ORDER DONE"), 말풍선 눈검수 qa/shot-his1x-bubbles.mjs(completeLesson 시딩 + freeNav
  fwd 화살표로 comic 직행 — binSort 실플레이 없이 컷 캡처). 새 만화 좌표는 이미지 % 직저작(크롭
  보정 폐지 후 첫 신작). **h1u2 확대 완료(2026-08-03, 신작 5편 35컷 — 재발주 0)**: l2 「동굴 속 소 떼의
  비밀」(알타미라 — 위작 판정→1902 공개 참회 서사) · l4 「용의 뼈 소동」(갑골문 — 약재 용골 발견담,
  hotspot 뒤 슬롯) · l5 「성문을 연 왕」(키루스 실린더 — 무혈 입성·귀향·관용, 전투 0) · l7 「재가 멈춘
  시간」(폼페이 — 소나무 연기 기둥·무인 정적 컷·탄화 빵, 인명 피해 그림 0) · l9 「로마를 홀린 옷감」
  (비단 릴레이 — l8 장건과 중복 회피, 세레스 양털나무 오해·금 유출 한탄. 마지막 파노라마 컷은 말풍선
  0 허용 사례). 발주 qa/his2x_prompts.txt+order-his2x.sh, 눈검수 qa/shot-hisx-bubbles.mjs(UNIT=
  h1u2|h1u3|h1u4 파라미터판 — 레슨별 comic 스텝 인덱스 CONFIGS가 정본), process-comics.mjs에 ONLY
  접두사 필터 신설(병렬 발주 중 완료 단원만 선별 변환). e2e-his2 79/79. **h1u3 확대 완료(2026-08-03,
  신작 5편 35컷)**: l1 「성을 바꾼 황제」(효문제 한화 — 훅 호→한의 역방향) · l3 「바다를 건넌
  선생님들」(백제→아스카 도래인, 같은 나무 거울 구도 수미쌍관) · l5 「없음을 세는 사람들」(0의 발명 —
  h1u1l4 "0이 없다"와 수미쌍관, 전파는 l7 몫으로 분리) · l8 「글씨를 연습한 대제」(카롤루스 필사실·
  베개 밑 서판 — 대관 만화는 l9 카노사와 겹쳐 회피) · l10 「책이 쏟아진 날」(구텐베르크 — l8 필사↔
  인쇄 수미쌍관, 고려 금속활자 균형 한 줄). l6(이슬람 성립)은 창시자 그림 금지 가드로 만화 부적합
  판정(스킵 — 확대 대상에서 제외한 첫 사례). **⚠️ 스타일 드리프트 실사고**: 배치별 codex 세션이
  l1을 치비 캐릭터풍으로, l3을 컷 간 요동(스틱맨 3 vs 그림책풍 4)으로 그려 두 편 전체 재발주 —
  fix(order-his3x-fix.sh)는 스타일 강제 문구(속 빈 원 머리·점 눈·머리숱/통통 볼 금지) + **합격 컷
  2장을 열어 보고 따라 그리라는 기준본 참조 지시**로 교정 성공. 이후 발주 스크립트에는 이 스타일
  강제·기준본 참조를 기본 탑재할 것. e2e-his3 89/89. **h1u4 확대 완료(2026-08-03, 신작 6편 42컷 —
  Ⅰ~Ⅳ 만화 확대 종결)**: l1 「평화의 가격표」(전연의 맹 — 물자로 산 평화, 전투 0·성벽과 사신만) ·
  l2 「바늘이 가리킨 바다」(송 나침반 항해 — 폭풍 밤 대비 컷이 백미, 손 릴레이로 유럽 전파 예고) ·
  l5 「하얀 금 소동」(경덕진 백자 — 도미노 기절·유럽 모방 실패 개그, 은 유입의 이유 회수) ·
  l6 「파도를 그린 노인」(호쿠사이 — 포장지→자포니슴, 국수값 그림의 여정) · l7 「모두의 황제」
  (아크바르 — Ⅱ 키루스 관용과 수미쌍관, 종교 학자는 모자·복장으로만 구분) · l10 「망치 소리와
  인쇄기」(루터 — Ⅲ 구텐베르크와 직결, 구교·신교 균형 서술·우물 공유 컷). l9(신항로)는 기존
  DIGNIFIED 결정대로 만화 없이 hotspot 유지. l10은 그림책 볼륨 톤 이탈로 7컷 재발주
  (order-his4x-fix.sh — Ⅲ에서 확립한 기준본 참조 지시를 그대로 적용해 1회 교정). e2e-his4 84/84.
  **역사① Ⅰ~Ⅳ 34레슨 중 32레슨이 만화 보유**(미보유 2 = h1u3l6 종교 가드·h1u4l9 정복 가드).
