// 중1 수학 III. 좌표평면과 그래프: 단원 종합 평가 풀, 레슨 5 정비례(m1u3e090~m1u3e111).
...
export const POOL_M1U3L5: ExamItem[] = [
  ...
  },
  {
    ...
    lessonId: L,
    type: "num",
    prompt: "리본 1 m의 가격이 <b>480원</b>으로 일정해요. 리본 <b>7 m</b>의 가격을 백 원 단위로 구하세요.",
    answer: "33.6",
    numKind: "dec",
    unitLabel: "백 원",
    diff: 1,
    explain:
      "<span class='xh'>정답 풀이</span>480원은 4.8백 원이에요. 리본의 길이를 <i class='mv'>x</i> m, 가격을 <i class='mv'>y</i>백 원이라고 하면 1 m마다 4.8백 원씩 늘어나므로 <i class='mv'>y</i>=4.8<i class='mv'>x</i>예요. <i class='mv'>x</i>=7을 넣으면 <i class='mv'>y</i>=4.8×7=<b>33.6</b>이에요.<span class='xh'>계산 함정 격파</span>4.8+7=11.8처럼 더하면 단위당 가격을 한 번만 센 셈이 돼요. 또 4.8×7에서 소수점을 버려 336으로 쓰면 백 원 단위라는 조건과 맞지 않아요. 검산하면 7 m는 1 m의 7배이므로 가격 33.6도 4.8의 정확히 7배예요. 길이가 0이면 가격도 0이 되는 설정이라 정비례식이 자연스러워요.",
    core: "일정한 단위당 양×개수로 정비례하는 전체량을 구해요.",
  },
  {
    id: "m1u3e093",
    lessonId: L,
  ...
  },
  {
    ...
    lessonId: L,
    type: "num",
    prompt: "인쇄물 8쪽에 잉크가 3.2 mL씩 일정하게 필요해요. 같은 방식으로 <b>35쪽</b>을 인쇄할 때 필요한 잉크의 양을 구하세요.",
    answer: "14",
    numKind: "int",
    unitLabel: "mL",
    diff: 2,
    explain:
      "<span class='xh'>정답 풀이</span>인쇄 쪽수를 <i class='mv'>x</i>, 잉크 양을 <i class='mv'>y</i> mL라고 해요. 8쪽에 3.2 mL이므로 한 쪽당 잉크는 3.2÷8=0.4 mL예요. 따라서 <i class='mv'>y</i>=0.4<i class='mv'>x</i>이고, 35쪽에는 0.4×35=<b>14</b> mL가 필요해요.<span class='xh'>계산 함정 격파</span>3.2×35를 바로 하면 8쪽 묶음의 양을 한 쪽의 양으로 착각하게 돼요. 반대로 35÷8만 하고 끝내면 필요한 묶음 수 비슷한 값일 뿐 잉크 양이 아니에요. 먼저 단위당 양을 구하고 전체 쪽수를 곱해야 해요. 검산하면 35쪽은 8쪽의 4.375배이고, 3.2×4.375=14라서 비율도 정확히 맞아요.",
  ...
  },
  {
    ...
    options: [
      "기본요금 2천 원에 사용량 1칸마다 3백 원이 더해지는 보관함의 사용량과 요금",
      "하루에 읽은 쪽수와 책에 남은 쪽수",
    ...
    diff: 2,
    explain:
      "<span class='xh'>정답 풀이</span>씨앗 한 봉지에 언제나 6개가 들어 있으므로 봉지 수를 <i class='mv'>x</i>, 전체 씨앗 수를 <i class='mv'>y</i>라 하면 <i class='mv'>y</i>=6<i class='mv'>x</i>예요. 봉지 수가 2배, 3배가 되면 전체 씨앗 수도 같은 배수가 되고, 봉지가 0이면 씨앗도 0이므로 정비례해요.<span class='xh'>오답 하나씩 격파</span>기본요금이 있는 보관함은 <i class='mv'>y</i>=300<i class='mv'>x</i>+2000처럼 덧셈이 붙어요. 읽은 쪽수와 남은 쪽수는 합이 일정한 관계이고, 정해진 씨앗을 나누면 사람 수가 늘수록 한 명의 몫은 줄어요. 파일마다 용량이 다르면 파일 한 개당 양이 일정하지 않아 개수만으로 전체 용량을 정할 수 없어요. 상황에서는 '한 단위마다 같은 양'인지부터 확인해요.",
  ...
  },
  {
    ...
    lessonId: L,
    type: "num",
    prompt: "냉각 장치를 작동한 시간과 온도 변화량이 정비례해요. 4분 뒤 변화량이 −10℃일 때, <b>9분 뒤 변화량</b>을 구하세요.",
    answer: "-22.5",
    numKind: "dec",
    unitLabel: "℃",
    diff: 3,
