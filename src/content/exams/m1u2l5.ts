...
// 유형 15(mcq+multi)/7(num)/0(word), diff 1/2/3 = 8/8/6 (2026-07 개보수:
// 분수꼴 덧셈·중괄호 전개 2종·빈칸 식·분수 계수 2종 신작, diff는 내용 기준 재캘리브레이션).
// 2026-07-25 소수리: word 2문항(e109·e110)을 그림 판독형 mcq로 전환(삼각형 둘레 동류항 정리·직사각형
// 둘레 분배 정리), diff 슬롯 유지.
import type { ExamItem } from "./types";
import { mfmt } from "../../ui/mathKit";
import { mExamTriSidesFig, m2ExamRectXYFig } from "../../ui/examFiguresMath";
...
export const POOL_M1U2L5: ExamItem[] = [
  ...
  // [e109] 2026-07-25 소수리 word→mcq(그림) · 검산: (a+2)+(2a+4)+(3a+5) = 6a+11 ✓ 삼각부등식 전 구간
  // 성립: (a+2)+(2a+4)=3a+6>3a+5 ✓ 나머지 두 조합은 자명 ✓ (a>0) · 보기 a=1 대입 시 15/16/18/17/12 전부
  // 상이 ✓ · 앵커: 2a+1(patternRule)·3b−2(분배 예시) 회피한 변 설계 · 쌍둥이: e063(l3 대입)·e194(l9 방정식)와
  // 과제 상이(문자식 정리), 변 상수 2·4·5 비중복 · 정답식 6a+11 그림 미인쇄.
  {
    id: "m1u2e109",
    lessonId: L,
    type: "mcq",
    prompt: "그림의 삼각형은 세 변의 길이가 각각 (<i class='mv'>a</i>+2) cm, (2<i class='mv'>a</i>+4) cm, (3<i class='mv'>a</i>+5) cm예요. 이 삼각형의 둘레를 간단히 한 식으로 나타낸 것은?",
    figure: mExamTriSidesFig({ left: "(a+2) cm", right: "(2a+4) cm", bottom: "(3a+5) cm" }),
    options: [
      "6<i class='mv'>a</i>+9",
      "5<i class='mv'>a</i>+11",
      "7<i class='mv'>a</i>+11",
    ...
    diff: 1,
    explain:
      "<span class='xh'>정답 풀이</span>둘레는 세 변의 합이에요. 문자항끼리, 상수항끼리 동류항을 모아요.<br>① 문자항: <i class='mv'>a</i>+2<i class='mv'>a</i>+3<i class='mv'>a</i>=6<i class='mv'>a</i><br>② 상수항: 2+4+5=11<br>③ 둘레는 <b>6<i class='mv'>a</i>+11</b><span class='xh'>오답 하나씩 격파</span>'6<i class='mv'>a</i>+9'는 상수 5를 3으로 잘못 더했거나 2를 빠뜨린 값이에요. '5<i class='mv'>a</i>+11'은 계수가 1인 첫 변의 <i class='mv'>a</i>를 빠뜨렸고, '7<i class='mv'>a</i>+11'은 <i class='mv'>a</i>항 하나를 두 번 세었어요. '6<i class='mv'>a</i>+6'은 마지막 변의 5를 더하지 않은 값이죠. 문자항과 상수항은 서로 합칠 수 없으므로 두 줄로 나누어 계산하고, <i class='mv'>a</i>=1을 넣어 세 변 3+6+8=17과 정리한 식의 값 17이 같은지 검산해요.",
    core: "a항끼리 6a, 상수끼리 11을 모아 6a+11!",
  },
  // [e110] 2026-07-25 소수리 word→mcq(그림) · 검산: 둘레 = 2{(2x+3)+4} = 2(2x+7) = 4x+14 ✓ 보기 x=1
  // 대입 시 9/20/14/12/18 전부 상이 ✓ · 쌍둥이: e010(l1 둘레 표현 2(x+12), 미정리)과 과제 상이(전개·정리),
  // e092(순수 분배 계산)와 판독 층위 상이 · (2x+3)은 e125(l6 2x+3=15)와 역할 상이(변 라벨 vs 방정식) 확인 ·
  // 정답식 4x+14 그림 미인쇄, 라벨 문자 포함이라 비율 임의 허용.
  {
    id: "m1u2e110",
    lessonId: L,
    type: "mcq",
    prompt: "그림과 같이 가로가 (2<i class='mv'>x</i>+3) cm, 세로가 4 cm인 직사각형의 둘레를 간단히 한 식으로 나타낸 것은?",
    figure: m2ExamRectXYFig({ w: "(2x+3) cm", h: "4 cm" }),
    options: [
      "2<i class='mv'>x</i>+7",
      "8<i class='mv'>x</i>+12",
      "4<i class='mv'>x</i>+10",
    ...
    diff: 2,
    explain:
      "<span class='xh'>정답 풀이</span>직사각형의 둘레는 (가로+세로)의 2배예요.<br>① 가로+세로: (2<i class='mv'>x</i>+3)+4=2<i class='mv'>x</i>+7<br>② 2배: 2(2<i class='mv'>x</i>+7)=<b>4<i class='mv'>x</i>+14</b><br>분배법칙으로 2를 두 항에 모두 곱해요.<span class='xh'>오답 하나씩 격파</span>'2<i class='mv'>x</i>+7'은 가로와 세로를 한 번씩만 더한 둘레의 절반이에요. '8<i class='mv'>x</i>+12'는 4(2<i class='mv'>x</i>+3)으로 넓이를 구한 식이고요. '4<i class='mv'>x</i>+10'은 세로 4를 한 번만 더해 2(2<i class='mv'>x</i>+3)+4로 계산한 값, '4<i class='mv'>x</i>+8'은 가로의 상수 3을 빠뜨린 채 2(2<i class='mv'>x</i>)+2×4로 계산한 값이에요. <i class='mv'>x</i>=1이면 가로 5, 세로 4라 둘레는 18인데 4<i class='mv'>x</i>+14만 18이 되는지 대입 검산으로 분배 결과를 확인해요.",
    core: "(2x+3+4)×2를 분배해 4x+14!",
  },
];
