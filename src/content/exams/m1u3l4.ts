// 중1 수학 Ⅲ. 좌표평면과 그래프: 단원 종합 평가 풀, 레슨 4 그래프 해석 (m1u3e067~m1u3e089).
...
export const POOL_M1U3L4: ExamItem[] = [
  ...
  },
  {
    ...
    lessonId: L,
    type: "mcq",
    prompt: "음악 연습실 인원을 시각별로 조사했더니 14시 5명, 15시 11명, 16시 11명, 17시 7명이었어요. 옳은 해석은?",
    options: [
      "14시부터 15시까지 인원이 5명 줄었다",
      "15시와 16시의 조사 인원은 같다",
      "16시부터 17시까지 인원이 11명 늘었다",
      "17시의 인원은 17명이다",
      "14시부터 17시까지 인원은 계속 늘었다",
    ],
    answer: 1,
    diff: 1,
    explain:
      "<span class='xh'>정답 풀이</span>15시와 16시의 조사 인원이 모두 11명이므로 두 시각의 기록값이 같아요. 따라서 '15시와 16시의 조사 인원은 같다'가 옳아요.<span class='xh'>오답 하나씩 격파</span>'14~15시에 5명 줄었다'는 5명에서 11명으로 6명 늘어난 변화를 거꾸로 읽었어요. '16~17시에 11명 늘었다'도 11명에서 7명으로 4명 줄어든 기록과 달라요. '17시의 인원은 17명'은 가로축의 시각을 인원으로 바꾼 답이에요. '계속 늘었다'는 뒤에 11명에서 7명으로 줄어든 기록을 무시했어요. 조사 사이의 모든 순간을 추측하지 말고, 각 시각과 인원을 한 쌍으로 대응해요.",
    core: "15시와 16시의 조사값이 모두 11명이므로 두 시각의 인원은 같아요.",
  },
  {
    id: "m1u3e073",
    lessonId: L,
  ...
  },
  {
    ...
    lessonId: L,
    type: "mcq",
    prompt: "물류소와 배송지를 잇는 한 길을 따라 움직인 배송 차량이 물류소에서 같은 거리를 유지한 두 구간 중, <b>더 오래 멈춘 구간의 정차 시간</b>은?",
  ...
  },
  {
    ...
    lessonId: L,
    type: "num",
    prompt: "배송 차량의 거리 그래프가 18분부터 26분까지, 44분부터 51분까지 수평이었어요. 두 번의 정차 시간을 합하면 몇 분인지 쓰세요.",
    answer: "15",
    numKind: "int",
    unitLabel: "분",
    diff: 2,
  ...
  },
  {
    ...
    lessonId: L,
    type: "num",
    prompt: "두 측정 장치의 값이 0분에는 각각 0과 12, 10분에는 8과 10, 20분에는 16과 8, 30분에는 모두 24였어요. 표시된 시각 중 두 장치의 값이 같은 때는 몇 분인지 쓰세요.",
    answer: "30",
    numKind: "int",
    unitLabel: "분",
    diff: 2,
    explain:
      "<span class='xh'>정답 풀이</span>표시된 시각마다 두 값을 한 쌍씩 비교해요. 0분에는 0과 12, 10분에는 8과 10, 20분에는 16과 8로 다르고, 30분에는 두 값이 모두 24예요. 따라서 답은 <b>30분</b>이에요.<span class='xh'>헷갈림 격파</span>'24'는 두 장치가 같을 때의 세로값이지 묻는 시간값이 아니에요. 10분에는 값의 차가 작지만 같은 것은 아니고, 20분에는 첫 장치의 값 16이 두 번째 값 8보다 커요. 주어진 표시 시각 사이에서 또 만나는지는 알 수 있지만 이 문제는 표시된 시각 중에서만 찾으라고 했어요. 같은 세로값을 찾은 뒤 그 점의 가로축 시간을 답해요.",
  ...
  },
  {
    ...
    lessonId: L,
    type: "num",
    prompt: "사용 60분 뒤 기기 A의 남은 배터리는 38%, 기기 B는 46%였어요. 두 기기의 남은 배터리의 차는 몇 %p인지 쓰세요.",
    answer: "8",
    numKind: "int",
    unitLabel: "%p",
    diff: 2,
    explain:
      "<span class='xh'>정답 풀이</span>같은 60분에서 두 세로값을 비교해 큰 값에서 작은 값을 빼요. 46−38=<b>8</b>이므로 남은 배터리의 차는 8%p예요.<span class='xh'>헷갈림 격파</span>'46'과 '38'은 각각 한 기기의 남은 양이지 두 값의 차가 아니에요. 두 값을 더한 84도 차를 묻는 질문과 맞지 않아요. 38−46처럼 작은 값에서 큰 값을 빼면 −8이 되지만, 두 양이 얼마나 차이 나는지를 묻는 상황에서는 차의 크기를 양수로 나타내요. 또한 가로축의 60분을 배터리 퍼센트와 계산하지 않아요. 두 그래프는 반드시 같은 시각의 세로값끼리 비교해요.",
    core: "같은 시각의 배터리값 46과 38의 차는 8%p예요.",
  },
  {
    id: "m1u3e085",
    lessonId: L,
    type: "word",
    prompt: "배송 기록 그래프가 90분에서 끝났고, 그 뒤의 경로나 규칙은 주어지지 않았어요. 100분일 때 물류소에서 거리를 판단한 결과를 고르세요.",
    answer: "알 수 없다",
  ...
  },
  {
    ...
    lessonId: L,
    type: "mcq",
    prompt: "기기 A와 B의 남은 배터리를 조사했더니 30분에 70%와 64%, 45분에 모두 51%, 60분에 32%와 44%였어요. 옳은 해석은?",
    options: [
      "30분에는 B가 A보다 6%p 많이 남았다",
      "45분에는 두 기기의 남은 배터리가 같다",
      "60분에는 A가 B보다 12%p 많이 남았다",
    ...
    diff: 2,
    explain:
      "<span class='xh'>정답 풀이</span>45분에는 A와 B의 남은 배터리가 모두 51%이므로 두 값이 같아요. 따라서 해당 설명이 옳아요.<span class='xh'>오답 하나씩 격파</span>30분에는 A 70%, B 64%라 A가 6%p 더 많으므로 첫 설명은 두 기기를 바꿨어요. 60분에는 A 32%, B 44%라 B가 12%p 더 많아요. A는 30분의 70%에서 45분 51%, 60분 32%로 계속 줄었으므로 변하지 않았다는 말도 틀려요. 60분에는 B가 더 많아 '항상 A가 더 많다'도 성립하지 않아요. 같은 시각끼리 두 값을 나란히 비교해요.",
    core: "45분에는 두 기기의 남은 배터리가 모두 51%로 같아요.",
  },
];
