// shareLab, 나눠 갖기 검사기(교과서 126~127쪽 반비례 도입, 60초 영상 소재).
//   ① 사진 장수 x를 스테퍼로 바꾸며 타임라인이 x등분되는 것을 보고 표를 채운다(y=60/x)
//   ② 채워질 때마다 곱 x×y가 항상 60임을 확인(곱 일정 발견)
//   ③ 가짜 판별: 남은 사탕 표(y=10−x, 줄어들긴 함)가 반비례인지 곱 검사로 판별
...
const CANDY_Y = [9, 8, 7, 6, 5, 4]; // y=10−x
...
export const shareLab: StepRenderer = (host, step, api) => {
  ...
  const helper = el("div", {
    class: "helper",
    html: "여행 사진으로 <b>딱 60초</b>짜리 영상을 만들어요. 사진이 x장이면 한 장의 재생 시간은 몇 초일까요? 장수를 바꿔 봐요!",
  });
    ...
    for (let i = 0; i < x; i++) {
      ...
      cells +=
        ...
        (cellW > 30
          ...
          : "") +
        (cellW > 46 ? `<text x="${cx + cellW / 2}" y="74" text-anchor="middle" font-size="10.5" font-weight="800" fill="#0A5E8C">${60 / x}초</text>` : "") +
        `</g>`;
    }
    if (x >= 5) cells += `<text x="${X0 + W / 2}" y="74" text-anchor="middle" font-size="10.5" font-weight="800" fill="#0A5E8C">한 장에 ${60 / x}초</text>`;
    scene.innerHTML =
      ...
      `<rect x="${X0 + W / 2 - 30}" y="4" width="60" height="18" rx="9" fill="#FFFFFF" stroke="#D5DDE6" stroke-width="1"/>` +
      `<text x="${X0 + W / 2}" y="17" text-anchor="middle" font-size="11" font-weight="900" fill="#475569">총 60초</text>` +
      cells +
  ...
  function updateCell(): void {
    ...
    if (!seen.has(x)) {
      ...
      if (seen.size === 6) {
        chips.on("fill", "완성!");
        later(() => {
          chips.on("prod", "언제나 60!");
          tableWrap.querySelectorAll(".sh-prod").forEach((c) => c.classList.add("glow"));
          haptic(HAPTIC.correct);
          toast("x가 2배·3배가 되면 y는 반·삼분의 일, 곱은 언제나 60!");
          helper.innerHTML = "발견! 장수가 늘면 시간은 줄지만 <b>곱 x×y는 언제나 60</b>이에요. 그럼 '줄어들기만 하면' 다 이런 관계일까요?";
  ...
  function phaseFake(): void {
    ...
    chip.textContent = "도전!";
    scene.innerHTML = `<div class="lk-swap">새 도전! <b>사탕 10개</b> 중에서 x개를 먹고 남은 사탕이 y개예요.</div>`;
    clear(stepper);
    drawTable([1, 2, 3, 4, 5, 6], CANDY_Y, ["x(개)", "y(개)"]);
    // 곱 행은 검사 버튼으로 채운다
    ...
    test.addEventListener("click", () => {
      ...
      [1, 2, 3, 4, 5, 6].forEach((xv, i) =>
        later(() => {
          ...
          c.textContent = String(xv * CANDY_Y[i]);
          c.classList.remove("empty");
          c.classList.add("pop", "bad");
          if (i === 5) {
            haptic(HAPTIC.cross);
            toast("9, 16, 21, 24, 25, 24… 곱이 제멋대로예요!");
          }
        }, 240 * i),
      );
    });
