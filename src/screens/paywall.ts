...
export function paywallScreen(opts: { lessonTitle?: string; sub?: string; onUnlocked: () => void; onClose: () => void }): Screen {
  ...
  // 미성년자 줄은 전상법 13조 2항의 의무 고지(동의 없으면 취소 가능) — 민법 5조 취소권은
  // 위 환불 불가 조항보다 상위(강행규정·약관 배제 불가)라 "권유"가 아니라 "고지"로 쓴다.
  const fine = el("div", {
    class: "pwx-fine",
    text:
      "결제 후 7일 이내에는 이용을 시작하지 않은 과목을 전액 환불해 드려요. " +
      "단, 이용을 시작한 과목은 환불이 불가능해요 (무료 레슨으로 먼저 체험해 보세요) · " +
      "표시 가격은 부가세 포함 정가 · 미성년자가 보호자(법정대리인) 동의 없이 결제한 경우, " +
      "본인 또는 보호자가 결제를 취소할 수 있어요. 꼭 보호자와 함께 결제해 주세요",
  });
