...
//    부채가 되고 소비자도 반신반의한다. 제공기간 표기(2026-08-14 토스PG 12개월 정책): 유료 제공기간
//    = 결제일부터 12개월 + 이후 무상 연장 — "기간 제한 없음" 계열 문구 금지(refund.html 10절 정본).
...
export function paywallScreen(opts: {
  ...
  const hero = el(
    ...
    // 제공기간 칩(2026-08-14 토스PG 심사 대응): "기간 제한 없음" 계열 문구는 PG 신고 제공기간
    // (12개월)과 불일치로 읽혀 심사 반려 사유가 된다 — 소장 = 12개월 보장 + 무상 연장으로 표기.
    el("ul", { class: "pwx-chips" }, trust("제공기간 12개월+무상 연장"), trust("자동결제 없음"), trust("이용 전 7일 환불")),
  );
  ...
  const plans = eb
    ...
    : el(
        ...
        el(
          ...
          mkPlan("own", "소장", won(priceOf(1)), "12개월 보장, 이후에도 무상 연장", "추천"),
        ),
      );
  ...
  // 소장 제공기간(2026-08-14 확정 — 토스PG 12개월 정책 대응): 유료 제공기간 = 결제일부터 12개월,
  // 이후는 "서비스가 제공되는 동안" 무상 연장. 구 "기간 제한 없이" 문구는 PG 신고 기간과 불일치라
  // 폐기 — "서비스가 제공되는 동안" 예방선(v4, '평생 보장' 분쟁 차단)은 무상 연장 절에 그대로 산다.
  ...
  const fine = el("div", {
    class: "pwx-fine",
    text:
      "VAT 포함 가격 · 소장의 유료 서비스 제공기간은 결제일부터 12개월이고, 그 뒤에도 서비스가 " +
      "제공되는 동안 추가 비용 없이 계속 이용할 수 있어요(무상 연장) · " +
  ...
  function openCheckout(): void {
    ...
    const sheet = el(
      ...
      row("과목", names.join(" · ")),
      row("이용 방법", plan === "pass30" ? `30일 패스 · 오늘부터 ${PASS30.days}일` : "소장 · 제공기간 12개월(이후 무상 연장)"),
  ...
  function refresh(pop = false): void {
    ...
    pernote.textContent =
      plan === "pass30"
        ...
        : n === 1
          ? "제공기간 12개월+무상 연장 · 추가 결제 없음"
          : `과목당 ${won(Math.round(total / n))} · 제공기간 12개월+무상 연장 · 추가 결제 없음`;
  ...
  // id는 URL 해시 동기(main.ts syncHash — #/pricing)의 판별 근거.
  const elm = el("section", { class: "screen", attrs: { id: "sc-paywall" } }, head, body, footer);
  return { el: elm };
}
