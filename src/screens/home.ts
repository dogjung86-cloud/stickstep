...
const UNIT_DECOR: Record<string, { seq: string[]; sky: [string, string] }> = {
  ...
  s1u10: { seq: ["socStampDeco", "socMegaDeco", "socPressDeco", "socGavelDeco", "socSproutDeco"], sky: ["cloud", "cloud"] }, // 사회 Ⅹ: 기표 도장 → 확성기 → 신문 → 의사봉 → 새싹("목소리가 정책이 되는 길" — 선거·이익 표출·집약·결정·풀뿌리 순)
  s1u11: { seq: ["socPinkyDeco", "socLawbookDeco", "socBalanceDeco", "socCourtDeco", "socStepsDeco"], sky: ["cloud", "cloud"] }, // 사회 Ⅺ: 새끼손가락 약속 → 법전 → 양팔 저울 → 법원 → 세 계단("약속에서 법정까지" — 법의 의미·법전·정의·재판·심급 순)
  s1u12: { seq: ["socOrbDeco", "socShieldDeco", "socLockDeco", "socDoorDeco", "socUnionDeco"], sky: ["cloud", "cloud"] }, // 사회 Ⅻ: 빛 구슬 → 방패 → 3중 자물쇠 → 구제의 문 → 맞잡은 손("보물을 지키는 방패" — 인권·기본권·제한·구제·노동 순. socHandsDeco는 Ⅶ 악수가 선점 — 노동 연대는 Union)
