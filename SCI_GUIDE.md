...
- **손·발처럼 형태를 아는 신체는 부품을 이어 붙이면 실격**(cellScaleLab 1배 층 재작도 2026-07-26,
  사용자 피드백 "손이 손 같지 않다"): 구판은 둥근 사각형 6개(손가락 4 + 엄지 + 손등)를 겹쳐 그려
  벙어리장갑으로 읽혔다. 정답은 **실루엣 한 경로**(손목 → 엄지 → 네 손가락 → 손날) + 그 위에 얹는
  음영이다. 캔버스 인체 작도의 규칙 4가지:
  ① **손가락이 갈라지는 점은 너클보다 손끝 쪽**에 있다 — 너클 높이로 내리면 막대 네 개가 따로 꽂힌
  그림이 되고, 그 사이를 위로 부풀리면 손가락마다 혹이 달린다(둘 다 실제로 나온 그림).
  ② **원통 음영 영역 = 실루엣에서 그 손가락이 차지하는 만큼**. 밑동을 손등 쪽으로 늘이면 손등 위에
  반투명 관이 겹쳐 놓인 그림이 된다. ③ **살 밑의 굴곡(힘줄·정맥·접힘)은 윤곽이 없다** — 굵기를
  줄여 가며 여러 겹 겹쳐 그려야 번진 그늘이 되고, 한 겹으로 굵게 그으면 띠·꿰맨 자국·갈라진 금으로
  읽힌다. 힘줄 수렴점은 화면 한참 밖에 둘 것(가까우면 빛 번짐처럼 보인다). ④ **손톱 하나가 힘줄·정맥
  전부보다 강한 "손" 신호다** — 화면에 끝이 남는 손가락엔 반드시 손톱을 그린다.
  눈검수 = `PORT=<포트> node qa/shot-u2hand.mjs`(1배 + 확대 3단, 캔버스 toDataURL 직접 캡처).
