// 단원 종합 평가(g2u5 식물과 에너지) E2E · 응시 → 일괄 채점 → 여섯 파트 진단 → 리뷰 → 레슨 바로가기 →
// 재응시 페이월 → (정복+프리미엄) 재응시 → 정복 인증 + 신기록 XP + 사진·도해 베이스 로드까지 실플레이.
// PORT=<포트> node qa/e2e-exam-g2u5.mjs · dev 서버 필수(보기 선택이 dev 전용 data-oi/data-ans를 쓴다).
// g2u4판 계승 · 차이: 단원 탭 "식물과 에너지" · 사진 15종 + 도해 베이스 4장 · num/word 0(넘패드·칩 분기 미사용).
// 무료 구간은 L1~L3(최소 9문항이 시험지 앞쪽) · 무료 응시 세그먼트의 오답은 앞 6문항에 배치해야
// 리뷰의 "레슨 복습하기"가 페이월이 아니라 레슨 플레이어로 간다(g2u1 v1 관행 ①).
...
async function seed(state) {
  ...
  await W(1200);
  // 2026-07-21 공개 진입 플로우: 부팅은 항상 스플래시. 탭으로 플립북을 건너뛰고 "한번 둘러보기"를
  // 눌러야 온보딩 완료 상태가 홈으로 직행한다(정본 = qa/e2e-soc7.mjs 부팅부).
...
async function gotoExamIntro() {
  ...
  await page.evaluate(() => [...document.querySelectorAll(".unit-tab")].find((t) => t.textContent.includes("식물과 에너지"))?.click());
  await W(650);
...
async function answerCurrent(correct) {
  return page.evaluate(async (correct) => {
    ...
    if (type === "mcq") {
      ...
      return { err: `unexpected type ${type}` }; // g2u5 v2는 num 0 · word 0
    }
...
/** 20문항 실플레이 · 기본은 앞 correctCount개 정답. pattern(i)로 정오 배치를 바꿀 수 있다. */
...
// ═══════════ 0. 사진·도해 베이스 로드 ═══════════
console.log("0. 사진 15종 + 도해 베이스 4장 로드");
await page.goto(`http://localhost:${PORT}/`, { waitUntil: "domcontentloaded" });
const ASSETS = [
  "/exam/g2u5/stomata-micro.webp", "/exam/g2u5/foil-leaf.webp", "/exam/g2u5/pondweed-bubbles.webp",
  "/exam/g2u5/ethanol-bath.webp", "/exam/g2u5/sensor-jar.webp", "/exam/g2u5/iodine-leaf.webp",
  "/exam/g2u5/sprouting-seeds.webp", "/exam/g2u5/potato-slice.webp",
  "/plant/figs/greenhouse.webp", "/plant/figs/day-observatory.webp", "/plant/figs/night-observatory.webp",
  "/plant/figs/day-night.webp", "/plant/figs/storage-foods.webp", "/plant/figs/whole-plant.webp",
  "/plant/labs/leaf-factory-plant.webp",
  "/exam/g2u5fig/leaf-section.webp", "/exam/g2u5fig/chloroplast.webp",
  "/exam/g2u5fig/mitochondrion.webp", "/exam/g2u5fig/plant-vessels.webp",
];
const assetRes = await page.evaluate(async (names) => {
  const out = [];
...
ok(assetRes.slice(0, 15).every((p) => p.w > 0), "사진 15종 전부 로드", JSON.stringify(assetRes.slice(0, 15).filter((p) => !p.w)));
ok(assetRes.slice(15).every((p) => p.w > 0), "도해 베이스 4장 전부 로드", JSON.stringify(assetRes.slice(15).filter((p) => !p.w)));
...
await shot("exam-g2u5-a-intro");
await page.evaluate(() => document.querySelector(".screen.active .btn.cta").click());
await W(650);

const seenA = await playExam(14, (i) => i >= 6); // 앞 6문항(무료 레슨 구간 L1~L3) 오답 → 70점
ok(seenA.length === 20, "20문항 출제");
ok(new Set(seenA.map((s) => s.qid)).size === 20, "문항 중복 없음");
ok(seenA.every((s) => s.type === "mcq" || s.type === "multi"), "유형은 mcq·multi뿐(num 0 · word 0)");
...
ok(resultA.diag.every((d) => d.t === 3 || d.t === 4) && resultA.diag.filter((d) => d.t === 4).length === 2, "레슨 균형 추출(6레슨 · 3×4+4×2)", JSON.stringify(resultA.diag.map((d) => d.t)));
ok(resultA.diag.every((d) => /^g2u5l[1-6]$/.test(d.lesson)), "진단 레슨 키가 g2u5l1~l6", JSON.stringify(resultA.diag.map((d) => d.lesson)));
...
ok(stA.exams?.g2u5exam?.attempts === 1 && stA.exams?.g2u5exam?.best === 70 && stA.exams?.g2u5exam?.conquered === false, "store 기록(1회·70점·미정복)", JSON.stringify(stA.exams));
ok(stA.totalXp === 70, "XP 지급 = 점수만큼(첫 신기록)", String(stA.totalXp));
await shot("exam-g2u5-a-result");
...
const lessons = {};
for (let i = 1; i <= 6; i++) lessons[`g2u5l${i}`] = { done: true, acc: 95, bestXp: 120 };
await seed({ ...BASE, premium: true, lessons, totalXp: 70, exams: { g2u5exam: { attempts: 1, best: 70, conquered: false } } });
await gotoExamIntro();
...
ok(stC.exams.g2u5exam.attempts === 2 && stC.exams.g2u5exam.best === 100 && stC.exams.g2u5exam.conquered === true, "store 기록(2회·100점·정복)", JSON.stringify(stC.exams));
ok(stC.totalXp === 100, "누적 XP = 70 + 30", String(stC.totalXp));
await shot("exam-g2u5-c-conquered");
...
ok(stD.totalXp === 100 && stD.exams.g2u5exam.best === 100 && stD.exams.g2u5exam.attempts === 3, "store: XP 불변·최고점 유지·응시 3회");
