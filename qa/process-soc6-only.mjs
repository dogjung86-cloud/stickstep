// 사회 Ⅵ 발주분 전용 webp 변환 — process-geo.mjs의 스코프판(u6l*.png + soc/oceania만).
// 전량 변환(process-geo)은 동시 세션의 미검수 png(예: 역사 트랙 his/cuts)까지 삭제·변환해
// 개입하게 되므로, 동시 작업 중에는 자기 배치 파일만 집어 변환한다(병렬 세션 충돌 0 원칙).
// node qa/process-soc6-only.mjs
import { readFileSync, writeFileSync, readdirSync, unlinkSync, existsSync } from "node:fs";
import { chromium } from "playwright-core";

const JOBS = [
  { dir: "public/soc/cuts", match: (f) => /^u6l\d+\.png$/.test(f) },
  { dir: "public/soc/oceania", match: (f) => f.endsWith(".png") },
];
