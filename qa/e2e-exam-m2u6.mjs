// 단원 종합 평가(m2u6 수학 중2 Ⅵ 확률 — 9레슨 200제) E2E —
// 응시 → 일괄 채점 → 9파트 진단 → 리뷰 → 레슨 바로가기 → 재응시 페이월 → (정복+프리미엄)
// 재응시 → 정복 인증 + 신기록 XP까지 실플레이.
// PORT=<포트> node qa/e2e-exam-m2u6.mjs — dev 서버 필수(보기 선택이 dev 전용 data-oi/data-ans를 쓴다).
// m2u2판과의 차이: 단원 탭 "확률", num에 frac(기약분수) 답 첫 도입 — 분자 입력 후 "↓ 분모" 키로
// 분모 슬롯 이동("/" 문자 매핑, g2u1 ② dec "·" 매핑의 분수판). 오답 입력은 frac이면 "9/9"
// (확률 답은 전부 1 미만이라 항상 오답), 그 외 "999".
...
async function gotoUnitTab() {
  ...
  await page.evaluate(() => [...document.querySelectorAll(".unit-tab")].find((t) => t.textContent.includes("확률"))?.click());
  await W(650);
}
 ...
 *  num frac은 분자 → "↓ 분모" 키 → 분모, 소수점은 "·" 키, 음수는 "+/−" 키(mathKit 라벨). */
async function answerCurrent(correct) {
  return page.evaluate(async (correct) => {
    ...
    if (type === "mcq") {
      ...
      const isFrac = String(ans).includes("/");
      const val = correct ? String(ans) : isFrac ? "9/9" : "999";
      for (const ch of val) {
        const key = ch === "/" ? "↓ 분모" : ch === "." ? "·" : ch === "-" ? "+/−" : ch;
    ...
    return {
      ...
      frac: type === "num" && String(ans).includes("/"),
      dec: type === "num" && String(ans).includes("."),
...
ok(!!nodeInfo, "수학 중2 지도(Ⅵ 확률)에 평가 노드 존재");
...
ok(introEyebrow.includes("확률"), "인트로 단원 표기(VI · 확률)", introEyebrow);
...
await shot("exam-m2u6-a-intro");
await page.evaluate(() => document.querySelector(".screen.active .btn.cta").click());
await W(650);

const seenA = await playExam(14, (i) => i >= 6); // 앞 6문항(무료 L1~L3 구간 최소 2×3) 오답 → 70점, 리뷰 점프가 무료 레슨으로
...
console.log("  (유형 분포:", JSON.stringify(seenA.reduce((m, s) => ((m[s.type] = (m[s.type] ?? 0) + 1), m), {})), "· frac num", seenA.filter((s) => s.frac).length, "개 · dec num", seenA.filter((s) => s.dec).length, "개 · 그림", seenA.filter((s) => s.hasFigure).length, "개)");
...
ok(tSorted === "222222233", "레슨 균형 추출(2×7+3×2, 잔여는 서로 다른 레슨)", tSorted);
...
ok(stA.exams?.m2u6exam?.attempts === 1 && stA.exams?.m2u6exam?.best === 70 && stA.exams?.m2u6exam?.conquered === false, "store 기록(1회·70점·미정복)", JSON.stringify(stA.exams));
ok(stA.totalXp === 70, "XP 지급 = 점수만큼(첫 신기록)", String(stA.totalXp));
await shot("exam-m2u6-a-result");
...
const lessons = {};
for (let i = 1; i <= 9; i++) lessons[`m2u6l${i}`] = { done: true, acc: 95, bestXp: 120 };
await seed({ ...BASE, premium: true, lessons, totalXp: 70, exams: { m2u6exam: { attempts: 1, best: 70, conquered: false } } });
await gotoExamIntro();
...
await page.evaluate(() => document.querySelector(".screen.active .btn.cta").click());
await W(650);
const seenC = await playExam(20); // 전부 정답 → 100점(frac 넘패드 정답 입력 경로 검증)
ok(seenC.filter((s) => s.frac).length >= 1, "frac 넘패드 입력 검증(이번 시험지에 frac num 출제)", `frac ${seenC.filter((s) => s.frac).length}개`);
...
ok(resultC.score === "100", "만점 채점(전 유형 정답 입력 — frac·dec num 포함)", resultC.score ?? "");
...
ok(stC.exams.m2u6exam.attempts === 2 && stC.exams.m2u6exam.best === 100 && stC.exams.m2u6exam.conquered === true, "store 기록(2회·100점·정복)", JSON.stringify(stC.exams));
ok(stC.totalXp === 100, "누적 XP = 70 + 30", String(stC.totalXp));
await shot("exam-m2u6-c-conquered");
...
ok(stD.totalXp === 100 && stD.exams.m2u6exam.best === 100 && stD.exams.m2u6exam.attempts === 3, "store: XP 불변·최고점 유지·응시 3회");
