// g2u4 v2 160문항 전수 검증 + 시험지 갤러리 렌더(u6 v2판 계승 · 재출제 8호).
// node qa/render-g2u4v2-full.mjs          → 존재하는 스테이징만 부분 검증(저작 중간 게이트)
// 전 파일(pilot + rest-a~f) 존재 시        → 160 전수 검증 + tmp/g2u4v2-full/index.html 렌더
// 사진: public/exam/g2u4(webp)를 tmp로 복사.
import { build } from "esbuild";
import { copyFileSync, existsSync, mkdirSync, readFileSync, readdirSync, writeFileSync } from "node:fs";

const SRC = [
  ["qa/g2u4v2-pilot.ts", "POOL_G2U4V2_PILOT"],
  ["qa/g2u4v2-rest-a.ts", "POOL_G2U4V2_REST_A"],
  ["qa/g2u4v2-rest-b.ts", "POOL_G2U4V2_REST_B"],
  ["qa/g2u4v2-rest-c.ts", "POOL_G2U4V2_REST_C"],
  ["qa/g2u4v2-rest-d.ts", "POOL_G2U4V2_REST_D"],
  ["qa/g2u4v2-rest-e.ts", "POOL_G2U4V2_REST_E"],
  ["qa/g2u4v2-rest-f.ts", "POOL_G2U4V2_REST_F"],
];
...
console.log(`로드: ${present.map(([p]) => p.replace("qa/g2u4v2-", "").replace(".ts", "")).join(" · ")} (${items.length}문항)${allPresent ? "" : " — 부분 검증 모드"}`);

const plain = (v) => String(v ?? "").replace(/<[^>]*>/g, "").replace(/&nbsp;/g, " ").trim();
let fails = 0;
const fail = (m) => { fails += 1; console.error("FAIL", m); };

// ── 정본 쿼터(설계표 qa/g2u4-v2-blueprint.md §4) ──
// 시각 파일별 값은 §7-3 재배분(16/15/15/15/18/16 — 총 95 · 59.4%)이 정본(무리한 장식 승격 대신
// 자료 의존 설계가 성립하는 자연 상한 — §7.2 원칙 우선).
const LESSON = {
  g2u4l1: { start: 201, end: 227, m: 24, M: 3, n: 0, d: [11, 11, 5], fig: 16 },
  g2u4l2: { start: 228, end: 254, m: 23, M: 2, n: 2, d: [11, 11, 5], fig: 15 },
  g2u4l3: { start: 255, end: 281, m: 21, M: 2, n: 4, d: [11, 11, 5], fig: 15 },
  g2u4l4: { start: 282, end: 307, m: 24, M: 2, n: 0, d: [10, 10, 6], fig: 15 },
  g2u4l5: { start: 308, end: 334, m: 18, M: 3, n: 6, d: [11, 11, 5], fig: 18 },
  g2u4l6: { start: 335, end: 360, m: 24, M: 2, n: 0, d: [10, 10, 6], fig: 16 },
};
// 언어 가드(설계표 §0). 도입어(원소·화합물·원자·분자·이온·족·주기·전하·수용액 등)는 제외.
// '원자가'(가전자 개념)는 조사 "원자가 ~"와 기계 구분 불가라 수동 검수 항목(여기 미포함).
...
const BAN_RE = [/주기율(?!표)/];
// ASCII 첨자 가드: 노출면의 H2O·CO2류(유니코드 ₂ 미사용)를 잡는다. A~E 위치 기호 뒤 숫자는 없다.
const ASCII_SUB = /[A-Z][a-z]?[0-9]/;
...
for (const it of items) {
  ...
  if (!/^g2u4e\d{3}$/.test(it.id) || slot < L.start || slot > L.end) fail(`${it.id} 슬롯 대역(${L.start}~${L.end}) 위반`);
  if (it.figure) {
  ...
  if (it.type === "multi") {
    ...
    // 순수 라벨 보기(전부 ㉮~/㉠~/(가)~ 꼴)는 shuffle:false 허용 — 그림 라벨 관례 순서가 깨져 보이면
    // 실격(CLAUDE.md 라벨형 고정 규칙 · 검산 B F12: 이때 첫 라벨 ㉮는 비정답으로 배치). 그 외 multi는
    // 셔플이 관행.
  ...
  if (it.type === "num") {
    ...
    if (!/^-?\d+$/.test(a)) fail(`${it.id} int answer "${a}"`);
    if (!it.unitLabel) fail(`${it.id} num unitLabel 없음`);
    // num은 전량 자료 동반(설계표 §0 — 문두 나열 수치만으로 푸는 num 금지)
    if (!it.figure) fail(`${it.id} num인데 자료(모형·표·조건 상자) 없음`);
  }
  ...
  for (const r of BAN_RE) if (r.test(all)) fail(`${it.id} 금지어 패턴 ${r}`);
  if (ASCII_SUB.test(noExp)) fail(`${it.id} 노출면 ASCII 첨자 의심(H2O류 — 유니코드 ₂ 사용)`);
  if (!it.core) fail(`${it.id} core 없음`);
  // 해설의 보기 위치 지칭 금지(셔플과 어긋남 — u1 v2 계보 6표현)
  if (/첫 번째 보기|두 번째 보기|세 번째 보기|네 번째 보기|다섯 번째 보기|마지막 보기/.test(exp)) fail(`${it.id} 해설이 보기 위치 지칭`);
  // 그림 aria에 num 정답 수치 노출 금지(숫자 경계)
  if (it.type === "num" && it.figure) {
    const aria = (String(it.figure).match(/aria-label="([^"]*)"/) ?? [])[1] ?? "";
    if (new RegExp(`(?<![\\d.])${it.answer}(?![\\d.])`).test(aria)) fail(`${it.id} 그림 aria에 정답 수치 노출`);
  }
  ...
  if (it.type === "mcq" && it.figure && /^<(img|div)/.test(String(it.figure))) {
    ...
    if (ansText.length <= 12 && ansText && alts.includes(ansText)) fail(`${it.id} 사진 alt에 정답 "${ansText}" 유출`);
  }
}

// ── 소스(주석 포함) em대시·한글 금지어 0 ──
for (const [p] of present) {
  const src = readFileSync(p, "utf8").replace(/\r\n/g, "\n");
  if (src.includes("—")) fail(`${p} 소스(주석 포함)에 em대시 존재`);
  for (const w of BAN.filter((b) => b !== "⭕")) if (src.includes(w)) fail(`${p} 소스에 금지어 "${w}"`);
  for (const r of BAN_RE) if (r.test(src)) fail(`${p} 소스에 금지어 패턴 ${r}`);
}
...
for (const [lid, arr] of byLesson) {
  ...
  if (complete) {
    ...
    const slots = arr.map((i) => Number(i.id.replace("g2u4e", ""))).sort((a, b) => a - b);
...
if (allPresent) {
  ...
  if (fig !== 95) fail(`전체 시각 ${fig} ≠ 95`);
  const nofig = items.filter((i) => !i.figure).length;
  if (nofig !== 65) fail(`무그림 ${nofig} ≠ 65`);
...
items.sort((a, b) => Number(a.id.replace("g2u4e", "")) - Number(b.id.replace("g2u4e", "")));
const CIRC = ["①", "②", "③", "④", "⑤"];
const GNL = ["ㄱ", "ㄴ", "ㄷ"];
const answerText = (it) => {
...
const LNAME = {
  g2u4l1: "L1 원소와 화합물", g2u4l2: "L2 원소 기호와 화학식", g2u4l3: "L3 원자의 구조",
  g2u4l4: "L4 주기율표", g2u4l5: "L5 원자·분자·이온", g2u4l6: "L6 이온의 이동",
};
...
<title>g2u4 v2 ${items.length}문항 · 검수용 시험지</title>
<style>
...
<div class="sheet">
<header>
  <h1>중2 과학 Ⅳ. 물질의 구성 · v2 재출제 ${allPresent ? "160문항" : `파일럿 ${items.length}문항`}</h1>
  <p>검수용 시험지 렌더 · 교과서 3사 준거 규격 v2(word 0 · num 12 전량 자료 동반 · 시각 96/160 ·
  diff 64/64/32 · 교차 유출 원조 단원의 산문형 유출 중점 저작) 적용.
...
mkdirSync("tmp/g2u4v2-full/exam/g2u4", { recursive: true });
writeFileSync("tmp/g2u4v2-full/index.html", html);
let nPhotos = 0;
if (existsSync("public/exam/g2u4")) {
  for (const f of readdirSync("public/exam/g2u4")) {
    if (/\.(webp|jpg|png)$/i.test(f)) {
      copyFileSync(`public/exam/g2u4/${f}`, `tmp/g2u4v2-full/exam/g2u4/${f}`);
      nPhotos += 1;
    }
  }
}
console.log(`렌더 완료: tmp/g2u4v2-full/index.html (${Math.round(html.length / 1024)}KB, 문항 ${items.length}) · exam/g2u4 사진 ${nPhotos}장 복사`);
