// m2u2 시험 그림 눈검수용 스크린샷 — 풀 파일 9개에서 figure가 있는 전 문항을 자동 수집해
// 실제 저작 파라미터 그대로 한 페이지에 렌더(문항 id·유형 라벨 포함). m2u1 자동화판 계승.
// PORT=<포트> node qa/shot-exam-figs-m2u2.mjs (dev 서버 필수 — vite 모듈 URL로 임포트)
...
const count = await page.evaluate(async () => {
  const mods = await Promise.all(
    ["l1", "l2", "l3", "l4", "l5", "l6", "l7", "l8", "l9"].map((s) => import(`/src/content/exams/m2u2${s}.ts`)),
  );
...
await page.screenshot({ path: "qa/shots/exam-m2u2-figs.png", fullPage: true });
...
for (let i = 0; i < 4; i++) {
  ...
  await page.screenshot({ path: `qa/shots/exam-m2u2-figs-${i + 1}.png` });
}
console.log(`SAVED qa/shots/exam-m2u2-figs.png (+분할 4장) — figure 문항 ${count}개`);
await browser.close();
