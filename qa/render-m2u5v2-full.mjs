// m2u5 v2 전체 200문항 검증 + 시험지 렌더(파일럿 40 + 확장 160 병합, 슬롯 순).
// node qa/render-m2u5v2-full.mjs  →  tmp/m2u5v2-full/index.html
import { build } from "esbuild";
import { mkdirSync, writeFileSync } from "node:fs";

const FILES = [
  ["qa/m2u5v2-pilot.ts", "POOL_M2U5V2_PILOT"],
  ["qa/m2u5v2-rest-a.ts", "POOL_M2U5V2_REST_A"],
  ["qa/m2u5v2-rest-b.ts", "POOL_M2U5V2_REST_B"],
  ["qa/m2u5v2-rest-c.ts", "POOL_M2U5V2_REST_C"],
  ["qa/m2u5v2-rest-d.ts", "POOL_M2U5V2_REST_D"],
  ["qa/m2u5v2-rest-e.ts", "POOL_M2U5V2_REST_E"],
];
...
for (let i = 1; i <= 200; i += 1) {
  const id = `m2u5e${String(i).padStart(3, "0")}`;
  if (!pool[i - 1] || pool[i - 1].id !== id) fail(`슬롯 ${i}: id ${pool[i - 1]?.id} ≠ ${id}`);
}

// 레슨별 쿼터(§0): 18슬롯 = m9/M2/n7, 19슬롯(L4·L10) = m10/M2/n7
const LQ = {
  m2u5l1: [9, 2, 7, [7, 8, 3]], m2u5l2: [9, 2, 7, [7, 7, 4]], m2u5l3: [9, 2, 7, [7, 7, 4]],
  m2u5l4: [10, 2, 7, [8, 8, 3]], m2u5l5: [9, 2, 7, [7, 7, 4]], m2u5l6: [9, 2, 7, [7, 7, 4]],
  m2u5l7: [9, 2, 7, [7, 7, 4]], m2u5l8: [9, 2, 7, [7, 7, 4]], m2u5l9: [9, 2, 7, [7, 7, 4]],
  m2u5l10: [10, 2, 7, [8, 8, 3]], m2u5l11: [9, 2, 7, [8, 7, 3]],
};
const byLesson = {};
...
for (const [lid, spec] of Object.entries(LQ)) {
  ...
  figuredTotal += g.filter((x) => x.figure).length;
  for (const it of g) {
    ...
    if (it.type === "num" && !/^-?\d+$/.test(String(it.answer))) fail(`${it.id} num answer`);
    if (it.shuffle === false && it.answer === 0) fail(`${it.id} shuffle:false 첫 보기 정답`);
    const exp = plain(it.explain);
    if (exp.length < 250) fail(`${it.id} 해설 ${exp.length}자`);
...
console.log(`그림: ${figuredTotal}/200 (${Math.round(figuredTotal / 2)}%)`);
const byType = { mcq: 0, multi: 0, num: 0 };
const byDiff = { 1: 0, 2: 0, 3: 0 };
for (const it of pool) { byType[it.type] += 1; byDiff[it.diff] += 1; }
console.log(`유형 합계: mcq ${byType.mcq} / multi ${byType.multi} / num ${byType.num} · diff ${byDiff[1]}/${byDiff[2]}/${byDiff[3]}`);
...
const cards = pool.map((it, i) => {
  const tag = `${it.lessonId.replace("m2u5", "")} · ${it.type} · 난이도 ${"●".repeat(it.diff)}${"○".repeat(3 - it.diff)}`;
...
<title>m2u5 v2 200문항 — 검수용 시험지</title>
<style>
...
<div class="sheet">
<header>
  <h1>중2 수학 Ⅴ. 도형의 닮음과 피타고라스 정리 — v2 전체 200문항</h1>
  <p>검수용 시험지 렌더(슬롯 번호순 · 파일럿 40 + 확장 160). 규격 v2 적용: 그림 89% · 값 구하기 중심 · word 0 ·
  그림 라벨 단위 병기 · 직각 마크 의무. 실제 앱에서는 응시마다 레슨 균형으로 20문항이 추출돼요.</p>
</header>
<div class="cols">
${cards}
</div>
...
mkdirSync("tmp/m2u5v2-full", { recursive: true });
writeFileSync("tmp/m2u5v2-full/index.html", html);
console.log(`렌더 완료: tmp/m2u5v2-full/index.html (${Math.round(html.length / 1024)}KB, 문항 ${pool.length})`);
