// 단원 종합 평가(m1u6 수학 중1 Ⅵ 통계 — 첫 수학 시험) E2E — 응시 → 일괄 채점 → 6파트 진단 → 리뷰 →
// 레슨 바로가기 → 재응시 페이월 → (정복+프리미엄) 재응시 → 정복 인증 + 신기록 XP까지 실플레이.
// PORT=<포트> node qa/e2e-exam-m1u6.mjs — dev 서버 필수(보기 선택이 dev 전용 data-oi/data-ans를 쓴다).
// g2u4판과의 차이: 수학 지도 시딩(viewSubject "math" · viewGrade "g1"), 발주 사진 없음(전부 SVG —
// 그림 문항은 q-figure svg 렌더로 검증), 무료 레슨이 L1~L2라 무료 응시 오답은 앞 6문항(L1·L2 구간 최소
// 3+3)에 배치해야 리뷰 점프가 페이월이 아닌 레슨 플레이어로 간다. num dec 입력은 "." → "·" 매핑(mathKit).
...
async function gotoUnitTab() {
  ...
  await page.evaluate(() => [...document.querySelectorAll(".unit-tab")].find((t) => t.textContent.includes("통계"))?.click());
  await W(650);
}
...
ok(!!nodeInfo, "수학 지도(Ⅵ 통계)에 평가 노드 존재");
...
ok(introEyebrow.includes("VI") && introEyebrow.includes("통계"), "인트로 단원 표기(VI · 통계)", introEyebrow);
...
await shot("exam-m1u6-a-intro");
await page.evaluate(() => document.querySelector(".screen.active .btn.cta").click());
await W(650);

const seenA = await playExam(14, (i) => i >= 6); // 앞 6문항(무료 L1·L2 구간) 오답 → 70점, 리뷰 점프가 무료 레슨으로
...
ok(stA.exams?.m1u6exam?.attempts === 1 && stA.exams?.m1u6exam?.best === 70 && stA.exams?.m1u6exam?.conquered === false, "store 기록(1회·70점·미정복)", JSON.stringify(stA.exams));
ok(stA.totalXp === 70, "XP 지급 = 점수만큼(첫 신기록)", String(stA.totalXp));
await shot("exam-m1u6-a-result");
...
const lessons = {};
for (let i = 1; i <= 6; i++) lessons[`m1u6l${i}`] = { done: true, acc: 95, bestXp: 120 };
await seed({ ...BASE, premium: true, lessons, totalXp: 70, exams: { m1u6exam: { attempts: 1, best: 70, conquered: false } } });
await gotoExamIntro();
...
ok(stC.exams.m1u6exam.attempts === 2 && stC.exams.m1u6exam.best === 100 && stC.exams.m1u6exam.conquered === true, "store 기록(2회·100점·정복)", JSON.stringify(stC.exams));
ok(stC.totalXp === 100, "누적 XP = 70 + 30", String(stC.totalXp));
await shot("exam-m1u6-c-conquered");
...
ok(stD.totalXp === 100 && stD.exams.m1u6exam.best === 100 && stD.exams.m1u6exam.attempts === 3, "store: XP 불변·최고점 유지·응시 3회");
