// 단원 종합 평가(g2u1 물질의 특성) E2E — 응시 → 일괄 채점 → 9파트 진단 → 리뷰 → 레슨 바로가기 →
// 재응시 페이월 → (정복+프리미엄) 재응시 → 정복 인증 + 신기록 XP + 발주 사진 10장 로드까지 실플레이.
// PORT=<포트> node qa/e2e-exam-g2u1.mjs — dev 서버 필수(보기 선택이 dev 전용 data-oi/data-ans를 쓴다).
// u6판과의 차이: 중2 트랙(viewGrade g2), 레슨 9개(추출 2×7+3×2), 인트로 "아홉 파트", exam/g2u1 사진 10장.
...
async function gotoExamIntro() {
  ...
  await page.evaluate(() => [...document.querySelectorAll(".unit-tab")].find((t) => t.textContent.includes("물질의 특성"))?.click());
  await W(650);
...
// ═══════════ 0. 발주 사진 10장 로드(exam/g2u1) ═══════════
console.log("0. 발주 사진 로드");
await page.goto(`http://localhost:${PORT}/`, { waitUntil: "domcontentloaded" });
const PHOTOS = ["metal-scale", "iceberg", "hotair-balloon", "aquarium-stone", "pot-stone", "egg-sink", "egg-float", "gold-pan", "salt-pond", "soju-still"];
const photoRes = await page.evaluate(async (names) => {
  ...
  for (const n of names) {
    const r = await new Promise((res) => {
      ...
      im.onerror = () => res(0);
      im.src = `/exam/g2u1/${n}.webp`;
    });
    out.push({ n, w: r });
  }
  return out;
}, PHOTOS);
ok(photoRes.every((p) => p.w > 0), "exam/g2u1 사진 10장 전부 로드", JSON.stringify(photoRes.filter((p) => !p.w)));
...
ok(stA.exams?.g2u1exam?.attempts === 1 && stA.exams?.g2u1exam?.best === 70 && stA.exams?.g2u1exam?.conquered === false, "store 기록(1회·70점·미정복)", JSON.stringify(stA.exams));
ok(stA.totalXp === 70, "XP 지급 = 점수만큼(첫 신기록)", String(stA.totalXp));
await shot("exam-g2u1-a-result");
...
const lessons = {};
for (let i = 1; i <= 9; i++) lessons[`g2u1l${i}`] = { done: true, acc: 95, bestXp: 120 };
await seed({ ...BASE, premium: true, lessons, totalXp: 70, exams: { g2u1exam: { attempts: 1, best: 70, conquered: false } } });
await gotoExamIntro();
...
ok(stC.exams.g2u1exam.attempts === 2 && stC.exams.g2u1exam.best === 100 && stC.exams.g2u1exam.conquered === true, "store 기록(2회·100점·정복)", JSON.stringify(stC.exams));
ok(stC.totalXp === 100, "누적 XP = 70 + 30", String(stC.totalXp));
await shot("exam-g2u1-c-conquered");
...
ok(stD.totalXp === 100 && stD.exams.g2u1exam.best === 100 && stD.exams.g2u1exam.attempts === 3, "store: XP 불변·최고점 유지·응시 3회");
