// u6 v2 160문항 전수 검증 + 시험지 갤러리 렌더(u3 v2판 계승 · 재출제 5호).
// node qa/render-u6v2-full.mjs            → 존재하는 스테이징만 부분 검증(저작 중간 게이트)
// 전 파일(pilot + rest-a~e) 존재 시        → 160 전수 검증 + tmp/u6v2-full/index.html 렌더
// 사진: public/exam/u6(webp)를 tmp로 복사.
import { build } from "esbuild";
import { copyFileSync, existsSync, mkdirSync, readFileSync, readdirSync, writeFileSync } from "node:fs";

const SRC = [
  ["qa/u6v2-pilot.ts", "POOL_U6V2_PILOT"],
  ["qa/u6v2-rest-a.ts", "POOL_U6V2_REST_A"],
  ["qa/u6v2-rest-b.ts", "POOL_U6V2_REST_B"],
  ["qa/u6v2-rest-c.ts", "POOL_U6V2_REST_C"],
  ["qa/u6v2-rest-d.ts", "POOL_U6V2_REST_D"],
  ["qa/u6v2-rest-e.ts", "POOL_U6V2_REST_E"],
];
...
console.log(`로드: ${present.map(([p]) => p.replace("qa/u6v2-", "").replace(".ts", "")).join(" · ")} (${items.length}문항)${allPresent ? "" : " — 부분 검증 모드"}`);

const plain = (v) => String(v ?? "").replace(/<[^>]*>/g, "").replace(/&nbsp;/g, " ").trim();
let fails = 0;
const fail = (m) => { fails += 1; console.error("FAIL", m); };

// ── 정본 쿼터(설계표 qa/u6-v2-blueprint.md §4) ──
const LESSON = {
...
// 언어 가드(설계표 §0). 도입어(압력·대기압·기압·보일 법칙·샤를 법칙·반비례·감압 용기·물중탕)는 제외.
// '분자' 금지(중1 = 입자) · 압력 단위는 "기압"만 · '열팽창'은 이 단원 금지(샤를과 용어 혼선 방지).
const BAN = ["분자", "절대 온도", "절대온도", "켈빈", "화씨", "파스칼", "이상 기체", "기체 상수", "아보가드로", "게이뤼삭", "절대영도", "하임리히", "열팽창", "등온", "등압", "⭕"];
// 노출면 전용 기호 가드: 기호식·로마자 축약 금지(소스 주석의 슬롯 코드명은 허용).
const SYM = [/PV/, /P-V/, /T-V/, /hPa/, /\bPa\b/, /N\/m/];
...
for (const it of items) {
  ...
  if (!/^u6e\d{3}$/.test(it.id) || slot < L.start || slot > L.end) fail(`${it.id} 슬롯 대역(${L.start}~${L.end}) 위반`);
  if (it.figure) {
...
for (const [lid, arr] of byLesson) {
  ...
  if (complete) {
    ...
    const slots = arr.map((i) => Number(i.id.replace("u6e", ""))).sort((a, b) => a - b);
...
if (allPresent) {
  ...
  if (bogi + multi < 33) fail(`합답 총량(bogi+multi) ${bogi + multi} < 33`);
...
items.sort((a, b) => Number(a.id.replace("u6e", "")) - Number(b.id.replace("u6e", "")));
const CIRC = ["①", "②", "③", "④", "⑤"];
const GNL = ["ㄱ", "ㄴ", "ㄷ"];
const answerText = (it) => {
...
const LNAME = {
  u6l1: "L1 기체의 압력", u6l2: "L2 보일 법칙", u6l3: "L3 생활 속 보일",
  u6l4: "L4 샤를 법칙", u6l5: "L5 생활 속 샤를",
};
...
<title>u6 v2 ${items.length}문항 · 검수용 시험지</title>
<style>
...
<div class="sheet">
<header>
  <h1>중1 과학 Ⅵ. 기체의 성질 · v2 재출제 ${allPresent ? "160문항" : `파일럿 ${items.length}문항`}</h1>
  <p>검수용 시험지 렌더 · 교과서 3사 준거 규격 v2(word 0 · num 16 전량 자료 동반 · 시각 100/160 ·
  diff 64/64/32 · 신작 헬퍼 데뷔 · 사진 10장 = 재사용 8 + 신규 2) 적용.
...
mkdirSync("tmp/u6v2-full/exam/u6", { recursive: true });
writeFileSync("tmp/u6v2-full/index.html", html);
let nPhotos = 0;
if (existsSync("public/exam/u6")) {
  for (const f of readdirSync("public/exam/u6")) {
    if (/\.(webp|jpg|png)$/i.test(f)) {
      copyFileSync(`public/exam/u6/${f}`, `tmp/u6v2-full/exam/u6/${f}`);
      nPhotos += 1;
    }
  }
}
console.log(`렌더 완료: tmp/u6v2-full/index.html (${Math.round(html.length / 1024)}KB, 문항 ${items.length}) · exam/u6 사진 ${nPhotos}장 복사`);
