// g2u1 v2 160문항 전수 검증 + 시험지 갤러리 렌더(u1 v2판 계승 · 부분 검증 모드 내장).
// node qa/render-g2u1v2-full.mjs   → 존재하는 스테이징만 부분 검증(저작 중간 게이트 · 파일럿 검수 겸용)
// 전 파일(pilot + rest-a~e) 존재 시 → 160 전수 검증 + tmp/g2u1v2-full/index.html 렌더
// 사진: public/exam/g2u1 을 tmp로 복사. 정본 쿼터 = qa/g2u1-v2-blueprint.md §4.
import { build } from "esbuild";
import { copyFileSync, existsSync, mkdirSync, readFileSync, readdirSync, writeFileSync } from "node:fs";

const SRC = [
  ["qa/g2u1v2-pilot.ts", "POOL_G2U1V2_PILOT"],
  ["qa/g2u1v2-rest-a.ts", "POOL_G2U1V2_REST_A"],
  ["qa/g2u1v2-rest-b.ts", "POOL_G2U1V2_REST_B"],
  ["qa/g2u1v2-rest-c.ts", "POOL_G2U1V2_REST_C"],
  ["qa/g2u1v2-rest-d.ts", "POOL_G2U1V2_REST_D"],
  ["qa/g2u1v2-rest-e.ts", "POOL_G2U1V2_REST_E"],
];
...
console.log(`로드: ${present.map(([p]) => p.replace("qa/g2u1v2-", "").replace(".ts", "")).join(" · ")} (${items.length}문항)${allPresent ? "" : " — 부분 검증 모드"}`);

const plain = (v) => String(v ?? "").replace(/<[^>]*>/g, "").replace(/&nbsp;/g, " ").trim();
let fails = 0;
const fail = (m) => { fails += 1; console.error("FAIL", m); };

// ── 정본 쿼터(설계표 qa/g2u1-v2-blueprint.md §4) ──
const LESSON = {
  g2u1l1: { start: 201, end: 218, m: 13, M: 2, n: 3, d: [7, 7, 4], fig: 12 },
  g2u1l2: { start: 219, end: 236, m: 13, M: 2, n: 3, d: [7, 7, 4], fig: 13 },
  g2u1l3: { start: 237, end: 254, m: 13, M: 2, n: 3, d: [7, 7, 4], fig: 13 },
  g2u1l4: { start: 255, end: 271, m: 15, M: 2, n: 0, d: [8, 8, 1], fig: 9 },
  g2u1l5: { start: 272, end: 289, m: 13, M: 2, n: 3, d: [7, 7, 4], fig: 14 },
  g2u1l6: { start: 290, end: 307, m: 15, M: 2, n: 1, d: [7, 7, 4], fig: 12 },
  g2u1l7: { start: 308, end: 325, m: 14, M: 2, n: 2, d: [7, 7, 4], fig: 13 },
  g2u1l8: { start: 326, end: 342, m: 12, M: 2, n: 3, d: [7, 7, 3], fig: 12 },
  g2u1l9: { start: 343, end: 360, m: 14, M: 2, n: 2, d: [7, 7, 4], fig: 14 },
};
// 언어 가드(설계표 §0 금지어) — 도입어(용해도·석출·증류 등)는 당연히 제외.
const BAN = [
  "분자", "원자", "이온", "부력", "비중", "헨리", "크로마토그래피", "추출", "분별 증류",
  "용해 평형", "증기 압력", "균일 혼합물", "불균일 혼합물", "끓는점 오름", "어는점 내림",
  "퍼센트 농도", "질량 백분율", "⭕",
];
...
for (const it of items) {
  ...
  ids.add(it.id);
  const slot = Number(it.id.replace("g2u1e", ""));
  const L = LESSON[it.lessonId];
  if (!L) { fail(`${it.id} lessonId ${it.lessonId}`); continue; }
  if (!/^g2u1e\d{3}$/.test(it.id) || slot < L.start || slot > L.end) fail(`${it.id} 슬롯 대역(${L.start}~${L.end}) 위반`);
  if (it.figure) {
    ...
    for (const src of [...f.matchAll(/src="([^"]*)"/g)].map((m) => m[1])) {
      if (src.includes("exam/") && !existsSync(`public/exam/${src.split("exam/")[1]}`)) fail(`${it.id} 사진 파일 없음: ${src}`);
    }
  }
  if (it.type === "word") fail(`${it.id} word 금지(v2 word 0)`);
  if (it.type === "num" && !it.figure) fail(`${it.id} num 자료 동반 위반(계산 조건도 자료 상자로)`);
  ...
  if (it.type === "num") {
    ...
    if (!it.numKind) fail(`${it.id} num numKind 없음`);
  }
  ...
  for (const w of POS_REF) if (exp.includes(w)) fail(`${it.id} 해설 보기 위치 지칭 "${w}"`);
  ...
  // 사진 alt에 정답 유출(동정 문항 휴리스틱)
  if (it.type === "mcq" && it.figure && String(it.figure).includes("<img")) {
    const alt = (String(it.figure).match(/alt="([^"]*)"/) ?? [])[1] ?? "";
    const ansText = plain(it.options?.[it.answer] ?? "");
    if (ansText.length <= 12 && ansText.length >= 2 && alt.includes(ansText)) fail(`${it.id} 사진 alt에 정답 "${ansText}" 유출`);
  }
...
for (const [lid, arr] of byLesson) {
  ...
  if (complete) {
    if (m !== L.m || M !== L.M || n !== L.n) fail(`${lid} 유형 m${m}/M${M}/n${n} ≠ m${L.m}/M${L.M}/n${L.n}`);
    if (d[0] !== L.d[0] || d[1] !== L.d[1] || d[2] !== L.d[2]) fail(`${lid} diff ${d.join("/")} ≠ ${L.d.join("/")}`);
    if (fig !== L.fig) fail(`${lid} 시각 ${fig} ≠ ${L.fig}`);
    const slots = arr.map((i) => Number(i.id.replace("g2u1e", ""))).sort((a, b) => a - b);
...
// ── 시험지 렌더 ──
items.sort((a, b) => Number(a.id.replace("g2u1e", "")) - Number(b.id.replace("g2u1e", "")));
const CIRC = ["①", "②", "③", "④", "⑤"];
const GNL = ["ㄱ", "ㄴ", "ㄷ"];
const answerText = (it) =>
  it.type === "mcq" ? CIRC[it.answer]
  : it.type === "multi" ? it.answer.map((i) => CIRC[i]).join(", ")
  : `${it.answer}${it.unitLabel ? " " + it.unitLabel : ""}`;
const LNAME = {
  g2u1l1: "L1 물질의 특성과 밀도", g2u1l2: "L2 뜨고 가라앉기", g2u1l3: "L3 고체의 용해도",
  g2u1l4: "L4 기체의 용해도", g2u1l5: "L5 녹는점과 끓는점", g2u1l6: "L6 순물질과 혼합물",
  g2u1l7: "L7 밀도 차로 분리하기", g2u1l8: "L8 재결정", g2u1l9: "L9 증류",
};
const cards = items.map((it, i) => {
  const slot = it.id.replace("g2u1e", "");
  ...
  if (it.type === "num") body += `<div class="q-numpad">수치 입력 · ${it.numKind}${it.unitLabel ? ` · 단위 ${it.unitLabel}` : ""}</div>`;
...
<title>g2u1 v2 ${items.length}문항 · 검수용 시험지</title>
<style>
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .q-numpad { margin-top: 9px; font-size: 11.5px; color: #8B95A1; border: 1px dashed #C4CAD2;
...
<div class="sheet">
<header>
  <h1>중2 과학 Ⅰ. 물질의 특성 · v2 ${allPresent ? "160문항" : `파일럿 ${items.length}문항`}</h1>
  <p>검수용 시험지 렌더 · 교과서 3사 준거 재출제 11호(word 0 · num 20 전량 계산·판독 · 시각 112/160 ·
  diff 64/64/32 · 신작 헬퍼 6종 · 사진 10장 재사용). 실제 앱에서는 20문항씩 추출되고 해설은 제출
...
mkdirSync("tmp/g2u1v2-full/exam/g2u1", { recursive: true });
writeFileSync("tmp/g2u1v2-full/index.html", html);
let n = 0;
for (const f of readdirSync("public/exam/g2u1")) {
  if (f.endsWith(".webp")) { copyFileSync(`public/exam/g2u1/${f}`, `tmp/g2u1v2-full/exam/g2u1/${f}`); n += 1; }
}
console.log(`렌더 완료: tmp/g2u1v2-full/index.html (${Math.round(html.length / 1024)}KB, 문항 ${items.length}) · 사진 ${n}장 복사`);
