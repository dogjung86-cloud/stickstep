// 단원 종합 평가(m2u2 수학 중2 Ⅱ 부등식과 연립방정식 — 9레슨 200제) E2E —
// 응시 → 일괄 채점 → 9파트 진단 → 리뷰 → 레슨 바로가기 → 재응시 페이월 → (정복+프리미엄)
// 재응시 → 정복 인증 + 신기록 XP까지 실플레이.
// PORT=<포트> node qa/e2e-exam-m2u2.mjs — dev 서버 필수(보기 선택이 dev 전용 data-oi/data-ans를 쓴다).
// m2u1판과의 차이: 파트 9개("아홉 파트"), 레슨 균형 2×7+3×2(잔여 2가 서로 다른 레슨에 1개씩),
// 무료 레슨이 L1~L3라 무료 응시 오답은 앞 6문항(L1~L3 구간 최소 2×3)에 배치해야
// 리뷰 점프가 페이월이 아닌 레슨 플레이어로 간다. num 음수는 "+/−" 키, 소수점은 "·" 키(mathKit).
...
async function gotoUnitTab() {
  ...
  await page.evaluate(() => [...document.querySelectorAll(".unit-tab")].find((t) => t.textContent.includes("부등식"))?.click());
  await W(650);
}
...
ok(!!nodeInfo, "수학 중2 지도(Ⅱ 부등식과 연립방정식)에 평가 노드 존재");
...
ok(introEyebrow.includes("부등식"), "인트로 단원 표기(II · 부등식과 연립방정식)", introEyebrow);
...
console.log("  (유형 분포:", JSON.stringify(seenA.reduce((m, s) => ((m[s.type] = (m[s.type] ?? 0) + 1), m), {})), "· 음수 num", seenA.filter((s) => s.neg).length, "개 · 그림", seenA.filter((s) => s.hasFigure).length, "개)");
...
ok(stA.exams?.m2u2exam?.attempts === 1 && stA.exams?.m2u2exam?.best === 70 && stA.exams?.m2u2exam?.conquered === false, "store 기록(1회·70점·미정복)", JSON.stringify(stA.exams));
ok(stA.totalXp === 70, "XP 지급 = 점수만큼(첫 신기록)", String(stA.totalXp));
await shot("exam-m2u2-a-result");
...
await seed({ ...BASE, premium: true, lessons, totalXp: 70, exams: { m2u2exam: { attempts: 1, best: 70, conquered: false } } });
await gotoExamIntro();
...
ok(resultC.score === "100", "만점 채점(전 유형 정답 입력 — num 음수·word 포함)", resultC.score ?? "");
...
ok(stC.exams.m2u2exam.attempts === 2 && stC.exams.m2u2exam.best === 100 && stC.exams.m2u2exam.conquered === true, "store 기록(2회·100점·정복)", JSON.stringify(stC.exams));
ok(stC.totalXp === 100, "누적 XP = 70 + 30", String(stC.totalXp));
await shot("exam-m2u2-c-conquered");
...
ok(stD.totalXp === 100 && stD.exams.m2u2exam.best === 100 && stD.exams.m2u2exam.attempts === 3, "store: XP 불변·최고점 유지·응시 3회");
