// 단원 종합 평가(u7 태양계) E2E — 응시 → 일괄 채점 → 6파트 진단 → 리뷰 → 레슨 바로가기 →
// 재응시 페이월 → (정복+프리미엄) 재응시 → 정복 인증 + 신기록 XP + 발주 사진 로드까지 실플레이.
// PORT=<포트> node qa/e2e-exam-u7.mjs — dev 서버 필수(보기 선택이 dev 전용 data-oi/data-ans를 쓴다).
// u6판과의 차이: 레슨 6개(추출은 3문항×4 + 4문항×2 — 잔여 2), 인트로 "여섯 파트" 문구, exam/u7 사진 8장 로드 검증.
...
async function gotoU7ExamIntro() {
  await page.waitForSelector(".unit-tab", { timeout: 12000 });
  await page.evaluate(() => [...document.querySelectorAll(".unit-tab")].find((t) => t.textContent.includes("태양계"))?.click());
  await W(650);
...
ok(stA.exams?.u7exam?.attempts === 1 && stA.exams?.u7exam?.best === 70 && stA.exams?.u7exam?.conquered === false, "store 기록(1회·70점·미정복)", JSON.stringify(stA.exams));
ok(stA.totalXp === 70, "XP 지급 = 점수만큼(첫 신기록)", String(stA.totalXp));
await shot("exam-u7-a-result");
...
const lessons = {};
for (let i = 1; i <= 6; i++) lessons[`u7l${i}`] = { done: true, acc: 95, bestXp: 120 };
await seed({ ...BASE, premium: true, lessons, totalXp: 70, exams: { u7exam: { attempts: 1, best: 70, conquered: false } } });
await gotoU7ExamIntro();
...
ok(stC.exams.u7exam.attempts === 2 && stC.exams.u7exam.best === 100 && stC.exams.u7exam.conquered === true, "store 기록(2회·100점·정복)", JSON.stringify(stC.exams));
ok(stC.totalXp === 100, "누적 XP = 70 + 30", String(stC.totalXp));
await shot("exam-u7-c-conquered");
...
ok(stD.totalXp === 100 && stD.exams.u7exam.best === 100 && stD.exams.u7exam.attempts === 3, "store: XP 불변·최고점 유지·응시 3회");
