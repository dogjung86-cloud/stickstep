// s1u2 v1 조립·갤러리 — 스테이징(qa/s1u2-*.ts)을 esbuild 실로드해 전 문항 카드를 렌더한다.
// 부분 검증 모드 내장(스테이징 일부만 있어도 렌더 · 저작 중간 게이트 겸용).
// 출력: tmp/s1u2-full/index.html (+ soc/climate.webp·soc/asia 사진 복사 · 경로 상대 치환).
// 서빙: 루트 launch.json "s1u2-full"(포트 6022 — 6021은 s1u1 사용 중 · 6000은 ERR_UNSAFE_PORT).
// 실행: node qa/render-s1u2-full.mjs
...
const LNAME = {
  s1u2l1: "L1 지역 구분", s1u2l2: "L2 지형·기후", s1u2l3: "L3 종교", s1u2l4: "L4 공존",
  s1u2l5: "L5 인구 분포", s1u2l6: "L6 인구 구조", s1u2l7: "L7 산업", s1u2l8: "L8 산업 변화",
};
const esc = (s) => String(s ?? "").replace(/&/g, "&amp;").replace(/</g, "&lt;");

const relPath = (s) => s.replaceAll('href="/soc/', 'href="./soc/').replaceAll('src="/soc/', 'src="./soc/');
    ...
    const opts =
      it.type === "word"
        ...
        : `<ol class="opts">${(it.options ?? [])
            ...
            .join("")}</ol>`;
    const fig = it.figure ? `<div class="fig">${relPath(it.figure)}</div>` : "";
...
const html = `<!doctype html><meta charset="utf-8"><title>s1u2 v1 시험지 갤러리(${items.length})</title>
<style>
  ...
  .fig{margin:10px 0}.fig svg{width:100%;height:auto;display:block}.fig img{display:block;width:100%;border-radius:14px}
...
<h1>s1u2 v1 단원 종합 평가 갤러리 · ${items.length}문항 ${allPresent ? "(전수)" : "(부분)"}</h1>
<div class="wrap">${cards}</div>`;

mkdirSync("tmp/s1u2-full/soc/asia", { recursive: true });
writeFileSync("tmp/s1u2-full/index.html", html);
if (existsSync("public/soc/climate.webp")) copyFileSync("public/soc/climate.webp", "tmp/s1u2-full/soc/climate.webp");
if (existsSync("public/soc/asia")) {
  for (const f of readdirSync("public/soc/asia")) {
    copyFileSync(`public/soc/asia/${f}`, `tmp/s1u2-full/soc/asia/${f}`);
  }
}
console.log("갤러리: tmp/s1u2-full/index.html");
