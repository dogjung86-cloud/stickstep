// 단원 종합 평가(u5 힘의 작용) E2E — 응시 → 일괄 채점 → 6파트 진단 → 리뷰 → 레슨 바로가기 →
// 재응시 페이월 → (정복+프리미엄) 재응시 → 정복 인증 + 신기록 XP + 발주 사진 로드까지 실플레이.
// PORT=<포트> node qa/e2e-exam-u5.mjs — dev 서버 필수(보기 선택이 dev 전용 data-oi/data-ans를 쓴다).
// u3판과의 차이: 레슨 7개(추출은 2×7+잔여 6 = t가 2 또는 3), 인트로 "일곱 파트" 동적 문구, exam/u5 사진 8장 로드 검증.
...
async function gotoU5ExamIntro() {
  await page.waitForSelector(".unit-tab", { timeout: 12000 });
  await page.evaluate(() => [...document.querySelectorAll(".unit-tab")].find((t) => t.textContent.includes("힘의 작용"))?.click());
  await W(650);
...
const photoRes = await page.evaluate(async (names) => {
  ...
  for (const n of names) {
    const r = await new Promise((res) => {
      ...
      im.onerror = () => res(0);
      im.src = `/exam/u5/${n}.webp`;
    });
    out.push({ n, w: r });
  }
  return out;
}, PHOTOS);
ok(photoRes.every((p) => p.w > 0), "exam/u5 사진 8장 전부 로드", JSON.stringify(photoRes.filter((p) => !p.w)));
...
ok(stA.exams?.u5exam?.attempts === 1 && stA.exams?.u5exam?.best === 70 && stA.exams?.u5exam?.conquered === false, "store 기록(1회·70점·미정복)", JSON.stringify(stA.exams));
ok(stA.totalXp === 70, "XP 지급 = 점수만큼(첫 신기록)", String(stA.totalXp));
await shot("exam-u5-a-result");
...
const lessons = {};
for (let i = 1; i <= 7; i++) lessons[`u5l${i}`] = { done: true, acc: 95, bestXp: 120 };
await seed({ ...BASE, premium: true, lessons, totalXp: 70, exams: { u5exam: { attempts: 1, best: 70, conquered: false } } });
await gotoU5ExamIntro();
...
ok(stC.exams.u5exam.attempts === 2 && stC.exams.u5exam.best === 100 && stC.exams.u5exam.conquered === true, "store 기록(2회·100점·정복)", JSON.stringify(stC.exams));
ok(stC.totalXp === 100, "누적 XP = 70 + 30", String(stC.totalXp));
await shot("exam-u5-c-conquered");
...
ok(stD.totalXp === 100 && stD.exams.u5exam.best === 100 && stD.exams.u5exam.attempts === 3, "store: XP 불변·최고점 유지·응시 3회");
