// 단원 종합 평가(g2u3 빛과 파동) E2E — 응시 → 일괄 채점 → 8파트 진단 → 리뷰 → 레슨 바로가기 →
// 재응시 페이월 → (정복+프리미엄) 재응시 → 정복 인증 + 신기록 XP + 발주 사진 10장 로드까지 실플레이.
// PORT=<포트> node qa/e2e-exam-g2u3.mjs — dev 서버 필수(보기 선택이 dev 전용 data-oi/data-ans를 쓴다).
// g2u2판과의 차이: 레슨 8개(추출 2×4+3×4), 인트로 "여덟 파트", exam/g2u3 사진 10장.
// 무료 구간은 L1~L3(최소 6문항이 시험지 앞쪽) — 무료 응시 세그먼트의 오답은 앞 6문항에 배치.
...
async function gotoExamIntro() {
  ...
  await page.evaluate(() => [...document.querySelectorAll(".unit-tab")].find((t) => t.textContent.includes("빛과 파동"))?.click());
  await W(650);
 ...
 *  (g2u3은 L4~L8이 프리미엄이라, 무료 응시 세그먼트에선 오답을 앞쪽(무료 레슨 구간)에 배치해야
...
const photoRes = await page.evaluate(async (names) => {
  ...
  for (const n of names) {
    const r = await new Promise((res) => {
      ...
      im.onerror = () => res(0);
      im.src = `/exam/g2u3/${n}.webp`;
    });
    out.push({ n, w: r });
  }
  return out;
}, PHOTOS);
ok(photoRes.every((p) => p.w > 0), "exam/g2u3 사진 10장 전부 로드", JSON.stringify(photoRes.filter((p) => !p.w)));
...
ok(stA.exams?.g2u3exam?.attempts === 1 && stA.exams?.g2u3exam?.best === 70 && stA.exams?.g2u3exam?.conquered === false, "store 기록(1회·70점·미정복)", JSON.stringify(stA.exams));
ok(stA.totalXp === 70, "XP 지급 = 점수만큼(첫 신기록)", String(stA.totalXp));
await shot("exam-g2u3-a-result");
...
await seed({ ...BASE, premium: true, lessons, totalXp: 70, exams: { g2u3exam: { attempts: 1, best: 70, conquered: false } } });
await gotoExamIntro();
...
ok(stC.exams.g2u3exam.attempts === 2 && stC.exams.g2u3exam.best === 100 && stC.exams.g2u3exam.conquered === true, "store 기록(2회·100점·정복)", JSON.stringify(stC.exams));
ok(stC.totalXp === 100, "누적 XP = 70 + 30", String(stC.totalXp));
await shot("exam-g2u3-c-conquered");
...
ok(stD.totalXp === 100 && stD.exams.g2u3exam.best === 100 && stD.exams.g2u3exam.attempts === 3, "store: XP 불변·최고점 유지·응시 3회");
