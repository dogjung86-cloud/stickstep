// 단원 종합 평가(s1u2 아시아 — 사회 시험 2호) E2E — 응시 → 일괄 채점 → 8파트 진단 →
// 리뷰 → 레슨 바로가기 → 재응시 페이월 → (정복+프리미엄) 재응시 → 정복 인증 + 신기록 XP까지 실플레이.
// PORT=<포트> node qa/e2e-exam-s1u2.mjs — dev 서버 필수(보기 선택이 dev 전용 data-oi/data-ans/data-w를 쓴다).
// 정본 골격 = qa/e2e-exam-s1u1.mjs 48검증판. s1u2 차이: 레슨 8개(추출 2×4+3×4 · 잔여 4),
// 인트로 "여덟 파트", 자산 = soc/climate.webp + soc/asia 사진(mosque.webp 대표 확인),
// 무료 레슨이 L1~L3뿐이라 무료 응시 오답은 앞 6문항에 배치(시험지는 진도 순 — L1~L3 구간).
...
async function gotoExamIntro() {
  ...
  await page.evaluate(() => [...document.querySelectorAll(".unit-tab")].find((t) => t.textContent.includes("아시아"))?.click());
  await W(650);
...
// ═══════════ 0. 지도·사진 실데이터 자산 로드 ═══════════
console.log("0. 기후 오버레이·사진 자산 로드");
...
const assets = await page.evaluate(async () => {
  ...
  return { climate: await load("/soc/climate.webp"), mosque: await load("/soc/asia/mosque.webp"), samarkand: await load("/soc/asia/samarkand.webp") };
});
ok(assets.climate > 0, "soc/climate.webp 로드(기후 지도 오버레이)", String(assets.climate));
ok(assets.mosque > 0 && assets.samarkand > 0, "soc/asia 사진 자산 로드(mosque·samarkand)", JSON.stringify(assets));
...
ok(!!nodeInfo, "사회 Ⅱ 지도에 평가 노드 존재(등록만으로 자동)");
...
await shot("exam-s1u2-a-intro");
await page.evaluate(() => document.querySelector(".screen.active .btn.cta").click());
await W(650);

// 오답 6을 앞 6문항(L1~L3 무료 구간)에 배치 — 리뷰 레슨 점프가 페이월이 아닌 레슨으로 가는지 검증용
...
ok(JSON.stringify(tCounts) === JSON.stringify([2, 2, 2, 2, 3, 3, 3, 3]), "레슨 균형 추출(8레슨 · 2×4+3×4 · 잔여 4는 서로 다른 레슨)", JSON.stringify(tCounts));
...
ok(stA.exams?.s1u2exam?.attempts === 1 && stA.exams?.s1u2exam?.best === 70 && stA.exams?.s1u2exam?.conquered === false, "store 기록(1회·70점·미정복)", JSON.stringify(stA.exams));
ok(stA.totalXp === 70, "XP 지급 = 점수만큼(첫 신기록)", String(stA.totalXp));
await shot("exam-s1u2-a-result");
...
const lessons = {};
for (let i = 1; i <= 8; i++) lessons[`s1u2l${i}`] = { done: true, acc: 95, bestXp: 120 };
await seed({ ...BASE, premium: true, lessons, totalXp: 70, exams: { s1u2exam: { attempts: 1, best: 70, conquered: false } } });
await gotoExamIntro();
...
ok(stC.exams.s1u2exam.attempts === 2 && stC.exams.s1u2exam.best === 100 && stC.exams.s1u2exam.conquered === true, "store 기록(2회·100점·정복)", JSON.stringify(stC.exams));
ok(stC.totalXp === 100, "누적 XP = 70 + 30", String(stC.totalXp));
await shot("exam-s1u2-c-conquered");

// 신기록 미갱신 재응시 — XP 0 확인(파밍 방지)
await page.evaluate(() => document.querySelector(".screen.active .ex-retake").click());
await W(700);
await playExam((i) => i < 10);
...
ok(stD.totalXp === 100 && stD.exams.s1u2exam.best === 100 && stD.exams.s1u2exam.attempts === 3, "store: XP 불변·최고점 유지·응시 3회");
