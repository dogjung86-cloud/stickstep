// 단원 종합 평가(g2u6 동물과 에너지) E2E — 응시 → 일괄 채점 → 여섯 파트 진단 → 리뷰 → 레슨 바로가기 →
// 재응시 페이월 → (정복+프리미엄) 재응시 → 정복 인증 + 신기록 XP + 라스터 로드까지 실플레이.
// PORT=<전용 포트> node qa/e2e-exam-g2u6.mjs — dev 서버 필수(보기 선택이 dev 전용 data-oi/data-ans를 쓴다).
// g2u1판과의 차이: 레슨 6개(추출 3×4+4×2), 인트로 "여섯 파트", num·word 0이라 넘패드·칩 분기 없음,
// 무료 L1~L3 + 프리미엄 L4~L6이라 무료 응시 오답은 시험지 **앞 9문항 안**(L1~L3 구간)에 둔다.
// 그림은 발주 라스터 하이브리드(exam/g2u6 3장 + body/figs 재사용 12장)라 사진 폴더 목록이 아니라
// 실제 참조 경로를 검증한다. 이 단원 e2e는 동시 세션 HMR 리로드에 특히 취약해 전용 포트 5307을 쓴다.
...
async function gotoExamIntro() {
  ...
  await page.evaluate(() => [...document.querySelectorAll(".unit-tab")].find((t) => t.textContent.includes("동물과 에너지"))?.click());
  await W(650);
...
async function answerCurrent(correct) {
  return page.evaluate(async (correct) => {
    ...
    if (type === "mcq") {
      ...
      // 소수점 키는 라벨이 "·"(mathKit mkKey("·","dot")) — "."를 매핑해야 dec 정답이 입력된다
      for (const ch of val) {
        const key = ch === "." ? "·" : ch;
 ...
 *  (g2u1은 L4~L9가 프리미엄이라, 무료 응시 세그먼트에선 오답을 앞쪽(무료 레슨 구간)에 배치해야
 *  리뷰의 "레슨 복습하기"가 페이월이 아닌 레슨 플레이어로 간다). 제출 후 결과 화면 대기. */
...
// ═══════════ 0. 발주 사진 10장 로드(exam/g2u6) ═══════════
console.log("0. 발주 사진 로드");
await page.goto(`http://localhost:${PORT}/`, { waitUntil: "domcontentloaded" });
const PHOTOS = [
  "exam/g2u6/villus.webp", "exam/g2u6/saliva-setup.webp", "exam/g2u6/blood-parts.webp",
  "body/figs/digestive.webp", "body/figs/heart.webp", "body/figs/respiratory.webp",
  "body/figs/urinary.webp", "body/figs/nephron.webp", "body/figs/alveoli.webp",
  "body/figs/v2/vessel-compare.webp", "body/figs/v2/breath-inhale.webp", "body/figs/v2/breath-exhale.webp",
  "body/figs/v2/nephron-process.webp", "body/figs/v2/double-circulation-base.webp", "body/figs/v2/pulse-wrist.webp",
];
...
ok(photoRes.every((p) => p.w > 0), "시험이 참조하는 라스터 15장 전부 로드", JSON.stringify(photoRes.filter((p) => !p.w)));
...
await shot("exam-g2u6-a-intro");
await page.evaluate(() => document.querySelector(".screen.active .btn.cta").click());
await W(650);

// 앞 6문항 오답 → 70점. 시험지는 진도 순 정렬이고 L1~L3이 최소 9문항이라, 앞 6문항은 반드시
// 무료 구간(L1~L3)이다 → 리뷰의 레슨 바로가기가 페이월이 아니라 레슨 플레이어로 간다.
const seenA = await playExam(14, (i) => i >= 6);
...
ok(stA.exams?.g2u6exam?.attempts === 1 && stA.exams?.g2u6exam?.best === 70 && stA.exams?.g2u6exam?.conquered === false, "store 기록(1회·70점·미정복)", JSON.stringify(stA.exams));
ok(stA.totalXp === 70, "XP 지급 = 점수만큼(첫 신기록)", String(stA.totalXp));
await shot("exam-g2u6-a-result");
...
const lessons = {};
for (let i = 1; i <= 6; i++) lessons[`g2u6l${i}`] = { done: true, acc: 95, bestXp: 120 };
await seed({ ...BASE, premium: true, lessons, totalXp: 70, exams: { g2u6exam: { attempts: 1, best: 70, conquered: false } } });
await gotoExamIntro();
...
ok(stC.exams.g2u6exam.attempts === 2 && stC.exams.g2u6exam.best === 100 && stC.exams.g2u6exam.conquered === true, "store 기록(2회·100점·정복)", JSON.stringify(stC.exams));
ok(stC.totalXp === 100, "누적 XP = 70 + 30", String(stC.totalXp));
await shot("exam-g2u6-c-conquered");
...
ok(stD.totalXp === 100 && stD.exams.g2u6exam.best === 100 && stD.exams.g2u6exam.attempts === 3, "store: XP 불변·최고점 유지·응시 3회");
