// m2u2(중2 수학 Ⅱ 부등식과 연립방정식) 단원 종합 평가 200제 기계 검사.
...
for (const [file, count, choiceCount, numCount, wordCount, diffSpec] of specs) {
  ...
  for (const word of ["연립부등식", "부등식의 영역", "해집합", "일차함수", "기울기", "그래프", "이차방정식", "근의 공식"]) {
    if (source.includes(word)) fail(`${file}: 금지어 "${word}" 발견`);
  }
  // '직선'은 단독만 금지 — '수직선'(수 직선 눈금)은 이 단원의 정식 어휘(○● 표기)
  if (/(?<!수)직선/.test(source)) fail(`${file}: 금지어 "직선"(수직선 제외) 발견`);
  // raw 부등호 태그 함정: 허용 태그(b·br·i·span·sub·sup·small) 밖의 <영문, 그리고 <b·<i가
  // 곧장 >·공백으로 닫히지 않는 경우("x<b일" 류)를 소스 수준에서 잡는다.
...
for (let index = 0; index < 200; index += 1) {
  const expected = `m2u2e${String(index + 1).padStart(3, "0")}`;
