// u3 v2 파일럿 갤러리 카드 전수 캡처(눈검수용 · u7 v2 계보: 그림 단독 렌더보다 문항 맥락 포함).
// PORT=6009 node qa/shot-u3v2-pilot.mjs → qa/shots/u3v2/<슬롯>.png (+preview-*.png)
import { chromium } from "playwright-core";
import { mkdirSync } from "node:fs";

const PORT = process.env.PORT || "6009";
mkdirSync("qa/shots/u3v2", { recursive: true });
...
for (let i = 0; i < n; i++) {
  ...
  await card.screenshot({ path: `qa/shots/u3v2/${name}.png` });
