// m1u5 v2 200제 중 그림 문항 전부를 4열 격자로 모아 렌더·촬영(눈검수 전용 — shot-m2u3v2-figs 계승).
// node qa/shot-m1u5v2-figs.mjs → qa/shots/m1u5v2-figs-N.png
...
all.sort((a, b) => Number(a.id.replace("m1u5e", "")) - Number(b.id.replace("m1u5e", "")));
const figs = all.filter((it) => it.figure);
const cells = figs.map((it) => `<div class="cell"><div class="cap">${it.id.replace("m1u5e", "슬롯 ")} · ${it.lessonId.replace("m1u5", "")} · ${it.type}</div>${it.figure}</div>`).join("");
...
mkdirSync("tmp/m1u5v2-figs", { recursive: true });
writeFileSync("tmp/m1u5v2-figs/index.html", html);
console.log(`그림 ${figs.length}개 격자 렌더`);
mkdirSync("qa/shots", { recursive: true });
const browser = await chromium.launch();
const page = await browser.newPage({ viewport: { width: 1240, height: 1000 } });
await page.goto(pathToFileURL("tmp/m1u5v2-figs/index.html").href, { waitUntil: "load" });
...
for (let y = 0; y < total; y += 950) {
  ...
  n += 1;
  await page.screenshot({ path: `qa/shots/m1u5v2-figs-${n}.png` });
}
console.log(`전체 높이 ${total}px → ${n}장(qa/shots/m1u5v2-figs-*.png)`);
await browser.close();
