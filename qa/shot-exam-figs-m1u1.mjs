// m1u1 시험 그림 눈검수용 스크린샷 — 풀 파일 12개에서 figure가 있는 전 문항을 자동 수집해
// 실제 저작 파라미터 그대로 한 페이지에 렌더(문항 id·유형 라벨 포함). m1u6판의 12레슨 확장.
// PORT=<포트> node qa/shot-exam-figs-m1u1.mjs (dev 서버 필수 — vite 모듈 URL로 임포트)
...
const count = await page.evaluate(async () => {
  const mods = await Promise.all(
    ["l1", "l2", "l3", "l4", "l5", "l6", "l7", "l8", "l9", "l10", "l11", "l12"].map((s) => import(`/src/content/exams/m1u1${s}.ts`)),
  );
...
// 그림이 많아 세로가 길다 — fullPage 한 장 + 스크롤 분할 4장(클립은 뷰포트 밖이라 스크롤로)
await page.screenshot({ path: "qa/shots/exam-m1u1-figs.png", fullPage: true });
...
for (let i = 0; i < SLICES; i++) {
  ...
  await page.screenshot({ path: `qa/shots/exam-m1u1-figs-${i + 1}.png` });
}
console.log(`SAVED qa/shots/exam-m1u1-figs.png (+분할 ${SLICES}장) — figure 문항 ${count}개`);
await browser.close();
