// m1u4 v2 파일럿 40문항 검증 + 시험지 모양 갤러리 렌더(m2u5 사이클 계승).
// node qa/render-m1u4v2-pilot.mjs  →  tmp/m1u4v2-pilot/index.html
import { build } from "esbuild";
import { mkdirSync, writeFileSync } from "node:fs";

async function loadPool() {
  const result = await build({
    entryPoints: ["qa/m1u4v2-pilot.ts"],
    bundle: true, write: false, format: "esm", platform: "node", logLevel: "silent",
  });
  const encoded = Buffer.from(result.outputFiles[0].text).toString("base64");
  const mod = await import(`data:text/javascript;base64,${encoded}`);
  return mod.POOL_M1U4V2_PILOT;
}
...
const cards = pool.map((it, i) => {
  const slot = it.id.replace("m1u4e", "");
  const tag = `슬롯 ${slot} · ${it.lessonId.replace("m1u4", "")} · ${it.type}${it.diff ? " · 난이도 " + "●".repeat(it.diff) + "○".repeat(3 - it.diff) : ""}${it.shuffle === false ? " · 순서고정" : ""}`;
...
<title>m1u4 v2 파일럿 40문항 — 검수용 시험지</title>
<style>
...
<div class="sheet">
<header>
  <h1>중1 수학 Ⅳ. 기본 도형 — v2 파일럿 40문항</h1>
...
mkdirSync("tmp/m1u4v2-pilot", { recursive: true });
writeFileSync("tmp/m1u4v2-pilot/index.html", html);
console.log(`렌더 완료: tmp/m1u4v2-pilot/index.html (${Math.round(html.length / 1024)}KB, 문항 ${pool.length})`);
