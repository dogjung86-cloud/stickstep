#!/usr/bin/env bash
# 다른 세션의 codex "발주 exec"가 완전히 끝나기를 기다렸다가(90초 연속 부재) u6 v2 시험 사진을 발주한다.
# 병렬 codex 금지 규칙 준수용 대기 러너. bash qa/wait-order-u6v2.sh
...
for i in $(seq 1 720); do   # 최대 2시간 대기
  ...
  if [ "${n:-0}" != "0" ] && [ -n "$n" ]; then
    ...
    if [ "$absent" -ge 90 ]; then
      echo "[wait-order] codex exec 90s absent — starting u6 v2 order"
      bash qa/order-examu6v2.sh
      exit $?
    fi
  fi
  sleep 10
