// shot-soc10-bubbles.mjs — 「100원 택시가 달리기까지」 만화 4컷의 말풍선 정렬 눈검수(컷 프레임 크롭, his 표준).
// 합격 기준: 말풍선이 **화자 머리 바로 위**(꼬리 끝이 정수리를 향함), 다른 인물·핵심 소품 가림 0,
// 컷 프레임 밖 잘림 0, 2줄 이내 가독. PORT=<포트> node qa/shot-soc10-bubbles.mjs → qa/shots/soc10-bub-*.png
...
await page.addInitScript(() =>
  localStorage.setItem(
    "science-app.v1",
    JSON.stringify({ onboarded: true, grade: "g1", viewGrade: "g1", viewSubject: "soc", premium: true, goalMin: 10, lessons: { s1u10l6: { done: true } } }),
  ),
);
...
await page.evaluate(async () => {
  ...
  nav.go(createLessonPlayer(findLesson("s1u10l6").lesson, { onExit: () => {}, onComplete: () => {} }));
});
await page.waitForTimeout(1000);
// L6 스텝: hook(1) → comic(2) — freeNav 앞으로 1번
...
for (let i = 0; i < 4; i += 1) {
  ...
  await el.screenshot({ path: `qa/shots/soc10-bub-${i}.png` });
  console.log(`SHOT soc10-bub-${i}`);
  if (i < 3) {
