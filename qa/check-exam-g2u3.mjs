// g2u3 시험 풀 기계 검사 — 커밋 전 스캔.
//   ① shuffle:false && answer===0 조합(셔플 규칙 위반) ② id 유일성·연번 ③ 유형 구성(113/18/19)
//   ④ 레슨 분포(19×6+18×2 — L3·L4가 18) ⑤ num 정답이 문자열·unitLabel 존재 ⑥ word 정답이 bank에 포함
//   ⑦ 해설 길이(태그 제외 250~450자 — 초과·미달 목록) ⑧ 그림 aria에 정답 수치 단서(수동 확인 보조)
// node qa/check-exam-g2u3.mjs
import { readFileSync } from "node:fs";

const files = ["g2u3l1", "g2u3l2", "g2u3l3", "g2u3l4", "g2u3l5", "g2u3l6", "g2u3l7", "g2u3l8"];
let all = [];
...
for (let i = 0; i < ids.length; i++) {
  const want = `g2u3e${String(i + 1).padStart(2, "0")}`;
  const want2 = `g2u3e${i + 1}`;
...
const want = { g2u3l1: 19, g2u3l2: 19, g2u3l3: 18, g2u3l4: 18, g2u3l5: 19, g2u3l6: 19, g2u3l7: 19, g2u3l8: 19 };
