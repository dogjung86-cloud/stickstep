// 단원 종합 평가(g2u7 전기와 자기) E2E — 응시 → 일괄 채점 → 8파트 진단 → 리뷰 → 레슨 바로가기 →
// 재응시 페이월 → (정복+프리미엄) 재응시 → 정복 인증 + 신기록 XP + 발주 사진 12장 로드까지 실플레이.
// PORT=<포트> node qa/e2e-exam-g2u7.mjs — dev 서버 필수(보기 선택이 dev 전용 data-oi/data-ans를 쓴다).
// g2u4판과의 차이: 레슨 8개(추출 2×4+3×4), 인트로 "여덟 파트", exam/g2u7 사진 12장.
...
async function gotoExamIntro() {
  ...
  await page.evaluate(() => [...document.querySelectorAll(".unit-tab")].find((t) => t.textContent.includes("전기와 자기"))?.click());
  await W(650);
 ...
 *  (g2u7은 L4~L8이 프리미엄이라, 무료 응시 세그먼트에선 오답을 앞쪽(무료 레슨 구간 L1~L3, 최소 6문항)에
 *  배치해야 리뷰의 "레슨 복습하기"가 페이월이 아닌 레슨 플레이어로 간다). 제출 후 결과 화면 대기. */
...
// ═══════════ 0. 발주 사진 12장 로드(exam/g2u7) ═══════════
console.log("0. 발주 사진 로드");
await page.goto(`http://localhost:${PORT}/`, { waitUntil: "domcontentloaded" });
const PHOTOS = ["balloon-wall", "water-bend", "paper-bits", "nichrome-circuit", "multitap", "string-lights", "iron-steam", "led-bulb", "filament-bulb", "coil-compass", "scrap-crane", "speaker-coil"];
const photoRes = await page.evaluate(async (names) => {
  ...
  for (const n of names) {
    const r = await new Promise((res) => {
      ...
      im.onerror = () => res(0);
      im.src = `/exam/g2u7/${n}.webp`;
    });
    out.push({ n, w: r });
  }
  return out;
}, PHOTOS);
ok(photoRes.every((p) => p.w > 0), "exam/g2u7 사진 12장 전부 로드", JSON.stringify(photoRes.filter((p) => !p.w)));
...
ok(ruleTxt.includes("여덟 파트"), "인트로 파트 수 동적 문구(여덟 파트)", ruleTxt);
ok((await page.evaluate(() => document.querySelector(".screen.active .btn.cta")?.textContent)) === "시험 시작하기", "첫 응시 CTA = 시험 시작하기");
await shot("exam-g2u7-a-intro");
await page.evaluate(() => document.querySelector(".screen.active .btn.cta").click());
await W(650);

const seenA = await playExam(14, (i) => i >= 6); // 앞 6문항(무료 레슨 구간 L1~L3) 오답 → 70점, 리뷰 점프가 무료 레슨으로 향한다
...
ok(resultA.diag.length === 8, "진단 8개 레슨 전부 표시", String(resultA.diag.length));
ok(resultA.diag.reduce((s, d) => s + d.c, 0) === 14 && resultA.diag.reduce((s, d) => s + d.t, 0) === 20, "진단 정오 합계 = 14/20");
ok(resultA.diag.every((d) => d.t === 2 || d.t === 3) && resultA.diag.filter((d) => d.t === 3).length === 4, "레슨 균형 추출(8레슨 — 2×4+3×4)", JSON.stringify(resultA.diag.map((d) => d.t)));
...
ok(stA.exams?.g2u7exam?.attempts === 1 && stA.exams?.g2u7exam?.best === 70 && stA.exams?.g2u7exam?.conquered === false, "store 기록(1회·70점·미정복)", JSON.stringify(stA.exams));
ok(stA.totalXp === 70, "XP 지급 = 점수만큼(첫 신기록)", String(stA.totalXp));
await shot("exam-g2u7-a-result");
...
const lessons = {};
for (let i = 1; i <= 8; i++) lessons[`g2u7l${i}`] = { done: true, acc: 95, bestXp: 120 };
await seed({ ...BASE, premium: true, lessons, totalXp: 70, exams: { g2u7exam: { attempts: 1, best: 70, conquered: false } } });
await gotoExamIntro();
...
ok(stC.exams.g2u7exam.attempts === 2 && stC.exams.g2u7exam.best === 100 && stC.exams.g2u7exam.conquered === true, "store 기록(2회·100점·정복)", JSON.stringify(stC.exams));
ok(stC.totalXp === 100, "누적 XP = 70 + 30", String(stC.totalXp));
await shot("exam-g2u7-c-conquered");
...
ok(stD.totalXp === 100 && stD.exams.g2u7exam.best === 100 && stD.exams.g2u7exam.attempts === 3, "store: XP 불변·최고점 유지·응시 3회");
