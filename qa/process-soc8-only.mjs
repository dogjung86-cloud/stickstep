// 사회 Ⅷ 발주분 전용 webp 변환 — process-geo.mjs의 스코프판(soc/cuts u8l*.png + comics/s1u8l7만).
// 전량 변환(process-geo)은 동시 세션의 미검수 png까지 삭제·변환해 개입하게 되므로,
// 동시 작업 중에는 자기 배치 파일만 집어 변환한다(병렬 세션 충돌 0 원칙 — soc6·soc7 관행).
// node qa/process-soc8-only.mjs
import { readFileSync, writeFileSync, readdirSync, unlinkSync, existsSync } from "node:fs";
import { chromium } from "playwright-core";

const JOBS = [
  { dir: "public/soc/cuts", match: (f) => /^u8l\d+\.png$/.test(f) },
  { dir: "public/comics/s1u8l7", match: (f) => f.endsWith(".png") },
];
