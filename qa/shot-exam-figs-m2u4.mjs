// m2u4 시험 그림 눈검수용 스크린샷 — 풀 파일 10개에서 figure가 있는 전 문항을 자동 수집해
// 실제 저작 파라미터 그대로 한 페이지에 렌더(문항 id·유형 라벨 포함). m2u2 자동화판 계승.
// PORT=<포트> node qa/shot-exam-figs-m2u4.mjs (dev 서버 필수 — vite 모듈 URL로 임포트)
...
const count = await page.evaluate(async () => {
  const mods = await Promise.all(
    ["l1", "l2", "l3", "l4", "l5", "l6", "l7", "l8", "l9", "l10"].map((s) => import(`/src/content/exams/m2u4${s}.ts`)),
  );
...
// 그림이 많아 세로가 길다 — fullPage 한 장 + 스크롤 분할 4장(m1u6 계보, 클립은 뷰포트 밖이라 스크롤로)
await page.screenshot({ path: "qa/shots/exam-m2u4-figs.png", fullPage: true });
const total = await page.evaluate(() => document.body.scrollHeight);
const sliceH = Math.ceil(total / 4);
await page.setViewportSize({ width: 420, height: Math.min(sliceH, 6000) });
for (let i = 0; i < 4; i++) {
  await page.evaluate((y) => window.scrollTo(0, y), i * sliceH);
  await page.waitForTimeout(150);
  await page.screenshot({ path: `qa/shots/exam-m2u4-figs-${i + 1}.png` });
}
console.log(`SAVED qa/shots/exam-m2u4-figs.png (+분할 4장) — figure 문항 ${count}개`);
await browser.close();
