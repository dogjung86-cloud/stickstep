// 프로필 아바타 발주 이미지 후처리 — public/avatars 하위 PNG를 512 정사각 WebP로(원본 삭제).
// process-geo.mjs와 같은 방식(playwright 캔버스) — 공용 스크립트를 건드리지 않는 전용본.
// node qa/process-avatars.mjs  (app 루트에서)
import { chromium } from "playwright-core";
import { readFileSync, writeFileSync, readdirSync, unlinkSync, existsSync } from "node:fs";

const DIR = "public/avatars";
if (!existsSync(DIR)) {
  console.error("public/avatars 없음 — 먼저 bash qa/order-avatars.sh");
  process.exit(1);
}
