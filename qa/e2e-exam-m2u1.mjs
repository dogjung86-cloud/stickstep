// 단원 종합 평가(m2u1 수학 중2 Ⅰ 유리수의 표현과 식의 계산 — 첫 중2 수학 시험) E2E —
// 응시 → 일괄 채점 → 10파트 진단 → 리뷰 → 레슨 바로가기 → 재응시 페이월 → (정복+프리미엄)
// 재응시 → 정복 인증 + 신기록 XP까지 실플레이.
// PORT=<포트> node qa/e2e-exam-m2u1.mjs — dev 서버 필수(보기 선택이 dev 전용 data-oi/data-ans를 쓴다).
// m1u6판과의 차이: 수학 지도 시딩(viewSubject "math" · viewGrade "g2"), 파트 10개("열 파트"),
// 레슨 균형 2×10, 무료 레슨이 L1~L3라 무료 응시 오답은 앞 6문항(L1~L3 구간 2×3)에 배치해야
...
async function gotoUnitTab() {
  ...
  await page.evaluate(() => [...document.querySelectorAll(".unit-tab")].find((t) => t.textContent.includes("유리수"))?.click());
  await W(650);
}
...
ok(!!nodeInfo, "수학 중2 지도(Ⅰ 유리수의 표현과 식의 계산)에 평가 노드 존재");
...
ok(introEyebrow.includes("유리수"), "인트로 단원 표기(I · 유리수의 표현과 식의 계산)", introEyebrow);
...
ok(resultA.diag.every((d) => d.t === 2), "레슨 균형 추출(10레슨 2×10)", JSON.stringify(resultA.diag.map((d) => d.t)));
...
ok(stA.exams?.m2u1exam?.attempts === 1 && stA.exams?.m2u1exam?.best === 70 && stA.exams?.m2u1exam?.conquered === false, "store 기록(1회·70점·미정복)", JSON.stringify(stA.exams));
ok(stA.totalXp === 70, "XP 지급 = 점수만큼(첫 신기록)", String(stA.totalXp));
await shot("exam-m2u1-a-result");
...
await seed({ ...BASE, premium: true, lessons, totalXp: 70, exams: { m2u1exam: { attempts: 1, best: 70, conquered: false } } });
await gotoExamIntro();
...
ok(stC.exams.m2u1exam.attempts === 2 && stC.exams.m2u1exam.best === 100 && stC.exams.m2u1exam.conquered === true, "store 기록(2회·100점·정복)", JSON.stringify(stC.exams));
ok(stC.totalXp === 100, "누적 XP = 70 + 30", String(stC.totalXp));
await shot("exam-m2u1-c-conquered");
...
ok(stD.totalXp === 100 && stD.exams.m2u1exam.best === 100 && stD.exams.m2u1exam.attempts === 3, "store: XP 불변·최고점 유지·응시 3회");
