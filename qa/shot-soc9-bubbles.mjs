// shot-soc9-bubbles.mjs — 「프닉스 언덕의 하루」 만화 4컷의 말풍선 정렬 눈검수(컷 프레임 크롭, his 표준).
// 합격 기준: 말풍선이 **화자 머리 바로 위**(꼬리 끝이 정수리를 향함), 다른 인물·핵심 소품 가림 0,
// 컷 프레임 밖 잘림 0, 2줄 이내 가독. PORT=<포트> node qa/shot-soc9-bubbles.mjs → qa/shots/soc9-bub-*.png
...
await page.addInitScript(() =>
  localStorage.setItem(
    "science-app.v1",
    JSON.stringify({ onboarded: true, grade: "g1", viewGrade: "g1", viewSubject: "soc", premium: true, goalMin: 10, lessons: { s1u9l3: { done: true } } }),
  ),
);
...
await page.evaluate(async () => {
  ...
  nav.go(createLessonPlayer(findLesson("s1u9l3").lesson, { onExit: () => {}, onComplete: () => {} }));
});
await page.waitForTimeout(1000);
// L3 스텝: hook(1) → comic(2) — freeNav 앞으로 1번
...
for (let i = 0; i < 4; i += 1) {
  ...
  await el.screenshot({ path: `qa/shots/soc9-bub-${i}.png` });
  console.log(`SHOT soc9-bub-${i}`);
  if (i < 3) {
