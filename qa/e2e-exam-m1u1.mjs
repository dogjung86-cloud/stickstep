// 단원 종합 평가(m1u1 수학 중1 Ⅰ 수와 연산 — 첫 200제 풀) E2E — 응시 → 일괄 채점 → 12파트 진단 →
// 리뷰 → 레슨 바로가기 → 재응시 페이월 → (정복+프리미엄) 재응시 → 정복 인증 + 신기록 XP까지 실플레이.
// PORT=<포트> node qa/e2e-exam-m1u1.mjs — dev 서버 필수(보기 선택이 dev 전용 data-oi/data-ans를 쓴다).
// m1u6판과의 차이: 12레슨 → pick 20이 1×4+2×8 균형, 인트로 "열두 파트", 무료 레슨 L1~L3(진도순
// 시험지의 앞 3문항은 반드시 무료 구간 — 오답을 앞 3문항에 배치해 리뷰 점프가 레슨으로 가는지 검증),
// num 음수 정답은 "+/−" 키 탭으로 입력(수와 연산 특성 — 부호 입력 경로 검증).
...
async function gotoUnitTab() {
  ...
  await page.evaluate(() => [...document.querySelectorAll(".unit-tab")].find((t) => t.textContent.includes("수와 연산"))?.click());
  await W(650);
}
 ...
 *  num 음수는 "+/−" 키, 소수점은 "·" 키(mathKit 라벨)로 입력한다. */
async function answerCurrent(correct) {
  return page.evaluate(async (correct) => {
    ...
    return {
      qid: q.dataset.qid, type, neg: type === "num" && String(ans).startsWith("-"),
...
console.log("A. 무료 첫 응시(진행 0% · 수학 중1 지도 Ⅰ)");
await seed(BASE);
await gotoUnitTab();
...
ok(!!nodeInfo, "수학 지도(Ⅰ 수와 연산)에 평가 노드 존재");
...
ok(introEyebrow.includes("수와 연산"), "인트로 단원 표기(I · 수와 연산)", introEyebrow);
const ruleTxt = await page.evaluate(() => [...document.querySelectorAll(".screen.active .ex-rule-s")].map((x) => x.textContent).join(" | "));
ok(ruleTxt.includes("열두 파트"), "인트로 파트 수 동적 문구(열두 파트)", ruleTxt);
ok((await page.evaluate(() => document.querySelector(".screen.active .btn.cta")?.textContent)) === "시험 시작하기", "첫 응시 CTA = 시험 시작하기");
await shot("exam-m1u1-a-intro");
await page.evaluate(() => document.querySelector(".screen.active .btn.cta").click());
await W(650);

const seenA = await playExam(17, (i) => i >= 3); // 앞 3문항(무료 L1~L3 구간 보장) 오답 → 85점, 리뷰 점프가 무료 레슨으로
...
console.log("  (유형 분포:", JSON.stringify(seenA.reduce((m, s) => ((m[s.type] = (m[s.type] ?? 0) + 1), m), {})), "· 음수 num", seenA.filter((s) => s.neg).length, "개)");
...
ok(resultA.diag.length === 12, "진단 12개 레슨 전부 표시", String(resultA.diag.length));
ok(resultA.diag.reduce((s, d) => s + d.c, 0) === 17 && resultA.diag.reduce((s, d) => s + d.t, 0) === 20, "진단 정오 합계 = 17/20");
ok(resultA.diag.every((d) => d.t === 1 || d.t === 2) && resultA.diag.filter((d) => d.t === 2).length === 8, "레슨 균형 추출(12레슨 1×4+2×8)", JSON.stringify(resultA.diag.map((d) => d.t)));
...
ok(stA.exams?.m1u1exam?.attempts === 1 && stA.exams?.m1u1exam?.best === 85 && stA.exams?.m1u1exam?.conquered === false, "store 기록(1회·85점·미정복)", JSON.stringify(stA.exams));
ok(stA.totalXp === 85, "XP 지급 = 점수만큼(첫 신기록)", String(stA.totalXp));
await shot("exam-m1u1-a-result");
...
await seed({ ...BASE, premium: true, lessons, totalXp: 85, exams: { m1u1exam: { attempts: 1, best: 85, conquered: false } } });
await gotoExamIntro();
...
ok(resultC.score === "100", "만점 채점(전 유형 정답 입력 — 음수 num·word 포함)", resultC.score ?? "");
...
ok(stC.exams.m1u1exam.attempts === 2 && stC.exams.m1u1exam.best === 100 && stC.exams.m1u1exam.conquered === true, "store 기록(2회·100점·정복)", JSON.stringify(stC.exams));
ok(stC.totalXp === 100, "누적 XP = 85 + 15", String(stC.totalXp));
await shot("exam-m1u1-c-conquered");
...
ok(stD.totalXp === 100 && stD.exams.m1u1exam.best === 100 && stD.exams.m1u1exam.attempts === 3, "store: XP 불변·최고점 유지·응시 3회");
