// 단원 종합 평가(m1u3 수학 중1 Ⅲ 좌표평면과 그래프 — 첫 수학 시험) E2E — 응시 → 일괄 채점 → 9파트 진단 → 리뷰 →
// 레슨 바로가기 → 재응시 페이월 → (정복+프리미엄) 재응시 → 정복 인증 + 신기록 XP까지 실플레이.
// PORT=<포트> node qa/e2e-exam-m1u3.mjs — dev 서버 필수(보기 선택이 dev 전용 data-oi/data-ans를 쓴다).
// g2u4판과의 차이: 수학 지도 시딩(viewSubject "math" · viewGrade "g1"), 발주 사진 없음(전부 SVG —
// 그림 문항은 q-figure svg 렌더로 검증), 무료 레슨이 L1~L3라 무료 응시 오답은 앞 6문항에 배치해
// 리뷰 점프가 페이월이 아닌 레슨 플레이어로 가게 한다. num dec 입력은 "." → "·" 매핑(mathKit).
import { chromium } from "playwright-core";
import fs from "node:fs";
import path from "node:path";

const PORT = process.env.PORT || "5203";
const OUT = path.resolve(process.env.SHOT_DIR || "../m1u3-temp/e2e-shots");
...
async function gotoUnitTab() {
  ...
  await page.evaluate(() => [...document.querySelectorAll(".unit-tab")].find((t) => t.textContent.includes("좌표평면과 그래프"))?.click());
  await W(650);
}
...
async function answerCurrent(correct) {
  return page.evaluate(async (correct) => {
    ...
    if (type === "mcq") {
      ...
      // 소수점 키 라벨은 "·"(mathKit) — dec 정답 "0.25"는 · 매핑으로 입력
      for (const ch of val) {
        const key = ch === "." ? "·" : ch === "-" ? "+/−" : ch;
    ...
    return {
      ...
      metaHidden: !q.textContent.includes(q.dataset.qid) && !/diff\s*[123]|\b(?:mcq|multi|num|word)\b/i.test(q.textContent),
      optionNumbersHidden: !q.querySelector(".opt-no, .option-number, [data-option-number]"),
      numpadOk: type !== "num" || (!!a.querySelector(".mnp") && a.querySelectorAll(".mnp-k").length >= 12),
    };
  }, correct);
}
...
async function playExam(correctCount, pattern) {
  ...
  await W(900);
  const wrongIds = await page.evaluate(() => [...document.querySelectorAll(".screen.active .xr-row.bad")].map((row) => row.dataset.qid));
  console.log("  (오답 ID:", JSON.stringify(wrongIds), ")");
  return seen;
}

// ═══════════ A. 무료 첫 응시 — 레슨 진행 0%, 수학 지도에서 ═══════════
console.log("A. 무료 첫 응시(진행 0% · 수학 중1 지도)");
await seed(BASE);
await gotoUnitTab();
...
ok(!!nodeInfo, "수학 지도(Ⅲ 좌표평면과 그래프)에 평가 노드 존재");
...
ok(introEyebrow.includes("III") && introEyebrow.includes("좌표평면과 그래프"), "인트로 단원 표기(III · 좌표평면과 그래프)", introEyebrow);
const ruleTxt = await page.evaluate(() => [...document.querySelectorAll(".screen.active .ex-rule-s")].map((x) => x.textContent).join(" | "));
ok(ruleTxt.includes("아홉 파트"), "인트로 파트 수 동적 문구(아홉 파트)", ruleTxt);
ok((await page.evaluate(() => document.querySelector(".screen.active .btn.cta")?.textContent)) === "시험 시작하기", "첫 응시 CTA = 시험 시작하기");
await shot("exam-m1u3-a-intro");
await page.evaluate(() => document.querySelector(".screen.active .btn.cta").click());
await W(650);

const seenA = await playExam(14, (i) => i >= 6); // 앞 6문항(무료 L1~L3 구간) 오답 → 70점, 리뷰 점프가 무료 레슨으로
...
ok(seenA.every((s) => s.metaHidden), "학생 화면에 diff·ID·유형 메타데이터 미노출");
ok(seenA.every((s) => s.optionNumbersHidden), "객관식 보기 번호 미노출");
ok(seenA.filter((s) => s.type === "num").every((s) => s.numpadOk), "num 문항은 실제 수학 숫자 패드 사용");
console.log("  (유형 분포:", JSON.stringify(seenA.reduce((m, s) => ((m[s.type] = (m[s.type] ?? 0) + 1), m), {})), ")");
...
ok(resultA.diag.length === 9, "진단 9개 레슨 전부 표시", String(resultA.diag.length));
ok(resultA.diag.reduce((s, d) => s + d.c, 0) === 14 && resultA.diag.reduce((s, d) => s + d.t, 0) === 20, "진단 정오 합계 = 14/20");
ok(resultA.diag.every((d) => d.t === 2 || d.t === 3) && resultA.diag.filter((d) => d.t === 3).length === 2, "레슨 균형 추출(9레슨 2×7+3×2)", JSON.stringify(resultA.diag.map((d) => d.t)));
...
ok(stA.exams?.m1u3exam?.attempts === 1 && stA.exams?.m1u3exam?.best === 70 && stA.exams?.m1u3exam?.conquered === false, "store 기록(1회·70점·미정복)", JSON.stringify(stA.exams));
ok(stA.totalXp === 70, "XP 지급 = 점수만큼(첫 신기록)", String(stA.totalXp));
await shot("exam-m1u3-a-result");
...
await seed({ ...BASE, premium: true, lessons, totalXp: 70, exams: { m1u3exam: { attempts: 1, best: 70, conquered: false } } });
await gotoExamIntro();
...
ok(stC.exams.m1u3exam.attempts === 2 && stC.exams.m1u3exam.best === 100 && stC.exams.m1u3exam.conquered === true, "store 기록(2회·100점·정복)", JSON.stringify(stC.exams));
ok(stC.totalXp === 100, "누적 XP = 70 + 30", String(stC.totalXp));
await shot("exam-m1u3-c-conquered");
...
ok(stD.totalXp === 100 && stD.exams.m1u3exam.best === 100 && stD.exams.m1u3exam.attempts === 3, "store: XP 불변·최고점 유지·응시 3회");
