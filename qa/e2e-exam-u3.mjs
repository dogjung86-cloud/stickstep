// 단원 종합 평가(u3 열) E2E — 응시 → 제출 일괄 채점 → 파트별 진단 → 전 문항 리뷰 → 레슨 바로가기 →
// 재응시 페이월 → (정복+프리미엄) 재응시 → 정복 인증 배지 + 신기록 XP까지 실플레이.
// PORT=<포트> node qa/e2e-exam-u3.mjs — dev 서버 필수(보기 선택이 dev 전용 data-oi/data-ans를 쓴다).
// 채점 검증 원리: 앞 N문항은 data-ans로 정답, 나머지는 고의 오답 → 점수·진단 수치가 결정적으로 예측된다.
...
async function playExam(correctCount) {
  ...
  for (let i = 0; i < 20; i++) {
    ...
    // CTA는 답을 고르기 전까지 잠겨 있어야 한다(몰입 게이트)
...
await shot("exam-a-intro");
await page.evaluate(() => document.querySelector(".screen.active .btn.cta").click());
await W(650);

const seenA = await playExam(14); // 14개 정답 → 70점
const lessonSet = new Set(seenA.map((s) => s.qid.replace(/e\d+$/, "")));
...
ok(stA.exams?.u3exam?.attempts === 1 && stA.exams?.u3exam?.best === 70 && stA.exams?.u3exam?.conquered === false, "store 기록(1회·70점·미정복)", JSON.stringify(stA.exams));
ok(stA.totalXp === 70, "XP 지급 = 점수만큼(첫 신기록)", String(stA.totalXp));
await shot("exam-a-result");

// 오답 리뷰 펼치기 — 해설·핵심 한 줄·레슨 바로가기
...
const lessons = {};
for (let i = 1; i <= 5; i++) lessons[`u3l${i}`] = { done: true, acc: 95, bestXp: 120 };
await seed({ ...BASE, premium: true, lessons, totalXp: 70, exams: { u3exam: { attempts: 1, best: 70, conquered: false } } });
...
ok(stC.exams.u3exam.attempts === 2 && stC.exams.u3exam.best === 100 && stC.exams.u3exam.conquered === true, "store 기록(2회·100점·정복)", JSON.stringify(stC.exams));
ok(stC.totalXp === 100, "누적 XP = 70 + 30", String(stC.totalXp));
await shot("exam-c-conquered");
...
ok(stD.totalXp === 100 && stD.exams.u3exam.best === 100 && stD.exams.u3exam.attempts === 3, "store: XP 불변·최고점 유지·응시 3회");
