// 단원 종합 평가(m2u4 수학 중2 Ⅳ 삼각형과 사각형의 성질 — 10레슨 200제) E2E —
// 응시 → 일괄 채점 → 10파트 진단 → 리뷰 → 레슨 바로가기 → 재응시 페이월 → (정복+프리미엄)
// 재응시 → 정복 인증 + 신기록 XP까지 실플레이.
// PORT=<포트> node qa/e2e-exam-m2u4.mjs — dev 서버 필수(보기 선택이 dev 전용 data-oi/data-ans를 쓴다).
// m2u2판과의 차이: 파트 10개("열 파트"), 레슨 균형 2×10(잔여 0), 무료 레슨 L1~L3 —
// 무료 응시 오답은 앞 6문항(L1~L3 구간 2×3)에 배치해야 리뷰 점프가 페이월이 아닌 레슨으로 간다.
// num 음수는 "+/−" 키, 소수점은 "·" 키(mathKit).
...
async function gotoUnitTab() {
  ...
  await page.evaluate(() => [...document.querySelectorAll(".unit-tab")].find((t) => t.textContent.includes("삼각형과 사각형의 성질"))?.click());
  await W(650);
}
...
ok(!!nodeInfo, "수학 중2 지도(Ⅳ 삼각형과 사각형의 성질)에 평가 노드 존재");
...
ok(introEyebrow.includes("삼각형과 사각형의 성질"), "인트로 단원 표기(IV · 삼각형과 사각형의 성질)", introEyebrow);
...
await shot("exam-m2u4-a-intro");
await page.evaluate(() => document.querySelector(".screen.active .btn.cta").click());
await W(650);

const seenA = await playExam(14, (i) => i >= 6); // 앞 6문항(무료 L1~L3 구간 2×3) 오답 → 70점, 리뷰 점프가 무료 레슨으로
...
ok(tSorted === "2222222222", "레슨 균형 추출(2×10, 잔여 0)", tSorted);
...
ok(stA.exams?.m2u4exam?.attempts === 1 && stA.exams?.m2u4exam?.best === 70 && stA.exams?.m2u4exam?.conquered === false, "store 기록(1회·70점·미정복)", JSON.stringify(stA.exams));
ok(stA.totalXp === 70, "XP 지급 = 점수만큼(첫 신기록)", String(stA.totalXp));
await shot("exam-m2u4-a-result");
...
const lessons = {};
for (let i = 1; i <= 10; i++) lessons[`m2u4l${i}`] = { done: true, acc: 95, bestXp: 120 };
await seed({ ...BASE, premium: true, lessons, totalXp: 70, exams: { m2u4exam: { attempts: 1, best: 70, conquered: false } } });
await gotoExamIntro();
...
ok(stC.exams.m2u4exam.attempts === 2 && stC.exams.m2u4exam.best === 100 && stC.exams.m2u4exam.conquered === true, "store 기록(2회·100점·정복)", JSON.stringify(stC.exams));
ok(stC.totalXp === 100, "누적 XP = 70 + 30", String(stC.totalXp));
await shot("exam-m2u4-c-conquered");
...
ok(stD.totalXp === 100 && stD.exams.m2u4exam.best === 100 && stD.exams.m2u4exam.attempts === 3, "store: XP 불변·최고점 유지·응시 3회");
