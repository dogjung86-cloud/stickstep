// 단원 종합 평가(u1 과학과 인류의 지속가능한 삶) E2E — 응시 → 일괄 채점 → 5파트 진단 → 리뷰 →
// 레슨 바로가기 → 재응시 페이월 → (정복+프리미엄) 재응시 → 정복 인증 + 신기록 XP + 사진 로드까지 실플레이.
// PORT=<포트> node qa/e2e-exam-u1.mjs — dev 서버 필수(보기 선택이 dev 전용 data-oi/data-ans를 쓴다).
// u7판과의 차이: 레슨 5개(추출은 정확히 4문항×5 · 잔여 0), 인트로 "다섯 파트" 문구,
// exam/u1 발주 사진 15장 로드 검증, num·word 0이라 넘패드·칩 분기는 타지 않는다.
// u1은 전 레슨 무료(unit1.ts에 premium 플래그 없음)라 리뷰의 레슨 점프는 어느 문항에서든 레슨으로 간다.
...
async function gotoU1ExamIntro() {
...
// ═══════════ 0. 시험 사진 로드(exam/u1 발주 15장) ═══════════
console.log("0. 시험 사진 로드");
await page.goto(`http://localhost:${PORT}/`, { waitUntil: "domcontentloaded" });
const PHOTOS = [
  "exam/u1/plant-light.webp", "exam/u1/measure-tools.webp", "exam/u1/seed-dishes.webp",
  "exam/u1/ramp-ball.webp", "exam/u1/old-microscope.webp", "exam/u1/steam-locomotive.webp",
  "exam/u1/vaccine-vial.webp", "exam/u1/satellite-dish.webp", "exam/u1/smart-speaker.webp",
  "exam/u1/ar-tablet.webp", "exam/u1/smart-home.webp", "exam/u1/solar-farm.webp",
  "exam/u1/wind-turbines.webp", "exam/u1/ev-charging.webp", "exam/u1/upcycle-bag.webp",
];
...
ok(photoRes.every((p) => p.w > 0), "시험 사진 15장 전부 로드", JSON.stringify(photoRes.filter((p) => !p.w)));
...
ok(resultA.diag.every((d) => d.t === 4), "레슨 균형 추출(5레슨 · 정확히 4문항씩 · 잔여 0)", JSON.stringify(resultA.diag.map((d) => d.t)));
...
ok(stA.exams?.u1exam?.attempts === 1 && stA.exams?.u1exam?.best === 70 && stA.exams?.u1exam?.conquered === false, "store 기록(1회·70점·미정복)", JSON.stringify(stA.exams));
ok(stA.totalXp === 70, "XP 지급 = 점수만큼(첫 신기록)", String(stA.totalXp));
await shot("exam-u1-a-result");
...
const lessons = {};
for (let i = 1; i <= 5; i++) lessons[`u1l${i}`] = { done: true, acc: 95, bestXp: 120 };
await seed({ ...BASE, premium: true, lessons, totalXp: 70, exams: { u1exam: { attempts: 1, best: 70, conquered: false } } });
await gotoU1ExamIntro();
...
ok(stC.exams.u1exam.attempts === 2 && stC.exams.u1exam.best === 100 && stC.exams.u1exam.conquered === true, "store 기록(2회·100점·정복)", JSON.stringify(stC.exams));
ok(stC.totalXp === 100, "누적 XP = 70 + 30", String(stC.totalXp));
await shot("exam-u1-c-conquered");
...
ok(stD.totalXp === 100 && stD.exams.u1exam.best === 100 && stD.exams.u1exam.attempts === 3, "store: XP 불변·최고점 유지·응시 3회");
