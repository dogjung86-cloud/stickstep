// 단원 종합 평가(m2u3 수학 중2 Ⅲ 일차함수 — 10레슨 200제) E2E —
// 응시 → 일괄 채점 → 10파트 진단 → 리뷰 → 레슨 바로가기 → 재응시 페이월 → (정복+프리미엄)
// 재응시 → 정복 인증 + 신기록 XP까지 실플레이.
// PORT=<포트> node qa/e2e-exam-m2u3.mjs — dev 서버 필수(보기 선택이 dev 전용 data-oi/data-ans를 쓴다).
...
async function gotoUnitTab() {
  ...
  await page.evaluate(() => [...document.querySelectorAll(".unit-tab")].find((t) => t.textContent.includes("일차함수"))?.click());
  await W(650);
}
...
ok(!!nodeInfo, "수학 중2 지도(Ⅲ 일차함수)에 평가 노드 존재");
...
ok(introEyebrow.includes("일차함수"), "인트로 단원 표기(III · 일차함수)", introEyebrow);
...
ok(stA.exams?.m2u3exam?.attempts === 1 && stA.exams?.m2u3exam?.best === 70 && stA.exams?.m2u3exam?.conquered === false, "store 기록(1회·70점·미정복)", JSON.stringify(stA.exams));
ok(stA.totalXp === 70, "XP 지급 = 점수만큼(첫 신기록)", String(stA.totalXp));
await shot("exam-m2u3-a-result");
...
const lessons = {};
for (let i = 1; i <= 10; i++) lessons[`m2u3l${i}`] = { done: true, acc: 95, bestXp: 120 };
await seed({ ...BASE, premium: true, lessons, totalXp: 70, exams: { m2u3exam: { attempts: 1, best: 70, conquered: false } } });
await gotoExamIntro();
...
ok(stC.exams.m2u3exam.attempts === 2 && stC.exams.m2u3exam.best === 100 && stC.exams.m2u3exam.conquered === true, "store 기록(2회·100점·정복)", JSON.stringify(stC.exams));
ok(stC.totalXp === 100, "누적 XP = 70 + 30", String(stC.totalXp));
await shot("exam-m2u3-c-conquered");
...
ok(stD.totalXp === 100 && stD.exams.m2u3exam.best === 100 && stD.exams.m2u3exam.attempts === 3, "store: XP 불변·최고점 유지·응시 3회");
