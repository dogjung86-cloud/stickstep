// m1u5 v2 파일럿 40문항 검증 + 시험지 모양 갤러리 렌더(m2u5·m1u4 사이클 계승).
// node qa/render-m1u5v2-pilot.mjs  →  tmp/m1u5v2-pilot/index.html
import { build } from "esbuild";
import { mkdirSync, writeFileSync } from "node:fs";

async function loadPool() {
  const result = await build({
    entryPoints: ["qa/m1u5v2-pilot.ts"],
    bundle: true, write: false, format: "esm", platform: "node", logLevel: "silent",
  });
  const encoded = Buffer.from(result.outputFiles[0].text).toString("base64");
  const mod = await import(`data:text/javascript;base64,${encoded}`);
  return mod.POOL_M1U5V2_PILOT;
}
...
let figured = 0;
// m1u5 언어 가드: 내각·외각은 이 단원 도입이라 허용(m1u4와 반대). 위학년 용어만 금지.
...
// num 정답 유일성: 파일럿 40 안에서는 전역 유일까지 확보(m2u5 표본 관행 — 값만 비교)
...
<title>m1u5 v2 파일럿 40문항 — 검수용 시험지</title>
<style>
...
<div class="sheet">
<header>
  <h1>중1 수학 Ⅴ. 평면도형과 입체도형 — v2 파일럿 40문항</h1>
  <p>검수용 시험지 렌더 · 교과서 준거 규격 v2(그림 우선 · 값 구하기 중심 · 그림 라벨 단위 병기 · word 0 ·
  π 답은 "a의 값" 부품 문항) 적용분. 실제 앱에서는 20문항씩 추출되고 해설은 제출 후에만 보여요.
...
mkdirSync("tmp/m1u5v2-pilot", { recursive: true });
writeFileSync("tmp/m1u5v2-pilot/index.html", html);
console.log(`렌더 완료: tmp/m1u5v2-pilot/index.html (${Math.round(html.length / 1024)}KB, 문항 ${pool.length})`);
