// g2u8 시험 풀 기계 검사 — 커밋 전 스캔.
//   ① shuffle:false && answer===0 조합(셔플 규칙 위반) ② id 유일성·연번 ③ 유형 구성(113/18/19)
//   ④ 레슨 분포(19×6 + 18×2 — 18문항 레슨은 L4·L8) ⑤ num 정답이 문자열·unitLabel 존재
//   ⑥ word 정답이 bank에 포함 ⑦ 해설 길이(태그 제외 250~450자 — 초과·미달 목록)
// node qa/check-exam-g2u8.mjs
import { readFileSync } from "node:fs";

const files = ["g2u8l1", "g2u8l2", "g2u8l3", "g2u8l4", "g2u8l5", "g2u8l6", "g2u8l7", "g2u8l8"];
let all = [];
...
for (let i = 0; i < ids.length; i++) {
  const want = `g2u8e${String(i + 1).padStart(2, "0")}`;
  const want2 = `g2u8e${i + 1}`;
...
// ④ 레슨 분포 (18문항 레슨은 L4·L8)
const per = {};
for (const it of all) per[it.file] = (per[it.file] ?? 0) + 1;
console.log("per-lesson:", JSON.stringify(per));
const want = { g2u8l1: 19, g2u8l2: 19, g2u8l3: 19, g2u8l4: 18, g2u8l5: 19, g2u8l6: 19, g2u8l7: 19, g2u8l8: 18 };
