// 사회 Ⅹ(정치과정과 시민 참여) 7레슨 실플레이 E2E — e2e-soc9 골격 + Ⅹ 신설 기함 2종.
// DEV 서버 필요(data-oi 사용). PORT=<포트> node qa/e2e-soc10.mjs
// 홈 10번째 탭 전환(voice 테마) · judgeLab 2종(ruleguard·polactor — 4개념 2×2 선반·오답 교정 경로) ·
// electLab(선거 6단계 릴레이 — 명부 3연타·등록 2연타·공약 2버튼·기표·개표 3연타·당선 확정 +
// 판정 3곳·배지 6개) · policyLab(정치과정 5정거장 릴레이 — 목소리 3연타·모으기·회의·실행·별점 +
// 담당 판정 3곳·환류 피날레·배지 5개) · comic 말풍선 개수 대조 · recap more · 개념 컷 로드 · 전 퀴즈.
// 부분 실행: ONLY=s1u10l1,s1u10l5 PORT=... node qa/e2e-soc10.mjs (홈 검증은 항상 수행)
...
// ── 홈 지도 검증(캐러셀 — Ⅹ = 10번째 탭 전환 후 카운트) ──
{
  ...
  await W(800);
  const u10map = await page.evaluate(async () => {
    const { findUnit } = await import("/src/content/curriculum.ts");
    return {
      nodes: document.querySelectorAll(".gm-node").length,
      voice: !!document.querySelector(".gm-terrain.voice"),
      // 시드가 premium:true라 크라운은 안 뜬다(잠금 표시) — 프리미엄 구성은 데이터로 검증
      prem: findUnit("s1u10").lessons.filter((l) => l.premium).length,
      coming: !!document.querySelector(".coming-card"),
      decos: document.querySelectorAll(".gm-deco").length,
    };
  });
  check(u10map.nodes === 7, `Ⅹ단원 레슨 노드 7개 (실제 ${u10map.nodes})`);
  check(u10map.voice, "Ⅹ단원 voice 테마 지형 적용(단원별 색 분리)");
  check(u10map.prem === 4, `프리미엄 레슨 4개(무료 3) (실제 ${u10map.prem})`);
  check(!u10map.coming, "comingSoon 카드 없음(신규 append 단원)");
  check(u10map.decos >= 5, `단원 데코 렌더 ≥5 (실제 ${u10map.decos})`);
}
...
// ── judgeLab(공통 — 4개념 2×2 선반, 오답 교정 경로 포함, e2e-soc9 문법) ──
const judgeStep = async (judgeId) => {
  ...
  check(shelves === def.concepts.length, `judgeLab(${judgeId}) 선반 ${def.concepts.length}개(4개념 2×2) (실제 ${shelves})`);
...
// ── Ⅹ 신설 기함 2종 ─────────────────────────────────────────
// 공용: 활성 릴레이 버튼 탭 + msn 판정(ppl-quiz)
...
// electLab: 명부 3연타 → 등록 2연타 → 공약 2버튼+판정(오답 경로) → 기표+판정 → 개표 3연타 → 확정+판정
const electStep = async () => {
  await page.waitForSelector(`${active} .ppl-scene`, { timeout: 9000 });
  for (let i = 0; i < 3; i += 1) {
    check(await actBtnTap("명단 확인하기"), `electLab 명부 확인 ${i + 1}/3`);
    await W(460);
  }
  await W(1400);
  for (let i = 0; i < 2; i += 1) {
    if (!(await actBtnTap("후보 등록 받기"))) throw new Error("등록 버튼 없음");
    await W(460);
  }
  await W(1400);
  const two = await page.evaluate(() => [...document.querySelectorAll(".screen.active .ppl-act")].filter((b) => !b.disabled).length);
  check(two === 2, `electLab 선거 운동 — 공약 버튼 2개 (실제 ${two})`);
  await actBtnTap("기호 ① 공약");
  await W(600);
  await actBtnTap("기호 ② 공약");
  await W(1000);
  await pplMsn(true); // 공약 판정(오답 경로 포함)
  if (!(await actBtnTap("기표하기"))) throw new Error("기표 버튼 없음");
  await W(900);
  await pplMsn(); // 비밀 칸막이 판정
  for (let i = 0; i < 3; i += 1) {
    if (!(await actBtnTap("개표하기"))) throw new Error("개표 버튼 없음");
    await W(460);
  }
  await W(1500);
  if (!(await actBtnTap("당선인 확정하기"))) throw new Error("확정 버튼 없음");
  await W(900);
  await pplMsn(); // 최다 득표 판정
  const badges = await page.evaluate(() => document.querySelectorAll(".screen.active .ppl-prop").length);
  check(badges === 6, `electLab 단계 배지 6개 적재 (실제 ${badges})`);
  const chips = await page.evaluate(() => document.querySelectorAll(".screen.active .pn-badge.on").length);
  check(chips === 3, `electLab 목표 3/3 (실제 ${chips})`);
  await clickCTA();
};

// policyLab: 목소리 3연타 → 모으기+판정(오답 경로) → 회의+판정 → 실행+판정 → 별점 → 환류 피날레
const policyStep = async () => {
  await page.waitForSelector(`${active} .ppl-scene`, { timeout: 9000 });
  for (let i = 0; i < 3; i += 1) {
    check(await actBtnTap("목소리 모으기"), `policyLab 목소리 ${i + 1}/3`);
    await W(460);
  }
  await W(1600);
  if (!(await actBtnTap("하나로 모으기"))) throw new Error("모으기 버튼 없음");
  await W(900);
  await pplMsn(true); // 집약 담당(정당·언론) — 오답 경로 포함
  if (!(await actBtnTap("회의 열기"))) throw new Error("회의 버튼 없음");
  await W(900);
  await pplMsn(); // 결정 담당(국회)
  if (!(await actBtnTap("정책 실행하기"))) throw new Error("실행 버튼 없음");
  await W(900);
  await pplMsn(); // 집행 담당(정부)
  if (!(await actBtnTap("별점 남기기"))) throw new Error("별점 버튼 없음");
  await W(1700);
  // 피날레 — 환류 화살표 + 최종 판정
  const loopArrow = await page.evaluate(() => (document.querySelector(".screen.active .ppl-scene")?.innerHTML ?? "").includes("환류"));
  check(loopArrow, "policyLab 피날레 — 트랙에 환류 화살표 점등");
  await page.waitForSelector(`${active} .ppl-quiz.show`, { timeout: 9000 });
  await page.evaluate(() => document.querySelectorAll(".screen.active .ppl-quiz .msn-opt")[0]?.click());
  await W(700);
  const badges = await page.evaluate(() => document.querySelectorAll(".screen.active .ppl-prop").length);
  check(badges === 5, `policyLab 정거장 배지 5개 적재 (실제 ${badges})`);
  const chips = await page.evaluate(() => document.querySelectorAll(".screen.active .pn-badge.on").length);
  check(chips === 3, `policyLab 목표 3/3 (실제 ${chips})`);
  await clickCTA();
};
...
const runLesson = async (id) => {
  ...
  for (let i = 0; i < count; i += 1) {
    ...
    else if (step.type === "electLab") await electStep();
    else if (step.type === "policyLab") await policyStep();
...
try {
  for (const id of ["s1u10l1", "s1u10l2", "s1u10l3", "s1u10l4", "s1u10l5", "s1u10l6", "s1u10l7"]) {
...
console.log(`\n===== 사회 Ⅹ e2e: ${checks - fails}/${checks} PASS${fails ? ` (${fails} FAIL)` : ""} =====`);
await browser.close();
