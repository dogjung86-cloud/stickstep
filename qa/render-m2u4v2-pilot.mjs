// m2u4 v2 파일럿 40문항 검증 + 시험지 모양 갤러리 렌더(m2u5·m1u4 사이클 계승).
// node qa/render-m2u4v2-pilot.mjs  →  tmp/m2u4v2-pilot/index.html
import { build } from "esbuild";
import { mkdirSync, writeFileSync, readFileSync } from "node:fs";

async function loadPool() {
  const result = await build({
    entryPoints: ["qa/m2u4v2-pilot.ts"],
    bundle: true, write: false, format: "esm", platform: "node", logLevel: "silent",
  });
  const encoded = Buffer.from(result.outputFiles[0].text).toString("base64");
  const mod = await import(`data:text/javascript;base64,${encoded}`);
  return mod.POOL_M2U4V2_PILOT;
}
...
// 소스 자체의 em대시(주석 포함 전면 금지) 검사
if (readFileSync("qa/m2u4v2-pilot.ts", "utf8").includes("—")) fail("소스에 em대시(—) 존재(주석 포함 금지)");
const ids = new Set();
const numAnswers = [];
...
const BAN = [
  ...
  "점대칭", "삼각비", "√", "원주각", "역이 성립", "역은 성립", "역도 성립", "의 역도", "의 역은", "의 역이",
  "2∠", "90°+½", "½∠",
];
for (const it of pool) {
  ...
  if (it.type === "multi") {
    ...
    if (!Array.isArray(it.answer) || it.answer.length < 2) fail(`${it.id} multi answer`);
      ...
      const sorted = [...it.answer].sort((a, b) => a - b);
      if (JSON.stringify(sorted) !== JSON.stringify(it.answer)) fail(`${it.id} multi answer 오름차순 아님`);
    }
  }
  if (it.type === "num") {
...
<title>m2u4 v2 파일럿 40문항: 검수용 시험지</title>
<style>
...
<div class="sheet">
<header>
  <h1>중2 수학 Ⅳ. 삼각형과 사각형의 성질: v2 파일럿 40문항</h1>
  <p>검수용 시험지 렌더 · 교과서 준거 규격 v2(그림 우선 · 값 구하기 중심 · 그림 라벨 단위 병기 · word 0) 적용분.
...
mkdirSync("tmp/m2u4v2-pilot", { recursive: true });
writeFileSync("tmp/m2u4v2-pilot/index.html", html);
console.log(`렌더 완료: tmp/m2u4v2-pilot/index.html (${Math.round(html.length / 1024)}KB, 문항 ${pool.length})`);
