// shot-soc12-bubbles.mjs — 「당연한 것들의 역사」 만화 4컷의 말풍선 정렬 눈검수(컷 프레임 크롭, his 표준).
// 합격 기준: 말풍선이 **화자 머리 바로 위**(꼬리 끝이 정수리를 향함), 다른 인물·핵심 소품(생각 구름·
// 비둘기·문서) 가림 0, 컷 프레임 밖 잘림 0, 2줄 이내 가독. c1(원경)은 말풍선 0(캡션 서사)이 정상.
// PORT=<포트> node qa/shot-soc12-bubbles.mjs → qa/shots/soc12-bub-*.png
...
await page.addInitScript(() =>
  localStorage.setItem(
    "science-app.v1",
    JSON.stringify({ onboarded: true, grade: "g1", viewGrade: "g1", viewSubject: "soc", premium: true, goalMin: 10, lessons: { s1u12l1: { done: true } } }),
  ),
);
...
await page.evaluate(async () => {
  ...
  nav.go(createLessonPlayer(findLesson("s1u12l1").lesson, { onExit: () => {}, onComplete: () => {} }));
});
await page.waitForTimeout(1000);
// L1 스텝: comic이 첫 스텝 — 바로 촬영
await page.waitForSelector(".screen.active .comic-art", { timeout: 9000 });
for (let i = 0; i < 4; i += 1) {
  await page.waitForTimeout(600);
  const el = await page.$(".screen.active .comic-art");
  await el.screenshot({ path: `qa/shots/soc12-bub-${i}.png` });
  console.log(`SHOT soc12-bub-${i}`);
  if (i < 3) {
