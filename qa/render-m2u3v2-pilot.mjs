// m2u3 v2 파일럿 40문항 검증 + 시험지 모양 갤러리 렌더.
// node qa/render-m2u3v2-pilot.mjs  →  tmp/m2u3v2-pilot/index.html
import { build } from "esbuild";
import { mkdirSync, writeFileSync, readFileSync } from "node:fs";

async function loadPool() {
  const result = await build({
    entryPoints: ["qa/m2u3v2-pilot.ts"],
    bundle: true, write: false, format: "esm", platform: "node", logLevel: "silent",
  });
  const encoded = Buffer.from(result.outputFiles[0].text).toString("base64");
  const mod = await import(`data:text/javascript;base64,${encoded}`);
  return mod.POOL_M2U3V2_PILOT;
}
...
const src = readFileSync("qa/m2u3v2-pilot.ts", "utf8");
if (src.includes("—")) fail("em대시(—) 발견(주석 포함 전면 금지)");
for (const word of ["정의역", "치역", "공역", "연립부등식", "이차함수", "이차방정식", "근의 공식", "상수함수", "x축의 방향"]) {
  if (src.includes(word)) fail(`금지어 "${word}" 발견`);
}
const ids = new Set();
const numAnswers = [];
...
for (const it of pool) {
  ...
  by[it.type] += 1;
  byDiff[it.diff] += 1;
  byLesson[it.lessonId] = (byLesson[it.lessonId] ?? 0) + 1;
  if (it.figure) {
    figured += 1;
    ...
    if (/aria-label="[^"]*(정답|=\s*-?\d+\s*(이다|입니다))/.test(it.figure)) fail(`${it.id} aria 유출 의심`);
  }
  if (it.type === "mcq") {
...
console.log(`레슨: ${Object.entries(byLesson).map(([k, v]) => `${k.replace("m2u3", "")}:${v}`).join(" ")}`);
...
const cards = pool.map((it, i) => {
  ...
  const tag = `슬롯 ${slot} · ${it.lessonId.replace("m2u3", "")} · ${it.type}${it.diff ? " · 난이도 " + "●".repeat(it.diff) + "○".repeat(3 - it.diff) : ""}${it.shuffle === false ? " · 순서고정" : ""}`;
...
<title>m2u3 v2 파일럿 40문항 검수용 시험지</title>
<style>
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  /* 변수 서체 통일: 앱(tokens.css --font-mvar) 그대로 — 문두 단독 변수(.mv)·mfmt 수식 변수(.mx-v)·
     SVG 이탤릭 라벨이 전부 같은 스택을 쓴다(갤러리와 앱의 x·y 표시 동일 보장) */
  ...
  /* mfmt 수식 표기(math.css 이식 — 갤러리 자립 렌더용) */
...
<div class="sheet">
<header>
  <h1>중2 수학 Ⅲ. 일차함수 v2 파일럿 40문항</h1>
  <p>검수용 시험지 렌더 · 교과서 준거 규격 v2(word 0 · num 40% · 그림 쿼터 40%대 · 판독값 눈금 위 ·
  발문 다이어트) 적용분. 실제 앱에서는 20문항씩 추출되고 해설은 제출 후에만 보여요.
  각 문항의 초록 줄을 누르면 정답과 해설이 열립니다.</p>
</header>
<div class="cols">
${cards}
</div>
...
mkdirSync("tmp/m2u3v2-pilot", { recursive: true });
writeFileSync("tmp/m2u3v2-pilot/index.html", html);
console.log(`렌더 완료: tmp/m2u3v2-pilot/index.html (${Math.round(html.length / 1024)}KB, 문항 ${pool.length})`);
