// 역사① Ⅲ(h1u3) 10레슨 실플레이 E2E — e2e-his2 문법 계승 + Ⅲ 신규 검증.
// DEV 서버 필요(data-oi 사용). PORT=<포트> node qa/e2e-his3.mjs  ·  ONLY=h1u3l9 부분 실행 지원.
// 검증: 홈 Ⅲ 지도(밴드 7·Ⅲ 탭 노드 10·프리미엄 7) · 훅 9장면 실조작 · comic 4편 말풍선 렌더 ·
// hotspot 종교 유산 5스팟(실사 카드) · timelineLab h1u3(전부 기원후 — 칩 2종·축 보정) 실조작 ·
// 세기 드릴 넘패드 · 유물 실사 로드(naturalWidth>0) · recap more · 전 퀴즈.
...
// ── 훅 9장면(조작 → 예측 → CTA) — HTML 버튼은 click, SVG g는 MouseEvent ──
const hookStep = async (scene) => {
  ...
  if (scene === "lambskewer") await tapHtml(".hh3-sk-btn");
  else if (scene === "examnotice") await tapSvg(".hh3-ex-note");
  else if (scene === "kanasign") await tapSvg(".hh3-kn-sign");
  else if (scene === "hanjahw") await tapSvg(".hh3-hj-note");
  else if (scene === "zeroscore") await tapSvg(".hh3-zr-board");
  else if (scene === "chessmate") await tapSvg(".hh3-ch-king");
  else if (scene === "arabnum") await tapSvg(".hh3-nm-book");
  else if (scene === "francejersey") await tapSvg(".hh3-js-shirt");
  else if (scene === "pepper") await tapSvg(".hh3-pp-mill");
...
// ── hotspot — 종교 유산 지도(전 스팟 탭 + 실사 카드 로드) ────
const hotspotStep = async (step, lessonId) => {
  ...
  check(photoOk, `[${lessonId}] hotspot 실사 카드 로드(${n}스팟 탭)`);
  await clickCTA();
};

// ── timelineLab 실조작 — 오답 교정 → 세기 2 + 사건 배치 3(전부 기원후·칩 2종) ──
const timelineStep = async (step) => {
  ...
  // Ⅲ 신규: 전부 기원후 def — 기원전 띠·원년 마커가 없어야 한다(축 보정 검증)
  ...
  check(!axis.bc && !axis.origin, "전부 기원후 연표 — 기원전 띠·원년 마커 생략");
  check(axis.firstTick === "400", `축 왼쪽 끝 눈금 400 (실제 ${axis.firstTick})`);
  ...
  check(done.events === 3, `사건 도장 3개 배치 — 서로마 멸망·헤지라·카노사 (실제 ${done.events})`);
...
// ── 레슨 주행 ────────────────────────────────────────────────
const LESSONS = ["h1u3l1", "h1u3l2", "h1u3l3", "h1u3l4", "h1u3l5", "h1u3l6", "h1u3l7", "h1u3l8", "h1u3l9", "h1u3l10"];
