// u4 v2 파일럿 갤러리 카드 전수 캡처(눈검수용 · u7 v2판 계승: 그림 단독 렌더보다 문항 맥락 포함).
// PORT=6008 node qa/shot-u4v2-pilot.mjs → qa/shots/u4v2/<슬롯>.png (+preview-*.png)
import { chromium } from "playwright-core";
import { mkdirSync } from "node:fs";

const PORT = process.env.PORT || "6008";
mkdirSync("qa/shots/u4v2", { recursive: true });
...
for (let i = 0; i < n; i++) {
  ...
  await card.screenshot({ path: `qa/shots/u4v2/${name}.png` });
