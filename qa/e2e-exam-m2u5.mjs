// 단원 종합 평가(m2u5 수학 중2 Ⅴ 도형의 닮음과 피타고라스 정리 — 11레슨 200제) E2E —
// 응시 → 일괄 채점 → 11파트 진단 → 리뷰 → 레슨 바로가기 → 재응시 페이월 → (정복+프리미엄)
// 재응시 → 정복 인증 + 신기록 XP까지 실플레이.
// PORT=<포트> node qa/e2e-exam-m2u5.mjs — dev 서버 필수(보기 선택이 dev 전용 data-oi/data-ans를 쓴다).
// m2u4판과의 차이: 파트 11개("열한 파트"), 레슨 균형 1×2+2×9(pick 20·11레슨 — 잔여 9),
// 무료 레슨 L1~L3 — 무료 응시 오답은 앞 3문항(L1~L3 구간 최소 1×3)에 배치해야
// 리뷰 점프가 페이월이 아닌 레슨으로 간다. 이 단원 num은 전부 자연수(음수·소수 키 불필요).
...
async function seed(state) {
  // addInitScript = 페이지 스크립트보다 먼저 실행(앱 부팅 저장과의 경합 차단 — 사고 기록 정석 패턴).
  ...
  // "한번 둘러보기"를 눌러야 온보딩 완료 상태가 홈으로 직행한다(구 "온보딩 시 홈 직행"은 폐기됨).
...
async function gotoUnitTab() {
  ...
  await page.evaluate(() => [...document.querySelectorAll(".unit-tab")].find((t) => t.textContent.includes("도형의 닮음과 피타고라스 정리"))?.click());
  await W(650);
}
 ...
 *  이 단원 num은 전부 자연수라 숫자 키만 필요(mathKit 넘패드). */
async function answerCurrent(correct) {
  return page.evaluate(async (correct) => {
    ...
    if (type === "mcq") {
      ...
      for (const ch of val) {
        const key = ch === "." ? "·" : ch === "-" ? "+/−" : ch;
        [...a.querySelectorAll(".mnp-k")].find((k) => k.textContent.trim() === key)?.click();
        await sleep(35);
      }
    } else {
...
/** 20문항 실플레이 — pattern(i)로 정오 배치. 제출 후 결과 화면 대기. */
async function playExam(correctCount, pattern) {
  const isCorrect = pattern ?? ((i) => i < correctCount);
  const seen = [];
  for (let i = 0; i < 20; i++) {
    ...
    const r = await answerCurrent(isCorrect(i));
...
// ═══════════ A. 무료 첫 응시 — 레슨 진행 0%, 수학 중2 지도에서 ═══════════
console.log("A. 무료 첫 응시(진행 0% · 수학 중2 지도)");
await seed(BASE);
await gotoUnitTab();
...
ok(!!nodeInfo, "수학 중2 지도(Ⅴ 도형의 닮음과 피타고라스 정리)에 평가 노드 존재");
...
const introEyebrow = await page.evaluate(() => document.querySelector(".screen.active .ex-eyebrow")?.textContent ?? "");
ok(introEyebrow.includes("도형의 닮음과 피타고라스 정리"), "인트로 단원 표기(V · 도형의 닮음과 피타고라스 정리)", introEyebrow);
const ruleTxt = await page.evaluate(() => [...document.querySelectorAll(".screen.active .ex-rule-s")].map((x) => x.textContent).join(" | "));
ok(ruleTxt.includes("열한 파트"), "인트로 파트 수 동적 문구(열한 파트)", ruleTxt);
ok((await page.evaluate(() => document.querySelector(".screen.active .btn.cta")?.textContent)) === "시험 시작하기", "첫 응시 CTA = 시험 시작하기");
await shot("exam-m2u5-a-intro");
await page.evaluate(() => document.querySelector(".screen.active .btn.cta").click());
await W(650);

const seenA = await playExam(17, (i) => i >= 3); // 앞 3문항(무료 L1~L3 구간 최소 1×3) 오답 → 85점, 리뷰 점프가 무료 레슨으로
ok(seenA.length === 20, "20문항 출제");
ok(new Set(seenA.map((s) => s.qid)).size === 20, "문항 중복 없음");
ok(seenA.every((s) => s.svgOk), "그림 문항 SVG 렌더(있다면 전부)", JSON.stringify(seenA.filter((s) => !s.svgOk)));
console.log("  (유형 분포:", JSON.stringify(seenA.reduce((m, s) => ((m[s.type] = (m[s.type] ?? 0) + 1), m), {})), "· 그림", seenA.filter((s) => s.hasFigure).length, "개)");
...
ok(resultA.score === "85", "일괄 채점 점수 85점", JSON.stringify(resultA.score));
ok(resultA.sub?.includes("17개 정답"), "정답 수 표기");
ok(resultA.xp.includes("+85 스텝"), "첫 응시 신기록 스텝(+85)");
ok(resultA.diag.length === 11, "진단 11개 레슨 전부 표시", String(resultA.diag.length));
ok(resultA.diag.reduce((s, d) => s + d.c, 0) === 17 && resultA.diag.reduce((s, d) => s + d.t, 0) === 20, "진단 정오 합계 = 17/20");
const tSorted = resultA.diag.map((d) => d.t).sort().join("");
ok(tSorted === "11222222222", "레슨 균형 추출(1×2+2×9, 잔여 9는 서로 다른 레슨에)", tSorted);
...
ok(resultA.review === 20 && resultA.wrong === 3, "전 문항 리뷰 20행 · 오답 3행");
ok(!resultA.conqBadge, "정복 전 응시라 인증 배지 없음");
const stA = await store();
ok(stA.exams?.m2u5exam?.attempts === 1 && stA.exams?.m2u5exam?.best === 85 && stA.exams?.m2u5exam?.conquered === false, "store 기록(1회·85점·미정복)", JSON.stringify(stA.exams));
ok(stA.totalXp === 85, "XP 지급 = 점수만큼(첫 신기록)", String(stA.totalXp));
await shot("exam-m2u5-a-result");
...
await shot("exam-m2u5-a-review");

// 레슨 바로가기 → 레슨 플레이어(무료 레슨) → 닫기 → 홈
await page.evaluate(() => document.querySelector(".screen.active .xr-row.bad.open .xr-lesson-btn").click());
await W(1000);
ok(await page.evaluate(() => !!document.querySelector(".screen.active.lesson-screen")), "리뷰에서 레슨 플레이어로 이동(무료 레슨)");
...
ok(nodeBest === "최고 85점", "지도 노드에 최고 점수 필", nodeBest ?? "none");
await page.evaluate(() => document.querySelector(".screen.active .gm-node.exam").click());
await W(850);
...
ok(introB.stats.some((s) => s.includes("최고 85점")) && introB.stats.some((s) => s.includes("1회 응시")), "인트로 응시 기록 칩");
await page.evaluate(() => document.querySelector(".screen.active .btn.cta").click());
await W(900);
...
const lessons = {};
for (let i = 1; i <= 11; i++) lessons[`m2u5l${i}`] = { done: true, acc: 95, bestXp: 120 };
await seed({ ...BASE, premium: true, lessons, totalXp: 85, exams: { m2u5exam: { attempts: 1, best: 85, conquered: false } } });
await gotoExamIntro();
...
ok(resultC.score === "100", "만점 채점(전 유형 정답 입력 — num·word 포함)", resultC.score ?? "");
ok(resultC.conqBadge && resultC.conqTitle.includes("정복 인증"), "정복 인증 배지");
ok(resultC.xp.includes("+15 스텝"), "신기록 갱신분만 스텝(100−85=+15)", resultC.xp);
...
ok(stC.exams.m2u5exam.attempts === 2 && stC.exams.m2u5exam.best === 100 && stC.exams.m2u5exam.conquered === true, "store 기록(2회·100점·정복)", JSON.stringify(stC.exams));
ok(stC.totalXp === 100, "누적 XP = 85 + 15", String(stC.totalXp));
await shot("exam-m2u5-c-conquered");
...
ok(stD.totalXp === 100 && stD.exams.m2u5exam.best === 100 && stD.exams.m2u5exam.attempts === 3, "store: XP 불변·최고점 유지·응시 3회");
