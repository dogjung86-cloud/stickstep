...
async function gotoExamIntro() {
  ...
  await page.evaluate(() => [...document.querySelectorAll(".unit-tab")].find((t) => t.textContent.includes("별과 우주"))?.click());
  await W(650);
 ...
 *  (g2u8은 L4~L8이 프리미엄이라, 무료 응시 세그먼트에선 오답을 앞쪽(무료 레슨 구간 L1~L3, 최소 6문항)에
...
ok(stA.exams?.g2u8exam?.attempts === 1 && stA.exams?.g2u8exam?.best === 70 && stA.exams?.g2u8exam?.conquered === false, "store 기록(1회·70점·미정복)", JSON.stringify(stA.exams));
ok(stA.totalXp === 70, "XP 지급 = 점수만큼(첫 신기록)", String(stA.totalXp));
await shot("exam-g2u8-a-result");
...
const lessons = {};
for (let i = 1; i <= 8; i++) lessons[`g2u8l${i}`] = { done: true, acc: 95, bestXp: 120 };
await seed({ ...BASE, premium: true, lessons, totalXp: 70, exams: { g2u8exam: { attempts: 1, best: 70, conquered: false } } });
await gotoExamIntro();
...
ok(stC.exams.g2u8exam.attempts === 2 && stC.exams.g2u8exam.best === 100 && stC.exams.g2u8exam.conquered === true, "store 기록(2회·100점·정복)", JSON.stringify(stC.exams));
ok(stC.totalXp === 100, "누적 XP = 70 + 30", String(stC.totalXp));
await shot("exam-g2u8-c-conquered");
...
ok(stD.totalXp === 100 && stD.exams.g2u8exam.best === 100 && stD.exams.g2u8exam.attempts === 3, "store: XP 불변·최고점 유지·응시 3회");
