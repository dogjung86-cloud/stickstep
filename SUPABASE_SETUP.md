...
>
> **버그·건의 접수함(2026-08-17 추가)**: schema.sql 맨 아래 `reports` 블록도 같은 방식으로
> 그 블록만 다시 실행하면 마이 탭의 "버그·건의 보내기"가 동작해요(안 돌리면 접수 실패 스낵).
> 접수 내용 확인은 대시보드 → Table Editor → `reports` 테이블에서 해요(클라이언트 조회 정책은
> 의도적으로 없어요. 운영자 수신함이라 앱에서는 아무도 못 읽어요).
