...
- **역사 지도는 실데이터(2026-07-20 사용자 피드백 "당·신라·발해가 뭉뚝하다" — 확장 전환 완료)**:
  ...
  **지도 그림 개편 시 그 그림을 쓰는 hotspot 스팟 %를 실좌표 검산으로 반드시 동기화**(searoutes→unit4·
  fourRivers→unit2, hisFigures 주석의 스팟 검산표가 정본). 남은 러프 개념도는 worldReligions(유산 위치
  개념도)와 silkroad(장면화)뿐 — 이후 지도 신작은 실데이터 문법이 기본. **곡선 화살표는 arrowHead/qArrow 공용
