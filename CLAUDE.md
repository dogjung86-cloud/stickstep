  ...
  **라벨 스케일 `LS = crop.w/348`**(아시아 기준 — 유럽 244 크롭에서 라벨이 1.4배 커져 겹친 실사고) +
  ...
  - **westWindLab(신규 소형 랩)**: monsoonLab과 대구 — 계절 토글 대신 **"난류가 없다면?" 가상 실험
    ...
    **흐름 속도 위계(사용자 피드백)**: 바닷물(난류)은 바람보다 뚜렷이 느리게 — 화면 기준 난류
    ≈3~5px/s(채널 편도 60~90초) vs 편서풍 ≈14.5px/s의 약 1:3(2026-07-19 "너무 빠르다" 재감속 —
    1차 캘리브레이션 26px/s도 빨랐다. 유체는 의심되면 더 느리게). 크롭이 좁아 화면 확대율이 큰
    랩일수록 같은 svg px/s가 빨라 보이니, 유체 연출 속도는 svg가 아니라 **화면 px/s로 검산**한다.
