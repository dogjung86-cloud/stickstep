...
- **하단 탭바 4개**(ui/gnav.ts, main.ts goTab이 nav.reset으로 전환): 학습(기존 홈)·복습(review.ts)·
  ...
  **복습·도전·마이 탭 상단엔 뒤로가기(.tab-back → 학습 탭 복귀, 2026-07-20)**. **비로그인 유저가 마이
  탭을 열면 로그인 화면을 위에 얹는다**(2026-07-20 사용자 확정 — 닫으면 마이. auth 스텁 모드(env 없음
  — dev·e2e)는 생략이라 e2e 무영향). **브라우저/안드로이드 하드웨어 뒤로가기 = 앱 내 뒤로가기**
...
- **게임 입장료(2026-07-20 사용자 확정 — spendXp 첫 실소비처)**: 게임을 열 때마다 보유 스텝(totalXp)에서
  ...
  ③ 차감**(순서 바꾸면 페이월행 유저 스텝을 훔침). 부족 시 스낵으로 학습 유도, 세션 내 재도전·다시하기는
  무료(요금은 카드 탭 시점뿐). 판수 하드캡 없음(잔고가 자연 제한 — 필요 시 상수 하나로 조절).
  게임 UI 통화 표기는 **'스텝'으로 통일**(구 '스틱' 문구 소급). e2e 4종은 totalXp 100 시딩+차감 검산,
  스텝 부족 차단 경로는 e2e-steprush 말미가 검증.
...
- **피커**: **과목 세그 + 학년 세그**(.wd-filters — 문제 은행이 있는 과목·학년만, 기본 = 지금 학습
  ...
  전부 함께 뽑힌다 = 과학+수학 혼합 문제지가 설계 의도. 세그 라벨의 담은 수 배지 + 요약 필(.wd-selsum
  "담은 소단원 N곳(과학 a · 수학 b)")이 이를 실시간 표기 — 각주만으론 안 보인다는 사용자 피드백,
  세그를 다중선택으로 바꾸는 안은 목록이 전 과목으로 쌓여 기각) → 소단원(레슨) 다중 선택 — 단원을
  넘나들며 담을 수 있다. 소단원·단원에 미극복 오답 수 배지
