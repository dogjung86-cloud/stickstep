...
- **미성년자 취소권(민법 5조 — 청약철회와 별개, 약관으로 배제 불가)**: 만 19세 미만이 법정대리인 동의
  ...
  (민법 6조)·사술(민법 17조 — 단순 성인 체크는 사술 아님, 판례 엄격). 페이월 파인프린트의 미성년자
  줄은 전상법 13조 2항 **의무 고지**("동의 없으면 취소 가능")라 지우면 안 된다. 결제(토스PG) 오픈 시
  **보호자 동의 체크+기록 단계**를 설계할 것(동의 입증이 유일한 실질 방어 — 타깃이 중학생이라 필수).
