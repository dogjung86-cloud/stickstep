...
- **구조**: 게임 코드는 같은 리포 **동적 import**(iframe 기각 — three 규칙과 동일). 진입·프리미엄
  ...
  전용 문구 페이월 → 구매 시 재진입), challenge.ts 카드(#btn-steprush·#btn-cosmo·#btn-onestroke·
  #btn-lasermaze, 크라운 병기)는 콜백만 부른다. 나가기 = goTab("challenge"). 섹션 헤더는
  **"쉬는 시간"**(2026-07-19 — '미니게임' 라벨 폐기), 카드 설명은 스텝 러시 문형 한 줄 통일.
  별자리 한붓그리기는 폐기(2026-07-19 사용자 확정 — starGame.ts·stg- 스타일·e2e-math 보너스 구간 삭제).
