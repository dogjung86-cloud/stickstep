...
- 폰트 Pretendard, 자간 `-.012em`. **SUIT 전환은 기각**(2026-08-03 A/B 비교샷 검토 후 사용자 확정 —
  장문 가독·숫자 명료성·눈검수 자산(캔버스/SVG 559곳 하드코딩 포함)이 Pretendard 메트릭 전제).
...
- **대시 금지(사용자 확정 2026-08-17)**: 사용자에게 보이는 모든 문구(레슨·문제·해설·UI·마케팅)에서
  문장 부호 대시(—, –)와 삽입구용 하이픈을 쓰지 않는다(AI 문체의 상징 기호). 쉼표·괄호·문장 분리로 대체.
  수학 마이너스 부호(−), 코드·CSS·파일명의 하이픈, 가운뎃점(·), 범위 물결(~)은 대시가 아니므로 그대로.
...
- **페이월 v4(2026-07-27 — "소장 + 가격 사다리". v3 "결제 구조가 곧 신뢰" 골격 계승)**: 히어로 =
  발자국 마크(홈 트레일 #bsfp 패스 재사용, 도장 스태거+깃발 팝) + **"문제집 한 권 값으로, 과목을
  통째로"**(문제집 가격 앵커) + 신뢰 칩(제공기간 12개월+무상 연장·자동결제 없음·이용 전 7일 환불) → 혜택
  2+2 → 과목 다중 선택(**개수 제한 없음**, 최소 1) → **플랜 카드 .pwx-plan(30일 패스 vs 소장·추천
  배지 — 정가 상태에서만 렌더)** → 영수증형 가격 카드(얼리버드 정가 취소선 .pwx-strike + 절약 필 +
  30일 패스 업셀 줄 .pwx-up "패스 세 번이면 소장 가격") → 파랑 CTA(.btn.cta).
  **'평생' 워딩 전면 금지(2026-07-27 사용자 확정)**: 서비스 무기한 보장을 약속하는 말이라 부채가
  되고(야나두류 논란) 소비자도 반신반의 — **'소장'으로 통일**(문제집 멘탈 모델). 히어로 소개문에는
  기간 언어를 쓰지 않는다(30일 패스와 충돌 — 기간은 플랜 카드·영수증 몫).
  **소장 제공기간 표기(2026-08-14 사용자 확정 — 토스PG 심사 대응, 되돌리지 말 것)**: PG 정책상
  서비스 제공기간 12개월 초과 상품은 입점 불가라, 소장 = **"유료 제공기간 결제일부터 12개월 +
  이후 서비스가 제공되는 동안 무상 연장"** 구조로 명문화했다(인강 업계 표준 구조 — 공단기류).
  **"기간 제한 없음" 계열 문구는 UI·문서 전면 금지**(PG에 신고한 제공기간 12개월과 불일치로 읽혀
  심사 반려 사유). 표기 4곳 = 신뢰 칩·플랜 카드 desc·영수증 pernote·파인프린트(+주문 시트 행),
  정본 문서 = refund.html 1·10절. "서비스가 제공되는 동안" 예방선은 무상 연장 절에 그대로 산다.
  entitlements의 own은 여전히 만료 없음(expires_at null) — 12개월은 "환불 채무 범위"지 이용권
  만료가 아니다. 토스 심사 답변서의 최대 제공기간 = 12개월. 심사에서 무상 연장 표기까지 지적하면
  B안(12개월 이용권 전면 전환 — 문구만 교체)으로 즉시 후퇴한다. **판매 카탈로그(SELLABLE_SUBJECTS)는 중1·중2 과학/수학 4종만** —
  중1 사회·역사는 콘텐츠가 있어도 일단 제외(2026-07-20 사용자 지시, 추후 재추가). 가격 카드 포함
  문구는 "새로운 콘텐츠가 추가되어도 모두 무료"(2026-07-20). 색 역할: 파랑 = 행동/신뢰(과목·플랜
  선택), 골드 = 브랜드 포인트(추천 배지·업셀 줄), 초록 = 절약 필. 전용 시트 styles/paywall.css
  (pwx- 네임스페이스 — .pwx-sub는 과목 버튼이 선점, 히어로 소개문은 .pwx-lead). **e2e 계약:
  .pw-title 텍스트에 "프리미엄" 포함**(qa/e2e-exam-*.mjs 24종 — 아이브로 "스틱스텝 프리미엄"이
  이 클래스를 진다). 눈검수 샷: `PORT=<포트> node qa/shot-paywall-v4.mjs`(얼리버드·정가 두 상태
  진입 + 가격·워딩 17검증 — '평생' 잔존 0 검사 포함).
- **가격 v4(2026-07-27 확정 — core/purchase.ts가 단일 진실 공급원)**: 이용권 단위 = 학년×과목이라
  실사용 수명이 ~1년 → **기간을 팔지 않고 '소장'으로 판다**(3년권 안은 겉포장이라 기각, 계정 재판매
  걱정은 가격 아닌 계정 쪽 — 약관 양도 금지+기기 제한이 담당). 플랜 2종 + 얼리버드:
  ① **소장(own)** — 정가 사다리 `priceOf(n)`: 1/2/3과목 = 14,900/24,900/33,900원(PLAN_TIERS),
  **4과목째부터 과목당 11,300원(= 33,900÷3) 고정** ×n(예: 4과목 45,200원). saveOf = 낱개 대비 절약액.
  ② **30일 시험 대비 패스(pass30)** — 과목당 4,900원 균일(PASS30, 묶음 할인 없음·단건 결제·자동 연장
  없음). 사다리 설계 = 4,900×3 ≈ 14,900("시험 세 번이면 소장 본전" 업셀). **월 정기결제는 기각·재제안
  금지**(2026-07-27 — 계속 구독하면 소장보다 손해라 해지 잘하는 체리피커만 남는 역선택 상품 + 빌링키
  심사·해지 CS 비용을 2,750원 매출로 회수 불가). ③ **얼리버드(EARLY_BIRD)** — 출시 기념 약 2달, 소장
  과목당 9,900원 균일(사다리 전 구간·4과목째 11,300원보다도 항상 저렴 = 취소선 근거). **켜져 있는 동안
  30일 패스는 페이월에서 숨긴다**(런칭 SKU 단일화). **기본값은 `active: false`**(2026-07-27 사용자
  확정 — 결제가 스텁인 동안 켜 둘 이유가 없고, 켜 두면 로컬 검수에서 패스·플랜 카드를 아예 못 본다):
  **출시(결제 오픈) 직전에 true**, 2달 뒤 다시 false가 정가 복귀(30일 패스가 함께 열림). DEV 상태
  강제 = sessionStorage `"ss.eb"`("1"/"0" — srxSeed 문법). **얼리버드 절약 필(.pwx-save)에는 금액·
  퍼센트를 쓰지 않는다(되돌리지 말 것)**: 균일가 ↔ 정가 사다리(3과목째 추가비용만 +9,000원으로 유독
  쌈 — 10,000/9,000/11,300)의 어긋남 때문에 정가 대비 절약액이 3과목에서만 꺾인다(5,000/5,100/
  **4,200**/5,600 — 퍼센트·과목당 환산도 같은 자리에서 꺾임) → 계산 오류로 읽힌다(사용자 적발).
  할인 크기는 취소선이 전달하고 필은 "출시 기념가 · 2달 한정" 희소성만 진다(shot-paywall-v4 회귀 가드).
  판매가 계산은 `priceOfPlan(plan, n)` 단일 창구, 구매는 `buyPremium({subjectIds, plan})` —
  **30일 패스의 만료 집행은 토스PG 서버 이용권(entitlement) 때 구현**(스텁·DEV 해금은 기간 개념 없음,
  store에 만료 필드 선반영 금지).
- **환불 정책(2026-07-15 확정 — 전자상거래법 17조 검토)**: "결제 후 7일 이내, **이용을 시작하지 않은
  ...
  **정식 문서화 완료(2026-08-06)**: 정본 = **public/refund.html 단 하나**(privacy.html 문법 —
  #policy-body를 screens/policy.ts 공용 화면이 fetch해 렌더, PG 심사·소비자 고지 제출 URL 겸용.
  내용 수정은 이 파일만). 진입 3곳 = 마이 탭 legal 행(main.ts openRefund — 인앱 렌더) + 스플래시
  사업자 정보 아래 + 페이월 파인프린트 아래(.pwx-fine-link — 둘은 부팅 전·prop 9곳 사정으로 정적
  URL 새 탭). 문서 규칙: 가격 숫자 금지(purchase.ts 이원화 방지 — "결제 화면에 표시된 금액" 참조),
  '평생' 금지 동일 적용, 페이월 .pwx-fine 요약과 어긋나게 고치지 말 것. 세부 확정: 이용 개시 = 그
  과목 프리미엄 콘텐츠 실행(프리미엄 레슨·재응시·프리미엄 복습 — 무료 체험 제외), 묶음 부분 환불 =
  **잔여 과목 재산정 차액 방식**(계속 이용 과목을 결제 당시 조건으로 재계산 — 할인 먹고 낱개 남기기
  차단), 미성년자 전상법 13조 2항 원문·회사 귀책 17조 3항·3영업일 환급·과오금·서비스 종료 30일 전
  공지 포함.
...
- 규칙 핵심: 시험 노드는 항상 열림 · 시험 중 정오·해설 노출 금지(일괄 채점) · 단원당 1회 무료,
  ...
  최상급(오답 선지 격파 포함, 250자+) · **무중복 순환 출제(2026-08-22 — 재응시마다 새 시험지·레슨
  균형 쿼터 불변, 같은 문항은 그 레슨 은행을 다 돌 때까지 재출제 금지. drawFreshExamItems+
  ExamRecord.seen, 상세는 EXAM_GUIDE)**.
...
- 앱 이름은 **"스틱스텝"**(core/brand.ts) — 과목이 늘어날 예정이라 브랜드에 과목명을 붙이지 않는다.
  ...
  (과목 추가 시 태그라인만 손본다).
- **첫 사용 플로우(2026-08-11 사용자 확정 — 설문 폐지)**: 스플래시 → "한번 둘러보기" →
  **바로 중1 과학 홈**(main.ts enterOnboarding이 기본값 중1·과학·하루 10분을 저장하고 goHome —
  학년은 홈 상단 세그로 전환). 구 학년·과목·학습량 설문(screens/onboarding.ts)은 배선 해제,
  파일만 보존. **과목 허브 진입은 하단 과목 탭**(2026-07-20 — 구 앱바 과목 상자
...
- **로그인 정책(결정)**: 로그인을 강요하지 않는다 — 학습·구매 모두 무로그인 가능, 기록은 기기 저장.
  ...
  **이메일/비밀번호 로그인(2026-08-14 토스PG 심사 대응)**: 소셜 버튼 아래 접힘 토글
  (.login-email-toggle → 폼) — auth.ts signInWithEmailPassword가 창구, 성공 처리는 소셜과 같은
  onAuthStateChange 공용 경로. **가입 UI는 두지 않는다**(가입 정본은 소셜 — 이메일 계정 발급은
  운영자가 Supabase 관리 API로, 심사용 테스트 계정 포함 절차는 PAYMENTS.md 심사 섹션). 비밀번호
  재설정도 미구현(발급 계정뿐이라 수요 0 — 일반 이메일 가입을 열 때 함께 설계).
  **로그인 후 목적지(2026-08-15 사용자 확정)**: 이 화면에서 방금 로그인(비로그인→로그인 전이)하면
  계정 화면을 그리지 않고 **메인 지도로 자동 진입**(login.ts extras.onLoggedIn — openLogin 기본
  "home" = resumeLearning, 스플래시 로그인 버튼도 동일). 예외 = 페이월 "로그인하고 결제 진행"
  경로만 openLogin("back")으로 페이월 복귀(홈으로 던지면 결제 흐름이 끊긴다 — 11곳 일괄).
  로그인된 상태로 연 계정 관리 화면·회원탈퇴 렌더는 불변(전이에만 발동).
...
- **단원 전환 = 방향 동조 슬라이드(2026-07-21 사용자 확정)**: 카드 드래그 중 지도(.map-swap)가
  0.35배 패럴랙스로 따라 밀리고, 커밋 순간 새 지도가 스와이프 방향에서 슬라이드+페이드 인(go()가
  오프셋 ±34px 심고 리플로 후 해제 — snapStrip instant 패턴). **"옆 지도 피크" 캐러셀은 기각**
  (단원마다 지도 높이가 달라 잘린 화면으로 읽힘 · 이웃 지도 상시 렌더 3배 · .gm-node 전역 셀렉터
  (qa 15개+) 계약 위반) — 재제안 금지. DOM엔 항상 지도 1장, 노드는 전환 즉시 존재라 e2e 무해.
- **사회·역사도 지도·카드 테마는 단원별**(2026-07-21 — "전 단원 같은 색" 수정): 팔레트·등록 절차는
  SOC_GUIDE.md(대륙 여행 7색)·HIS_GUIDE.md(유물 재질 4색 + his 잉크 폴백 버그 수정) 참조.
...
- **병합은 같은 계정·첫 로그인(게스트 흡수)일 때만 — 계정이 다르면 교체**(2026-07-21 사용자 버그
  리포트: A 로그아웃 후 같은 기기에서 B 로그인 → A 닉네임이 B 마이 탭에 보이고 profiles에도 밀려
  올라감): store.syncedUserId(기기 데이터의 주인, 로그아웃해도 유지·동기화 제외)를 fullSync가
  기록하고, 주인 ≠ 새 로그인이면 **accountSwitchPatch로 서버 상태 교체**(닉네임·avatarCustom/
  Preset·lastUnits는 리셋, 서버 행 없으면 초기값. 기기 설정·onboarded는 유지). 풀 구간은 syncing
  플래그+디바운스 취소로 이전 계정 로컬이 새 계정 행에 push되는 경합도 차단. **회원탈퇴 성공 시
  syncedUserId를 null로 해제**(login.ts — 주인 소멸 = 게스트 귀속, 재가입하면 병합 부활).
  구버전 저장분(표식 없음)은 첫 로그인이 주인을 스탬프할 때까지 1회 병합될 수 있다(과거 데이터
  한정, 소급 불가).
...
- **닉네임(2026-07-16)**: store.nickname(기기 저장, 공백 정리+12자 컷, null=기본 이름) — 마이 탭 이름 옆
  ...
  로그인 직후엔 sync.ts fullSync가 기기 값 우선으로 밀고 기기에 없으면 서버 값 채택 — **단 기기 우선은
  같은 계정·첫 로그인일 때만**(계정 전환이면 교체 패치가 로컬 닉네임을 비워 서버 값 채택, 위 병합/교체
  규칙 참조). 리더보드 표시명도 이 값.
...
- **진입**: **스플래시 = 랜딩**(별도 랜딩 화면 없음 — 사용자 확정). 플립북 정착 후 하단 버튼 3개
  ...
  (splash.ts, `.splash-foot.done` 스태거 fadeUp). **둘러보기 → 바로 중1 과학 홈**(2026-08-11 사용자
  확정 — 구 "과목 선택 → 온보딩 설문" 폐지, enterOnboarding이 기본값 저장 후 goHome). **부팅은 항상
  스플래시**(2026-07-21 공개 진입 플로우 182da0b — onboarded 시드든 재방문이든 자동 전진 없음.
  "구 재방문·e2e 시딩은 홈 직행" 문구는 폐기). onboarded면 둘러보기 버튼이 `resumeLearning()`으로
  홈에 꽂아 준다(숨김 과목의 최근 단원은 공개 게이트가 걸러 과학 지도로).
- **과목 공개 게이트(2026-08-11 사용자 확정 — 일반 사용자에게는 과학만)**: `store.canSeeAllSubjects()`
  가 단일 게이트 — **운영 계정(auth.ts PRIVILEGED_EMAILS) 로그인·검토 모드(7연타)·로컬 dev**에서만
  수학·사회·역사가 보인다(운영 주입은 main.ts onAuthChange의 `setAdminOverride` — premiumOverride와
  같은 런타임 겹층, 저장·동기화 없음·로그아웃 시 해제). 거르는 표면 = 과목 허브 카드(subject.ts)·
  페이월 판매 SKU(paywall.ts sellable — 카탈로그 SELLABLE_SUBJECTS는 불변)·취약 드릴 문제 은행
  (weakDrill.ts drillUnits)·`getViewSubject()` 클램프(항상 "sci", 저장값은 보존)·resumeLearning 복귀
  목적지. 오답노트는 실제 오답에서 과목을 유도하므로 그대로(과거 수학 오답은 계속 열람 가능 — 데이터
  존중). **e2e·QA 전 스크립트는 dev 서버로 돌므로 게이트가 항상 열려 있어 계약 불변**; 공개(실사용자)
  뷰 검수는 sessionStorage `"ss.pub"="1"`(ss.eb 문법 — 어디서든 실사용자 시점 강제, 여는 방향은 없음).
  전용 QA = `PORT=<dev포트> PPORT=<preview포트> node qa/e2e-pubgate.mjs`(dev 게이트 열림 + ss.pub
  공개 뷰 + 프로덕션 번들 실동작 14검증). 새 화면이 과목을 열거하게 되면 반드시 이 게이트를 지날 것.
  과목을 다시 공개할 때는 게이트에서 그 과목만 여는 게 아니라 노출 표면별 필터를 함께 검토한다. **e2e는 goto 직후 "둘러보기" 클릭 우회를 반드시 넣는다**(우회 도입 = qa/e2e-exam-m2u5.mjs
  seed(), **베끼는 정본은 하드닝판 qa/e2e-soc7.mjs 부팅부**): `#sc-splash` 대기 → 플립북 스킵 탭 →
  둘러보기 버튼 대기·클릭 → `#sc-home` 대기. 고정 sleep보다 이 조건 대기가 안전하다(실측 2026-07-26:
  goto(networkidle) 뒤 둘러보기 버튼까지 ~1.5초라 구 "1.2s+0.5s" 고정 대기는 여유가 거의 없다).
  **QA 스크립트 전량 적용 완료(2026-07-26)**: 홈 UI e2e 29종(구 산출 26 + `gm-node`로만 움직여 누락됐던
  e2e-his1·e2e-math·e2e-soc1 3종) + 시험 e2e 24종 + shot/check 22종. 새 QA 스크립트도 이 부팅부를 복사해 쓴다
  (홈을 거치지 않는 스플래시 촬영 구간만 예외 — qa/shot-u1.mjs 0번 샷).
  **"우회를 넣었는데도 스플래시에서 죽는다"의 진범은 대개 동시 세션의 HMR 풀리로드**(2026-07-26 규명):
  다른 세션이 src를 저장하면 내 e2e 페이지가 통째로 리로드돼 스플래시로 되돌아가고, 실패 지점은
  실행마다 달라진다(m2u4가 두 번 다른 곳에서 죽고 `@vite/client` 스텁을 물리자 10레슨 완주). 동시 작업
  중이라면 미니게임 e2e의 함정 ③ 스텁(`page.route("**/@vite/client", fulfill)`)을 함께 물릴 것.
  모듈 직접 import(router.ts + nav.go — 예: qa/e2e-g2u6v2.mjs)는 스플래시를 안 거쳐 우회가 필요 없다.
...
- **URL 해시 라우팅(2026-08-14 토스PG 심사 대응 — 계약 전문은 core/route.ts 헤더)**: 주소 있는
  화면 = 탭 5종(#/home·#/subjects·#/review·#/challenge·#/my)·과목(#/subject/sci|math|soc|his)·
  학년(#/grade/g1|g2)·로그인(#/login)·상품(#/pricing)·정책(#/refund·#/privacy). 심사 제출 URL =
  /#/pricing(상품)·/refund.html(환불 정본). 쓰기는 전부 writeHash(replaceState — **pushState 금지**,
  뒤로가기 가드가 히스토리 깊이를 소유), 읽기는 부팅 딥링크(스플래시 생략 + 미온보딩이면 둘러보기
  기본값 자동 시드) + hashchange(popstate 직후 것은 무시 — 이중 내비 차단). 아웃바운드 판별은 화면
  id(sc-splash/sc-login/sc-paywall/sc-policy[data-policy-file])와 depth·currentTab — 새 최상위
  화면을 만들면 syncHash 분기와 id를 함께 단다. 레슨·시험 등 학습 화면은 주소를 갖지 않는다.
  QA = `PORT=<포트> node qa/e2e-route.mjs`(딥링크·아웃바운드 동기·뒤로가기 이중 내비 0).
...
- **homeScreen 시그니처 변경**: onOpenGame 파라미터 제거, nav2.onTab 추가. UNIT_GAME·지도 게임 노드 삭제 —
  ...
  **폐기(2026-07-19 사용자 확정 — starGame.ts 삭제)**. **도전 탭 게임 섹션 헤더는 "쉬는 시간"**
  ...
  설명은 스텝 러시 문형("…하는 ~게임/퍼즐" 한 줄)으로 통일. **다크 아케이드 리디자인은
  2026-07-27**(미니게임 섹션의 "도전 탭 아케이드 패널" 항목 참조 — 흰 카드 나열 폐기).
...
- **구조**: 게임 코드는 같은 리포 **동적 import**(iframe 기각 — three 규칙과 동일). 진입·프리미엄
  ...
  #btn-onestroke — **카드 순서 = 코스모→스텝러시→미로→한붓, 2026-07-20 사용자 확정**)가 입장료를
  걷고 콜백을 부른다. 나가기 = goTab("challenge"). 코스모·미로 카드 설명은 "~을 배우면서" 학습
  톤(2026-07-20 사용자 지시). 별자리 한붓그리기는 폐기(2026-07-19 — starGame.ts 삭제).
- **도전 탭 아케이드 패널(2026-07-27 리디자인 — 사용자 "처음 들어왔을 때 답답해 보인다")**:
  쉬는 시간 섹션 = **밤 톤 패널 `.arc-panel`**(별 스펙 2겹 + 그라데이션 — 게임 4종이 전부 다크
  무대라 대비가 곧 "놀이 구역" 신호, 레슨 .stage 문법의 탭판) 안에 헤더(타이틀+내 스텝+오늘
  N/15판)·입장 규칙 한 줄(.play-note "한 판 입장료 20 스텝 · 하루 15판 · …")·**2×2 카드
  `.arc-tile`**(상단 3:2 키 비주얼 + 제목 + 설명 + **최고 기록 필 `.arc-rec`**(store.minigame,
  기록 없으면 "첫 도전!") + 비프리미엄 크라운). 입장료 필 4번 반복은 헤더로 일원화, **랭킹 준비 중
  카드는 그리드 아래로 강등**(죽은 카드가 첫인상을 만들지 않게). 카드 키 비주얼 = **발주
  public/game/cards/{cosmo,rush,laser,stroke}.webp**(프롬프트 qa/gamecard_prompts.txt, 발주
  bash qa/order-gamecards.sh → node qa/process-gamecards.mjs 720×480 webp — subjArt 문법: 로드
  실패 시 톤 글로우+gameIcon 폴백이라 이미지 없이도 성립). **e2e 계약 클래스는 유지**: .play-bal
  b(잔고)·.play-cnt(+done)·.play-note(입장료·소진 문구)·카드 첫 b(제목)·.prep-desc(설명)·
  .prep-pill.gold(크라운) — 스타일만 다크 재정의. 소진 문구는 "오늘 쉬는 시간은 끝났어요. 내일
  다시 열려요!"(e2e-steprush includes 검사와 정합 — 구 문구는 검사와 어긋난 잠복 불일치였다).
...
- **수익화(2026-07-15 방향 승인 — 미구현)**: 튜터는 소장 상품에 무제한으로 묶지 않는다(종량 원가).
...
- **기회(게임 오버) — 2026-07-26 사용자 지시로 도입**: 판마다 기회 `min(5, 3 + ⌊선 수/7⌋)`개(HUD
  핍, 마지막 1개는 붉게), **막다른 길 1회 = 기회 1 소모**. 다 쓰면 phase `over` + 판 실패 시트
  (기회를 다 썼어요 · 이번 판 N · 최고 기록 · 코치 팁 · 다시 도전/그만하기 — 시트 골격은 srx
  게임오버 계승, CSS는 game.css "판 실패 층"에 lzm과 **묶어** 정의). **되돌리기·처음부터는 무료**
  (되돌리기 연타로 빈 판까지 되돌릴 수 있어 과금하면 편법만 늘어난다 — 스스로 알아차린 건 실력),
  **다시 도전도 무료**(입장료·판수는 카드 탭 시점뿐)이고 **기록을 깎지 않는다**. 홀수판은 게임
  오버 시 홀수점 힌트를 켠 채 재시작(배운 규칙을 빼앗지 않는다 = 마지막 자비).
- QA: `PORT=<포트> node qa/e2e-onestroke.mjs`(67검증 — 소크·실그리기 클리어·이어 그리기·오시작
  토스트·되돌리기·막힘 2회→힌트·3회째 기회 소진→시트→다시 도전·13판 기회 커브 5/5·재플레이
  무보상·나가기 복귀·페이월 게이트). 눈검수 샷 qa/shots/onestroke-over.png.
...
- **이동 예산 + 기회(게임 오버) — 2026-07-26 사용자 지시로 도입**: 배치 퍼즐엔 막다른 길이 없어
  실패 사건을 만들어야 했다 → 한 시도의 자원 = **이동 예산 `max(8, 흩은 조각 수 × 2 + 3)`**(HUD
  "이동 N번 남음", 2 이하면 붉게. 하한 8은 조각 1~2개인 튜토리얼 1~4판 몫 — 첫 판 실패 금지). 예산을 다 쓰고도 미완성이면 시도 실패 = **기회 1 소모**(판당 3, 핍) +
  900ms 뒤 처음 배치로 자동 복귀·예산 재충전. 기회 0이면 phase `over` + 판 실패 시트(팁은 판 종류별
  — 하양/합성/유리/기본 4종). **"처음 배치로" 버튼은 배치만 되돌리고 예산은 그대로 흐른다**(예산까지
  되돌리면 리셋 연타로 게임 오버를 무한 회피할 수 있고, 반대로 기회를 물리면 마지막 기회에 누른
  순간 즉사가 된다 — 토스트가 "이동 횟수는 그대로예요"로 고지). 다시 도전은 무료·기록 불변.
- QA: `PORT=<포트> node qa/e2e-lasermaze.mjs`(65검증 — 소크·탭-탭 클리어·실드래그 문법(집기→끌기→
  놓기)·이동 집계/리셋(예산 유지)·예산 소진 3회→시트→다시 도전(여분 블록 왕복으로 예산만 태우는
  burnBudget 헬퍼)·재플레이 무보상·합성판/하양판 실플레이·페이월). 눈검수 샷 lasermaze-over.png. 눈검수
