...
- **homeScreen 시그니처 변경**: onOpenGame 파라미터 제거, nav2.onTab 추가. UNIT_GAME·지도 게임 노드 삭제 —
  ...
  (2026-07-17 사용자 확정 — screens/minigame.ts 삭제, ui.css 전용 스타일 제거. .mg-title/.mg-best만 별그리기가
  써서 잔존). 별자리 한붓그리기(starGame.ts·store.minigame 채점)는 보존 — 다시 열 때 challenge.ts에서
  starGameScreen을 연결. 도전 탭 간판 신작은 '스텝 러시'(무한의 계단 문법, 제작 중).
