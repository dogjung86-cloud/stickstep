...
- **페이월 v4(2026-07-27 — "소장 + 가격 사다리". v3 "결제 구조가 곧 신뢰" 골격 계승)**: 히어로 =
  ...
  통째로"**(문제집 가격 앵커) + 신뢰 칩(기간 제한 없는 소장·자동결제 없음·이용 전 7일 환불) → 혜택
  ...
  **'평생' 워딩 전면 금지(2026-07-27 사용자 확정)**: 서비스 무기한 보장을 약속하는 말이라 부채가
  되고(야나두류 논란) 소비자도 반신반의 — **'소장'으로 통일**(문제집 멘탈 모델). 약관에는 "서비스
  제공 기간 내 기간 제한 없음"으로 결제 오픈 때 명문화하며, 파인프린트의 "서비스가 제공되는 동안"
  줄이 그 예방선이라 지우지 말 것. 히어로 소개문에는 기간 언어를 쓰지 않는다(30일 패스와 충돌 —
  기간은 플랜 카드·영수증 몫). **판매 카탈로그(SELLABLE_SUBJECTS)는 중1·중2 과학/수학 4종만** —
