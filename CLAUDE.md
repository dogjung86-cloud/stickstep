...
- **m1u2(수학 중1 Ⅱ 문자와 식) 200문항**: 9레슨을 22×7+23×2로 배분하고 유형 120/60/20,
  diff 80/80/40을 정확히 맞춘다. 대수 num은 음수 정수·유한소수를 허용하되 저장값만 ASCII 하이픈,
...
- **세이프가드(현재 구현)**: 시스템 프롬프트(해요체·이모지/마크다운 금지·과학/수학 범위 제한·개인정보
  금지·유해 주제 거절·힌트 우선 교수법·되물음 마무리) + safetySettings 보수 세팅 + 화면 상단 상시
  AI 고지("AI가 만드는 답변이라 가끔 틀릴 수 있어요"). 답변은 textContent로만 렌더(XSS 차단) —
