...
1. **추상 지침보다 기준 구현 복제.** 새 코드를 설계하지 말고, 아래 '기준 구현 표'의 파일을 열어
   구조를 그대로 옮긴 뒤 내용만 바꾼다. 확립된 문법(버튼 프레스, 채점 플로우, 랩 뼈대)은 재설계 금지.
...
### ⑦ 마무리
- 이번 단원에서 생긴 **새 관행을 CLAUDE.md에 기록**(다음 세션의 자산). 커밋 메시지는 기존 스타일(한 줄 제목 + 불릿).
