...
1. `CLAUDE.md` 전체 — 특히 "인터랙션 원칙", "콘텐츠 규칙", "훅 예측 규칙", "표준 레슨 공식", 해당 단원 유형 섹션.
   **트랙·시험 헌법은 분권됨(2026-07-21)**: 수학 = `MATH_GUIDE.md`, 사회 = `SOC_GUIDE.md`,
   역사 = `HIS_GUIDE.md`, 단원 종합 평가 = `EXAM_GUIDE.md` — 해당 작업이면 그 헌법 전체도 필독
   (CLAUDE.md의 요약 스텁만 보고 착수 금지).
...
- 이번 단원에서 생긴 **새 관행을 기록**(다음 세션의 자산) — 전 과목 공통이면 CLAUDE.md, 트랙·시험
  전용이면 해당 헌법(MATH/SOC/HIS/EXAM_GUIDE.md). 커밋 메시지는 기존 스타일(한 줄 제목 + 불릿).
